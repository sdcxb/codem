...
const UNUSED_VAR_ALLOWLIST = new Map([
  ...
  /*
   * 第 45 轮（D-3）登记：`--font-family` 是 `--font-ui` 的**兼容别名**（`styles.css:41`），
   * 面向的消费者是**第三方插件 CSS**（插件里可以写 `var(--font-family, …)`），不是本仓库。
   *
   * 为什么现在才被报出来：这条规则的"引用"判定把 JS 里的 `setProperty("--font-family", …)`
   * 也算引用 —— 而设置页/侧栏此前正是往这个**没有任何 var() 消费方**的变量上写字，
   * 于是"全局字体"这个设置项从来没有生效点（D-3 的病灶）。修好那两处写入后，
   * 本仓库里就只剩"定义"，于是被规则报成死令牌。
   *
   * 保留而不删的理由：它是文档化的对外契约（别名让插件跟随用户的 `--font-ui` 选择）；
   * 删掉它会让插件里 `var(--font-family)` 整条声明失效（插件必须能独立渲染，但别名本身
   * 就是给它们的）。因此按本清单的既有用途（公开刻度/对外契约）登记豁免。
   */
  ["--font-family", "对外兼容别名（= var(--font-ui)），消费者是第三方插件 CSS，本仓库内已无引用"],
]);
