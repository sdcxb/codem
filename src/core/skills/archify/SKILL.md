...
## Codem environment note

This skill is bundled inside the Codem desktop app. When you run the CLI from a
shell, `bin/`, `schemas/`, `examples/`, `assets/` and `references/` are relative
to this skill directory. Resolve it first, e.g.:

```bash
cd "<skill_dir>/src/core/skills/archify"   # absolute path on the user's machine
node bin/archify.mjs validate <type> <candidate.json> --quality showcase --json
```

Deliver HTML into the workspace the user asked about (e.g. `artifacts/archify/`),
then report the absolute HTML path. `meta.repository` source evidence requires
passing `--repo-root <checkout>` matching the declared revision.
