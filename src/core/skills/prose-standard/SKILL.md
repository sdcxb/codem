---
name: prose-standard
description: Use when writing, reviewing, restoring, trimming, or auditing prose — comments, JSDoc, code comments, documentation, prompts, descriptions, diagnostics, and CLI or UI strings. Owns editorial judgment and required prose coverage across all surfaces.
tags: ["prose", "writing", "review", "documentation", "quality"]
version: "1.0.0"
author: "Codem"
forcePreload: false
---
...
Always exclude `vendor/` and `node_modules/` from discovery, review, and edits. Put exclusions after inclusion globs so a later include cannot re-admit them.
...
- **READMEs:** include the consumer contract: configuration, semantics, failures, limitations, extension points, and model-visible effects. Quote stable model-visible text owned by the package; link generated catalogs and cross-package owners. Keep durable gaps and maintainer traps, not ordinary cleanup inventories.
...
1. Confirm the scope, mode, and applicable coding standards. Do not inspect unrelated areas.
...
6. Run the narrow relevant checks, lint, and behavior tests for visible strings. Verify the final diff contains no `vendor/` or `node_modules/` path.
