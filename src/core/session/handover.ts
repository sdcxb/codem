...
export interface HandoverCheck {
  ...
  warning?: string;
  /** 机械量到的信息，用于日志/测试 */
  stats: { chars: number; hasAbsolutePath: boolean; hasPointer: boolean; hasDoneCriteria: boolean; hasNoRescan: boolean };
}
...
export function checkHandover(task: string): HandoverCheck {
  ...
  const stats = {
    ...
    hasDoneCriteria: DONE_CRITERIA.some((k) => text.toLowerCase().includes(k.toLowerCase())),
