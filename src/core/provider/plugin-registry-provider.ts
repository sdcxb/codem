...
export const pluginRegistryProvider: Plugin = (ctx: any) => {
  ...
  const knownPlugins = [
    ...
    { name: '@codem/sandbox-local', version: '1.0.0', description: 'Sandbox Provider — 沙箱隔离执行环境', provides: ['sandbox'], inject: [], keywords: ['sandbox'], category: 'provider', tags: ['provider', 'security'], hot: true, icon: '📦', author: 'Codem Team', riskLevel: 'caution', riskDescription: '关闭后沙箱执行环境不可用，代码运行将在直接本地执行', status: 'stub' },
    ...
    // ===== P6 Providers（扩展能力） =====
    { name: '@codem/identity', version: '1.0.0', description: 'Identity Provider — 身份认证服务', provides: ['identity'], inject: [], keywords: ['identity'], category: 'provider', tags: ['provider', 'security'], icon: '🪪', author: 'Codem Team', riskLevel: 'safe', riskDescription: '关闭后身份认证不可用', status: 'stub' },
    ...
    { name: '@codem/context-info', version: '1.0.0', description: 'Context Info Provider — 上下文信息收集', provides: ['contextInfo'], inject: [], keywords: ['context'], category: 'provider', tags: ['provider'], icon: '📋', author: 'Codem Team', riskLevel: 'safe', riskDescription: '关闭后上下文信息收集不可用', status: 'stub' },
    { name: '@codem/commands', version: '1.0.0', description: 'Commands Provider — 命令注册服务，快捷操作', provides: ['commands'], inject: [], keywords: ['commands'], category: 'provider', tags: ['provider', 'tool'], icon: '⌘', author: 'Codem Team', riskLevel: 'safe', riskDescription: '关闭后自定义命令快捷操作不可用', status: 'stub' },
    ...
    { name: '@codem/dynamic-runner', version: '1.0.0', description: 'Dynamic Runner Provider — 动态运行器，运行时代码执行', provides: ['dynamicCordisRunner'], inject: [], keywords: ['runner'], category: 'provider', tags: ['provider', 'runtime'], icon: '⚡', author: 'Codem Team', riskLevel: 'safe', riskDescription: '关闭后动态运行器不可用', status: 'stub' },
    { name: '@codem/plugin-registry', version: '1.0.0', description: 'Plugin Registry Provider — 插件注册表服务，插件市场的数据源', provides: ['pluginRegistry'], inject: [], keywords: ['registry'], category: 'core', tags: ['infra', 'service'], icon: '📜', author: 'Codem Team', riskLevel: 'caution', riskDescription: '关闭后插件市场将无法显示插件列表和搜索', status: 'stub' },
    ...
    { name: '@codem/guard', version: '1.0.0', description: 'Guard Provider — 工具调用去重和截止时间检查，防止 Agent 循环', provides: ['guard'], inject: [], keywords: ['guard', 'dedup'], category: 'provider', tags: ['provider', 'security'], icon: '🛡️', author: 'Codem Team', riskLevel: 'safe', riskDescription: '关闭后工具调用去重不可用，Agent 可能重复调用相同工具', status: 'stub' },
    { name: '@codem/credentials', version: '1.0.0', description: 'Credentials Provider — API key 和密钥管理服务', provides: ['credentials'], inject: [], keywords: ['credentials', 'api-key'], category: 'provider', tags: ['provider', 'security'], icon: '🔑', author: 'Codem Team', riskLevel: 'danger', riskDescription: '关闭后 API key 管理不可用，LLM Provider 无法获取密钥', status: 'stub' },
    { name: '@codem/attachments', version: '1.0.0', description: 'Attachments Provider — 内容寻址附件存储服务', provides: ['attachments'], inject: [], keywords: ['attachments', 'content'], category: 'provider', tags: ['provider', 'storage'], icon: '📎', author: 'Codem Team', riskLevel: 'safe', riskDescription: '关闭后附件内容寻址存储不可用', status: 'stub' },
    { name: '@codem/schedule', version: '1.0.0', description: 'Schedule Provider — 定时提醒和调度服务', provides: ['schedule'], inject: [], keywords: ['schedule', 'reminder'], category: 'provider', tags: ['provider', 'automation'], icon: '⏰', author: 'Codem Team', riskLevel: 'safe', riskDescription: '关闭后定时提醒不可用', status: 'stub' },
    { name: '@codem/plans', version: '1.0.0', description: 'Plans Provider — 结构化任务计划管理，步骤跟踪和持久化', provides: ['plans'], inject: [], keywords: ['plans', 'steps'], category: 'provider', tags: ['provider', 'agent'], icon: '📝', author: 'Codem Team', riskLevel: 'safe', riskDescription: '关闭后结构化计划管理不可用，步骤进度不持久化', status: 'stub' },
    { name: '@codem/preset', version: '1.0.0', description: 'Preset Provider — 配置预设管理，批量设置加载/保存', provides: ['preset'], inject: [], keywords: ['preset', 'config'], category: 'provider', tags: ['provider'], icon: '🎛️', author: 'Codem Team', riskLevel: 'safe', riskDescription: '关闭后配置预设不可用', status: 'stub' },
    { name: '@codem/host-client', version: '1.0.0', description: 'Host/Client Provider — 多端协同架构基础（Bundle/SDK/ACP/Host/Client/Installer）', provides: ['bundle', 'sdk', 'acp', 'host', 'client', 'pluginInstaller'], inject: [], keywords: ['host', 'client', 'rpc', 'remote'], category: 'provider', tags: ['provider', 'infra'], icon: '🌐', author: 'Codem Team', riskLevel: 'safe', riskDescription: '关闭后多端协同和插件安装功能不可用', status: 'stub' },
    ...
    { name: '@codem/ui-skin-default', version: '1.0.0', description: 'Default Skin Plugin — 默认皮肤配色方案', provides: [], inject: [], keywords: ['skin', 'default'], category: 'ui', tags: ['ui', 'theme'], icon: '🎨', author: 'Codem Team', riskLevel: 'safe', riskDescription: '关闭后默认配色不可用，界面恢复浏览器默认样式' },
    { name: '@codem/ui-skin-pet', version: '1.0.0', description: 'Pet Skin Plugin — 宠物皮肤配色变体', provides: [], inject: [], keywords: ['skin', 'pet'], category: 'ui', tags: ['ui', 'theme'], icon: '🐱', author: 'Codem Team', riskLevel: 'safe', riskDescription: '关闭后宠物皮肤不可用' },
    { name: '@codem/ui-pet', version: '1.0.0', description: 'Pet Overlay UI Plugin — 宠物覆盖层 UI 组件', provides: [], inject: ['slots'], slots: ['app.overlay'], keywords: ['pet', 'overlay'], category: 'ui', tags: ['ui'], icon: '🐾', author: 'Codem Team', riskLevel: 'safe', riskDescription: '关闭后宠物覆盖层不显示' },
  ]
