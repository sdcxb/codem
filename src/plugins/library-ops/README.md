...
| **相机可交互** | 滚轮缩放（以指针为锚点，**手动档位 0.3×–3.2×**）/ 拖拽平移 / 双击复位 / HUD 缩放与在馆统计 / 「适应窗口」+ 选中角色镜头平滑居中。**「适应窗口」不受 0.3× 手动下限约束**（概览卡片的宿主只有约 481×191，把 1920×1072 的画布塞进去需要 ≈0.18×；下限只保留 0.05× 的病态兜底）—— 两条下限的区分见 `core/scene-view.ts` 文件头 |
...
| `src/test/library-ops-pixel-hit-area.test.tsx` | **房间热区命中归属**：角色层不吃命中 / 两处死交互接线 / 同 bounds 两岗位的命中条带（外层关命中 + 内层接管）/ 对位模式不切分 / 指针捕获时机 / 高亮跟随选中岗位（9 例）。**弱判据** —— happy-dom 不做布局，真机命中归属由 `.preview-shot/audit-loroom*.mjs` 承担 |
| `src/test/library-ops-scene-fit.test.ts` | **取景缩放策略**：`fitViewFor` 允许低于手动下限 0.3（概览 481×191 → ≈0.1766）/ 画布整幅进视口且居中 / 上限与病态兜底 / 手动缩放仍守 0.3 / 已在 0.3 以下时"缩小"不得把它抬回（7 例） |
| `src/test/library-ops-room-hit-split.test.ts` | **同 bounds 热区切分（纯几何）**：两块条带互不重叠且都在原框内 / 各含自己的锚点 / 82 点网格各自 ≥20 点（实测 27 与 55）/ 其余 10 间不受影响 / 三岗位与锚点重合的通用性（8 例） |
...
| **「适应窗口」与手动缩放下限分开** | 一条下限服务两个目标时会互相打架：概览卡片的宿主只有约 481×191，装下 1920×1072 需要 ≈0.18×，而手动缩放下限 0.3× 会把 fit 顶回去（真机：画布 576×321.6 被裁掉上 65.3 / 左右 47.5，8 个房间可见区 < 1）。故 `fitMin = 0.05`（只防病态宿主）与 `manualMin = 0.3`（防用户把画面缩成看不清的点）分列；手动缩放的下限取 `min(manualMin, current)`，避免 fit 之后的"按缩小反而被放大"。见 `core/scene-view.ts` |
| **同 bounds 的多个岗位 → 按锚点切热区（不挪视觉）** | `gateway`(前台·调度台) 与 `task_queues`(借还台·交付) 共用同一块 `bounds`，两个热区逐像素重合 ⇒ 靠后的那个永远赢，前者 82 点采样命中自己 **0**。做法：按各岗位**标签锚点**把矩形切成互不重叠的条带（每块含自己的锚点 ⇒ "点谁的名字附近选谁"），命中与视觉解耦（外层 `.is-hit-split` 关命中、内层 `.lo-pixel-room__hit` 接管）。否决"内缩一圈"（条带太窄且与标签无关）与"两块之间循环"（点一下选谁不确定）。见 `core/room-hit-area.ts` |
| **指针捕获推迟到真正拖拽时** | 原来在 `pointerdown` 里就 `setPointerCapture`，同一次手势的 `pointerup/mouseup/click` 全被改派到 `.lo-scene` ⇒ 房间热区的 `onClick` 永不触发（真机事件链实测）。现在"只点不拖"不取捕获（click 落到真实目标上），"移动超过 2px 才取"（拖出场景外仍跟手，且这次手势自然选不中任何东西） |
