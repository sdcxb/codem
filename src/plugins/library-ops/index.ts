 ...
 * - 监控界面完全对标 lobster-pet（标题栏 + KPI 卡 + 卡片网格 + 实时事件流 +
 *   场景嵌入），把图书馆作为监控界面内的「场景」页签；
 * - 启停互不影响：插件禁用 → provider 不装配 → 入口与面板都不存在，
 *   宿主功能与数据零变化（插件全程只读宿主数据）。
 *
 * 公共导出（供宿主或其它插件复用）：
 * - `LibraryOpsLauncher` 入口组件（provider 注册到 `app.overlay`）
...
export { LibraryOpsLauncher, OPEN_EVENT } from "./components/LibraryOpsLauncher";
export { LibraryOpsPanel } from "./components/LibraryOpsPanel";
