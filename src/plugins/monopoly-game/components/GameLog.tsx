  ...
  return (
    <div className="game-log">
      ...
      {priceIndex !== undefined && (
        <div style={{ fontSize: 'var(--fs-sm)', color: "#bdc3c7", marginBottom: 4, padding: "2px 4px", background: "rgba(44, 62, 80, 0.4)", borderRadius: "var(--radius-xs)" }}>
          物价指数: <span style={{ color: "#f1c40f", fontWeight: "bold" }}>{priceIndex}</span>
        </div>
      )}
      {/* G23: 胜利条件 */}
      {winningMultiplier !== undefined && winningMultiplier > 0 && (
        <div style={{ fontSize: 'var(--fs-sm)', color: "#bdc3c7", marginBottom: 4, padding: "2px 4px", background: "rgba(241, 196, 15, 0.1)", borderRadius: "var(--radius-xs)" }}>
