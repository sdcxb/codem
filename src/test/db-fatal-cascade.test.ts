 ...
 * | 同上 **STOR-H3/H4/H5** | 载入时 `PRAGMA quick_check` 失败（或打不开）→ 备份 `.corrupt-` + 删除 + 重建 | `quick_check` 现在由新引擎承担：`db-contract.test.ts` **C5**（`integrity` ok）、**C26**（真实生产库副本 quick_check 必须 ok）+ Rust `integrity_check_reports_ok`；"库损坏/丢失 → 重建 + schema 幂等"由 **C25**（把主库+WAL+SHM 全删后重开 = 全新库、`integrity` ok）与 **C24**（反复打开幂等）守。**第 19 轮已补齐备份那一步**（这句话原来写的是"没有等价物"，已作废）：**C28**（走**真 CLI**：造一个"SQLite 头 + 乱码正文"的文件 → `init` 必须成功 → 目录里必须出现 `.corrupt-<ts>` 改名备份 → 重建后的库可写可读，且 `init.result.recovered_from` 必须指向那个备份）+ Rust `corrupt_database_is_backed_up_and_rebuilt`（① 普通 `open` 必须报 `CORRUPT` ② `open_with_recovery` 给出备份路径且备份与坏文件**同内容**（改名而非改写）③ 重建库 schema 就位、可读可写、完整性通过） |
