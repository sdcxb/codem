...
function seedPort(): FakeStoragePort | null {
   ...
   * 与其留一个不成立的守卫，不如把不变量**断言出来**（见本文件 **MR-6**：
 * "生产代码里**不许再有旧引擎入口**"）。
 *
 * ⚠️ 第 58 轮更正：这句注释原来写的是"（见下面 port 赋值处的 **MR-0** 说明）"——
 * 那个编号**从来不存在**（本文件的编号是 MR-1/2/3/6/7…14），
 * 而"port 赋值处"（`seedPort()` 里的 `setStoragePort(p)`）也只有第 18 轮的夹具说明。
 * 也就是说这句引用**指向了不存在的东西** —— 排查的人会去翻一个不存在的台账。
 * 这正是审计里「覆盖移交**未逐条对齐**」的同类形态：**注释在说"谁守着这件事"，
 * 而被点名的编号不存在**。现在指向真实的 MR-6。
   */
