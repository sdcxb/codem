...
同一条链路的另一半：命令名对齐了，**错误码**不对齐照样静默失真 ——
引擎发一个渲染侧不认识的 code 会被映射成 `OTHER`（错误码这个"值"就丢了），
而两侧 `retryable` 规则不一致会导致"一直在重试引擎说别重试的失败"或反过来。

门禁把三件事都钉住（全是解析字面量，不做语义推断）：

| 判据 | 当前实测 |
| --- | --- |
| 引擎 `as_str()` 的字符串集合 == 渲染侧 `StorageErrorCode` 联合类型 | **10 个逐一相同** |
| 引擎 `retryable()` 的变体（换算成线上字符串）== 渲染侧 `RETRYABLE` | **4 个相同**：BUSY / LOCKED / IO / UNAVAILABLE |
| 每个 code 都有 `hint()`（界面与日志的可执行建议） | **10/10 覆盖** |

**验证过有牙齿**：往渲染侧联合类型里加一个引擎永不发的 `"TEAPOT"` → 门禁立刻报
`渲染侧声明了错误码 TEAPOT，但引擎 as_str() 永远不会发它`。
用例 `audit-gates.test.ts::GATE-11` 同时守真仓库（零分歧 + 金丝雀 ≥5 个码）与"故意写坏的夹具"。
...
### 第 36 轮：最终出口也 0 命中 —— 渲染侧 JS 已可完整排除

按上一轮的计划，在 `tauriTransport.invokeCommand` / `invokeBatch` 上装了同一个留痕
（`[IpcTrace]`）。选择这一层的理由：**所有** IPC 都必须经过它 ——
`RustDataPort`、三个镜像类、任何直接持有 transport 的代码都绕不过去。
装之前也专门确认了 `RustStoragePort` 的 transport 默认值就是它（`rust-port.ts:1359/1504`）。

结果：**0 命中**。而同一轮里：

| 对照 | 结果 |
| --- | --- |
| 监听通道本身有效 | `__codemDb` 可调用、日志收到 **117 条** |
| 点击确实触发了删除 | `821→0`、`3→1`；审计记下 `sessions` 1 行（key = 我点开的那个会话）+ 其全部子行 |
| **应用开着、不碰任何东西 20 秒** | `821/3` **完好**、审计为空 |
| 新代码确实在运行的 bundle 里 | `IpcTrace` 在 `bootstrap-*.js`（懒加载 chunk） |

### 完整排除矩阵（全部真机实测，共 6 层）

| 层 | 仪器 | 命中 |
| --- | --- | --- |
| 端口三层 `data.execute/write/command` | 端口层缓冲 | 0 |
| `domain-store` 五个写穿点 | `write-audit.recordWrite`（含栈） | 0 |
| 删除类写操作（控制台） | `write-audit` | 0 |
| 仓储命令唯一出口 `RustDataPort` | `[StorageTrace]` | 0 |
| **所有 IPC 的最终出口 `tauriTransport`** | **`[IpcTrace]`** | **0** |
| 三个镜像类 / 端口实例数 | 静态（只发读命令）/ 计数=1 | 不适用 |

**结论（本轮可以下定论的部分）**：那条 `DELETE FROM sessions` **不是渲染进程的 JavaScript 发出的**。
渲染进程里所有能发命令的位置都装过仪器，且仪器有效性经过验证 —— 它们没有一个看到它。

### 由此收敛到的三个候选（按可验证成本排序）

1. **Rust/Tauri 侧**：`src-tauri` 里除 `storage_invoke`/`storage_batch` 之外的代码路径
   （例如某个持有 `StorageState` 的命令、或启动/退出钩子）直接删了会话。
   **验证方式**：在 `codem_db::dispatch` 里对 `sessions.delete` 打印一次调用即定位
   —— 这是**唯一**还没装过仪器的执行点（本轮只装了 JS 侧）。
2. **第二个连接/进程**：另有一个进程或连接打开同一个库文件做清理。
   **验证方式**：删除发生时用文件句柄枚举看谁持有 `codem-db-rust.bin`。
3. **我尚未覆盖的 JS 入口**：可能性已经很低（最终出口已排除），除非存在
   **另一个 webview**（如宠物窗口）加载了同一份 JS 并拥有自己的端口实例 ——
   本轮 CDP 只看到一个 page target，所以这一条与"只有一个端口实例"互相印证，基本可排除。

### 排查方法论（三轮沉淀，值得固化）

1. 插桩前先确认**运行中实际加载的 chunk** 里有这段代码；
2. 仪器装在**最外层唯一汇聚点**（本轮已到 IPC 最终出口，无法再往外）；
3. 每排除一层就**进矩阵** —— "把可能性一条条划掉"本身就是可复用的产出；
4. 对照组同样重要：本轮"闲置 20 秒完好 / 点击即删"这一对，把触发条件锁死在**交互路径**上。

### 第 36 轮（下）：`dispatch` 层也 0 命中 —— 边界收到"应用代码之外"

上一节排除了渲染侧 JS。本轮把仪器推进到**Rust 侧唯一的执行汇聚点**：
`codem_db::dispatch`（`lib.rs`）—— 所有仓储命令的落地处，无论调用来自 Tauri 命令、
CLI，还是任何直接持有 `Engine` 的代码。留痕打到 stderr（带 backtrace）。

**结果：0 命中**，而且这一轮**先验证了 stderr 通道可用**（同一文件里成功收到了
`[crash-marker] previous run did not exit cleanly`），所以这不是"通道没通"的假阴性。

同时复现成功且数据确实被删：`821→0 / 3→1`，SQLite 审计记下被点开的那个会话及其全部子行。
...
| --- | --- | --- |
| 端口三层 `data.execute/write/command` | 端口层缓冲 | 0 |
| `domain-store` 五个写穿点 | `write-audit`（含栈） | 0 |
| 删除类写操作 | `write-audit` 控制台 | 0 |
| 仓储命令唯一出口 `RustDataPort` | `[StorageTrace]` | 0 |
| 所有 IPC 最终出口 `tauriTransport` | `[IpcTrace]` | 0 |
| 三个镜像类 | 静态：只发读命令 | 不适用 |
| 端口实例数 | `__codemStoragePorts` | 1 |
| **Rust 侧执行汇聚点 `codem_db::dispatch`** | **`[RustTrace]`（stderr + backtrace）** | **0** |

**而 SQLite 触发器每次都记下删除。**

### 结论

`DELETE FROM sessions` 与随后的外键级联**确实发生**（触发器是硬证据），
但它**不经过应用代码的任何一层** —— 既不是渲染进程 JS 发的，也没有经过
Rust 侧的命令分派。剩下的解释只有一类：

> **旁路 `StoragePort`/`dispatch` 抽象、直接对同一个库文件执行删除的东西** ——
> 无论是另一个连接、另一个进程，还是某条绕过 `Engine` 的路径。

### 下一步（验证成本最低的两件事，按顺序）

1. **看清谁持有文件**：在"点击触发删除"的那一刻枚举 `codem-db-rust.bin`
   的句柄/连接（`handle.exe` 或 Rust 侧 `PRAGMA database_list` + 连接计数）。
   这一步能直接区分"同一进程的第二连接"与"另一个进程"。
2. **看 WAL 里到底是谁写的**：删除发生前后各做一次 `checkpoint`，
   比对 WAL 变化 + 用 `PRAGMA data_version` 观察是否被**其它连接**改过
   （`data_version` 只在别的连接提交时才变，正是为这种场景设计的）。

### 排查方法论（第四轮沉淀）

本轮新增两条：

5. **先验证"仪器通道"本身可用**再相信"0 命中" —— 本轮用 crash-marker 证明 stderr 通道是通的；
6. **每往外套一层就重测一次**：从 JS 最外层一路推进到 Rust 分派，
   每一层都独立复现、独立取证，避免"以为排除了、其实那层没生效"。

### 第 37 轮：文件级取证 + **运行期守护**，数据在"使用中"也不再丢

#### 文件级取证（把删除定位到毫秒级）

以 150ms 粒度采样库文件与 WAL：

| 时刻 | db 大小 | WAL 大小 |
| --- | --- | --- |
| 点击前 | 115,191,808 | 28,872 |
| 点击后 +1.1s | 115,191,808 | **1,751,032** |
| 点击后 +1.7s | 115,191,808 | 1,755,152 |

**WAL 在 1 秒内涨了约 1.7MB**（主库文件不变 → 全部走 WAL 提交），
紧接着 `messages` 从 821 变 0。与 SQLite 审计时间戳一致。

同一轮还确认了两件此前只是推测的事：

- **点"项目"不会触发**（数据保持 `821/3`），**只有点"会话"才触发** ——
  触发面被进一步窄化到"打开会话"这条路径；
- 应用**读的就是那个文件**：`storage_info` 返回
  `C:\Users\…\com.codem.app\codem-db-rust.bin`，且点之前
  `messages.count(session)` 读到 **277**、`sessions.list` 读到 **3** ——
  所以不是"应用在看另一个库"。

#### 运行期守护（本轮的实际修复）

原来自检只在**启动时**跑一次，而实测证明数据是在**使用过程中**消失的 ——
中间这段时间用户是裸奔的。所以新增 `guardContentBeforeSessionOpen()`：

- 触发点：`loadMessages` 读到 0 条时（即"正要打开一个会话"这个瞬间）；
- 判据与启动自检**完全一致**（同一水位、同一阈值、同样要求旧库确有内容），
  并带"进行中"闩锁避免并发恢复；
- 恢复后**清会话日志缓存并重读**，让用户不用重启就看到内容。

**实测（打包版）**：点开会话 → 数据被清空 → **同一进程内自动恢复**
`messages=821 / sessions=3 / events=2131 / tool_calls=883`，
控制台如实报告"上次 821 条消息，现在 0 条 —— 正在从旧库恢复"。

也就是说：**用户现在即使遇到这条删除，也不需要重启，数据当场就回来了。**

#### 仍待解决

- 删除的**发起者**依然没定位（8 层仪器全部 0 命中，见上）；
- 恢复后**界面气泡仍为 0**（数据库已恢复，但重读没拿到内容 ——
  会话在清空时被删过，恢复后 `currentSession` 指向的对象可能已失效）。
  这是"显示"问题，不影响数据安全，下一轮处理。

#### 排查方法论（第五轮沉淀）

7. **文件级取证**（大小/WAL/mtime 高粒度采样）能把"什么时候改的"钉到毫秒，
   与代码层时间戳交叉验证 —— 比只看最终状态强得多；
8. **区分"启动时"与"使用中"**：只在启动做自检，会漏掉整个使用期 ——
   守护逻辑要放在真正会出问题的那条路径上。

---

## 第 39 轮：找到"压缩等于没做"的完整机制（不是某一处漏了，而是整条链路空转）

把注意力从"追那条删除"转回**目标本身**（删掉 L3 回退分支）之后，端口模式的失败清单
立刻指出了真正值钱的东西：一批压缩用例报的正是**用户现场那个 bug** ——
"压缩说移除 N 条、下一次读又回来 N 条 → 上下文 token 一点没降"。

### 完整机制（三个环节连环空转）

1. **`deleteMessagesByIds`（压缩的软删除入口）第一行就是 `getDatabase()`** ——
   而 rust 模式下旧库**刻意不存在**，这一行直接抛 `Database not initialized`。
   后果：压缩**一条都没隐藏**。（这条路径此前从未被端口化。）
2. **`sessionIdsForMessages()` 只查旧库** —— 即使绕过第 1 点，这里也返回**空 Map**，
   于是"按会话分组"整条链路（写墓碑、剔日志镜像）全部空转。
   这是关键洞见：**不是某处漏了，而是删除链路的会话归属查不到，导致整条链路无效**。
3. **镜像的 hidden 集合在"未加载完"时为空** —— 而压缩发生在使用中，
   那一刻镜像可能正好没加载完，于是合并阶段把已软删除的消息从日志整批加回来。

### 修法

| 环节 | 修法 |
| --- | --- |
| ① 软删除入口 | **端口优先**：`messages.delete` + `soft: true`（Rust 侧真改 hidden）；端口不可用才走旧库（回滚开关） |
| ② 会话归属 | `sessionIdsForMessages` 改为**先问镜像** `byIdLookup`，端口在就不再退回不存在的旧库 |
| ③ 未加载窗口 | 新增 `localHiddenIds`（本进程隐藏过的 id，按会话分组、有界 5 万）：读路径的 hidden 判定叠加它，**不必等镜像重新加载** |

### 顺带修掉一个"测试双比实现更宽松"的问题

假端口的 `applyWrite` 会把会话直接标成 `loaded`（真实端口只写行、不标 loaded），
等于**谎报"这个会话的镜像已完整"**；同时它没有实现 `messages.delete`。
两者叠加会让一批压缩用例**假绿**。已按真实语义修正：
`applyWrite` 只写行；`messages.delete` 支持 `soft` 软删除。
...
| 本轮开始时（端口模式） | 96 | 5103 |
| 本轮结束时（端口模式） | **91** | **5108** |
| 基线（`CODEM_TEST_PORT=0`） | **0** | 5199 |

基线保持全绿 → 本轮改动**没有引入回归**，端口模式的 5 个净改善全部来自真实修复。

### 剩余（下一轮接着做）

压缩相关的 4 个用例仍红，指向**重启后**仍会复活（CB-8）与"硬删除后同 id 再写入"
的后写者胜语义（CB-9）—— 都属于"权威日志与索引的 hidden 语义要对齐"这一族，
修法方向已在上面第 ②③ 条的延长线上。
...
| 条件 | 证据 |
| --- | --- |
| **渲染进程不再持有 WASM 数据库** | 打包版启动 **0 个** sql.js/.wasm 网络请求；`initDatabase()` 从未被调用；L2「引擎本体外泄」= **0**（sql.js 只出现在 `database.ts`） |
| **大文档批处理不再触发内存越界** | 消息路径实测 **1.0×**（不再是 sql.js 的 3.4–3.7×）；知识库入库 4MB/2048 chunk 实测 **+0.0MB** |

`RuntimeError: memory access out of bounds` 的根因（整库常驻渲染堆 + `db.export()` 全量拷贝 +
wasm32 的 2GB 上限）在架构上已被移除 —— 这是本目标的原始诉求，已闭环。

### 用户可见缺陷：本轮又关掉一个（v1.16.54）

**上下文压缩曾等于没做** —— 三个环节连环空转（软删除入口抛错 / 会话归属查不到 /
镜像未就绪时 hidden 为空）。已修，真机实测软删除使可见条数 277 → 276。
这条直接对应「移除 840 条、token 一点没降、硬停请开启新对话」。

### 仍未完成（下一阶段的工作清单）

1. **L3 回退分支还剩 23 个文件**（`wasm-removal-readiness.mjs` 可随时量化）。
   这是"删除旧引擎"的最后一大块 —— 端口模式的失败数就是它的**进度条**。
2. **端口模式 91 个失败**（本轮 96 → 91）。它们集中的主题很明确，且都指向真实语义：
   - 压缩/软删除的**跨重启耐久性**（CB-8/CB-9/CB-11）与"后写者胜"；
   - 一批把**旧库当唯一真源**的测试（直接对旧库写 hidden、直接读旧库行），
     在纯端口基座下需要改成"通过端口表达意图"。
3. **那条删除的发起者**仍未定位（8 层仪器全部 0 命中）。**但危害已消除**：
   批量删除闸门拦第一道、启动自检 + 运行期守护兜第二道，界面也能显示恢复后的内容。
   下一步建议按文档里的"候选三条"逐一验证（Rust dispatch 已排除 → 下一个是
   "第二个连接/进程"，可用 `PRAGMA data_version` 或文件句柄枚举区分）。
4. **L1 依赖清理（2 处）**：`database.ts` 里的 sql.js 引用与 wasm 资源。
   只有在 L3 清零之后才能动 —— 现在删了就没有回退路径了。

### 本轮沉淀的方法论（六条，已可复用）

1. 插桩前先确认**运行中实际加载的 chunk** 里有这段代码（本仓是懒加载分片）；
2. 仪器装在**唯一汇聚点**，而不是逐个调用点；
3. **每排除一条路径就进矩阵** —— "把可能性划掉"本身就是可交付的产出；
4. **先验证仪器通道可用**，再相信"0 命中"；
5. **文件级取证**（大小/WAL/mtime 高粒度采样）能把"何时改的"钉到毫秒；
6. **测试双不得比实现更宽松** —— 假端口早先谎报 `loaded`、漏实现 `messages.delete`，
   让一批压缩用例假绿；这类"宽松的替身"比没有替身更危险。
