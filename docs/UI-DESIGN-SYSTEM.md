...
7. **UI 字体**：我们以 `AlimamaFangYuanTi` 打头、静态字重（第 33 波补的 560/620 细档会被浏览器就近取整），
   且 CSS 里 `font-family` 声明 186 处（参考实现只有 15 处，全部走 `var(--font-mono, …)` 这类令牌栈）——
   建议收敛成 `--font-ui` / `--font-mono` / `--font-display` 三个令牌栈，并换可变字体，细档字重才会真正生效。
