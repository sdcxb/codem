...
> 注意：当前 UI 字体是静态字重（`AlimamaFangYuanTi` 等），560/620 会被浏览器就近取整；
> 换用可变字体后细档才会真正生效（这是与参考实现的最后一段差距，见 §7 B 组第 7 条）。

### 2.2 圆角（`--radius-*`）

| 令牌 | 值 | 用途 |
| --- | --- | --- |
| `--radius-xs` | 6px | 徽标、极密集的小块（第 33 波 6 → 8px 上移后留给最紧的场景） |
| `--radius-sm` | 6px | 小控件、标签、滚动条滑块、密集行（第 33 波 4 → 6px 软化一档） |
...
参考实现的圆角分布是「8px 主力 ×173 + 999 胶囊 ×96」——**短文本小块用胶囊**是它看起来"像产品"
的细节之一（方角小块更像数据表）。
...
32. **第 33 波（本轮）**：**"为什么像项目不像产品"的带数据诊断**（§2.8）与其对应的底层修正 ——
...
### 尚未追平参考实现的部分（第 33 波诊断后剩下的，按"要不要动"分类）

**A. 技术性差距，还能继续追（不需要产品决策）**
1. `display: grid` 50 → 参考 583：目前只把设置/表单行改成了 grid（第 32 波）。下一步可轮到列表行、
   卡片宫格、工具条的「图标 + 文本 + 尾部操作」三段式（参考实现全部用 grid 模板而不是 flex + margin）。
2. `:has()` 7 → 参考 41：已用两个真实场景（卡片聚焦、字段非法）。可继续用于「卡片内有选中项」「行内有禁用控件」。
3. `prefers-reduced-motion` 8 → 参考 27：已覆盖主要浮层族，剩余是图表/动画类组件。
4. `focus-visible` 带环 21 → 参考 43：全局兜底规则已在，逐组件还差一圈（表格行、菜单项、树节点）。

**B. 需要产品/品牌决策，本轮刻意没动**
5. **窗口外壳**：参考实现有 mac 风格窗口（`mac-window` 34 处 + `workbench-window` 7 处：红黄绿灯、
   一体化工具条），我们只有 `titlebar` 37 处 —— 这是"产品感"最强的单一信号，但改的是应用外框，
   属于品牌决策。
6. **默认主题明度**：我们默认深色冷色（平均明度 0.50），参考实现默认浅色暖灰（0.60）。
   浅色主题的能力我们已具备（第 22–23 波修完令牌后浅色才真正正确），缺的只是"默认档位"。
7. **UI 字体**：我们以 `AlimamaFangYuanTi` 打头且用静态字重（细档令牌在静态字体下会被浏览器就近取整），
   参考实现用 `"Space Grotesk", Inter` 这类可变字体 —— 换可变字体后第 33 波补的 560/620 档才会真正生效。
