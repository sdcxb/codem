...
### 还没量、但该量的队列（第 38 波新开的一维）

- **死类名**：`.preview-shot/dead-classes.mjs` 量出 **3396 个 CSS 顶层类名里 258 个（7.6%）**
  在 TSX/TS 里从未出现（`composer-*` 整族、`nb-guided-questions` 族、`ppt-*` 族、`sidebar-*` 若干）。
  这是 `css-class-undefined` 的**镜像维度**（那边查"用了没定义"，这边查"定义了没人用"）。
  **本轮不批量删**：部分可能是动态拼接的前缀（`${}` 拼类名），误删风险大于收益；
  正确做法是给规则加"动态前缀排除"后逐族清理，并按 warn 级入门禁（数字先降下来再锁）。
  **顺手已修**：第 32 波文档点名的 `.sp-field-row` 与两处 `.tool-pill` 选择器都是死类名，
  已改到真实类名 `.tool-call-pill` 上。
