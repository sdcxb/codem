...
### 门禁清单（第 54 波后的全貌：24 条审计规则 + 4 组契约测试）

| 层 | 位置 | 看什么 | 拦得住什么 |
| --- | --- | --- | --- |
| 审计规则 24 条 | `tools/ui-audit/scan-ui.mjs` + `baseline.json` | 令牌化、类名、层级、焦点、减动效、跨文件冲突、编码损坏 | 「用了错的写法」（数量只降不升） |
...
### 明确保留、不再动的（避免下一轮重复劳动）
0. **页签/按钮行容器的「单行」原则有边界**：容器高度**固定**时（左右侧栏的标签条）必须单行 + 文字省略号（不换行、不滚动）；
