...
| **第 69 波** | 2026-09-13 | **0** ✅ | **0** ✅ | **上下文超限后的「白重试 + 假续写」+ 思考模型被输出上限掐断（用户日志给出完整真相）**。<br>① **现场**：`finish_reason=length — text 0 chars, tool calls 0`（**思考吃光输出预算**）→ 自动续写 → 之后每次请求 400 `maximum context length is 1048576 tokens. However, you requested 1048735 tokens`，且请求还在变大（1048735 → 1048992 → 1049249）、每轮白重试 3 次。<br>② **根因一：错误分类只认自家措辞**。反应式压缩只匹配 `prompt_too_long` / `context_length_exceeded`，而 DeepSeek 说的是 `maximum context length is ...` → **能自动救场的压缩从未触发**。新增 `provider-errors.ts` 做**语义匹配**（含 `reduce the length of the messages` 等各家措辞），并能解析「上限/实际请求」数字用于给用户一句人话。<br>③ **根因二：确定性错误被白重试**。4xx（非 429）重试毫无意义；且 provider **没把 HTTP 状态挂到错误对象**上，`classifyError` 分不清 400/500。现在挂 `err.status`、错误体给足 2000 字符（超限数字不再被 200 字符截掉）、不可重试的错误立即失败。<br>④ **根因三：我上一波引入的 bug**。`lastFinishReason` 跨迭代不重置 → 失败轮（没有任何 finish_reason）沿用上一轮的 `length` 触发**假续写**，把超限上下文继续撑大。现在每轮重置，且记录本轮**正文长度**：`正文 0 字符` 判定为「思考吃光预算」，续写提示改为「少想、直接产出」而不是「从断点继续」（没有断点可续）。<br>⑤ **按模型族给输出上限**（补强第 67 波）：事故模型 `deepseek-flash` 在目录里查不到 → 以前落到 8192（对带思考的模型明显偏小，直接造成「正文 0 字符」）→ 现在按族推断（DeepSeek/推理系 65536 天花板、Claude 4 32000、Gemini 2.5+ 65536），未知型号仍保守兜底。<br>⑥ **溢出后的收尾是可执行的**：压缩 3 次仍放不下 → `context_overflow` 停下并说明「本次请求约 N tokens / 上限 M / 超出 K」+ 三个出路（开新对话 / 收敛内容 / 换大上下文模型）。<br>**教训**：**错误分类不能只匹配「自家见过的措辞」** —— 只认两个字符串，等于让一条能自动救场的路径形同虚设；而日志里那句 `maximum context length is ...` 一直在明明白白地说清原因。<br>校验：`tsc` 0 错误 / 219 文件 4729 用例通过 / 审计 25 条规则 0/0 / css-contract（2743 类）无变化 / 打包成功。 |
| **第 68 波** | 2026-09-13 | **0** ✅ | **0** ✅ | **修复「回复被输出上限截断 ⇒ 被当成完成」（用户报：说「继续之前没完成的任务」，一轮就结束了）**。<br>① **现象**：控制台只有 `Single-response dedup: 0 tool calls` 然后直接 `[extractMemories]` —— 没报错、没重试，看起来「任务自己断了」。<br>② **根因**：`finish_reason === length`（达到单次输出上限）此前**只用于内容型工具的提示**，从不参与「要不要停」的判断 —— 于是被截断的纯文本回复被当成「写完了」；而 `finish_reason` 只写在默认静默的 `debugLog` 里，控制台连线索都没有。<br>③ **修复**：截断 ⇒ **自动续写**（注入「从断点继续」提示 + `continue`），界面提示「⏩ 正在自动续写」；**预算 3 次**，用完则明确停下并给出下一步（分块写入 / 调大 maxTokens），reason `output_truncated`。<br>④ **结束原因进入循环状态**：provider 的 `end` 事件不向上游 yield，所以写进 `LoopState.lastFinishReason`；主循环这才看得见它。<br>⑤ **诊断可见性**：`finish_reason` 非 `stop`/`tool_use` 时 `console.warn`（含「length（达到单次输出上限，回复被截断）」）—— **下次同类问题一眼可判**。<br>⑥ **测试是行为的，不是纸面的**：用脚本化 provider 真跑 `AgenticLoop.run`，断言「截断后必须再请求一次」「连续截断到预算用完以 output_truncated 停」「正常 stop 不受影响」。<br>**教训（第三次同族）**：**「一个信号被采集了但没被使用」与「信号根本不存在」一样危险** —— `finish_reason` 一直被读进局部变量，却没有任何决策用它。<br>校验：`tsc` 0 错误 / 218 文件 4722 用例通过 / 审计 25 条规则 0/0 / css-contract（2743 类）无变化 / 打包成功。 |
| **第 67 波** | 2026-09-13 | **0** ✅ | **0** ✅ | **输出上限按模型动态解析 + 同类问题（静默丢数据）清查（用户追问：maxTokens 该按模型动态取或临时调整，而不是让用户手调）**。<br>① **用户是对的，而且证据就在代码里**：模型目录本来就有 `maxOutputTokens`（`deepseek-v4-flash` 384000 / `gpt-4o` 16384 / `moonshot-v1-8k` 4096）。写死常量**两头都错**：比模型小 → 大文件的工具参数被截断（第 66 波用户报的错）；比模型大 → 请求被 API 拒绝、任务当场失败。<br>② 新增 `model-output-limit.ts`：**显式配置 → 被拒后学到的值 → 模型目录值（夹在 65536 天花板）→ 兜底 8192**；目录查不到（自定义模型）才用兜底。<br>③ **被拒自动折半降档 + 按模型记住 + 自动重试一次**（「临时调整」的自动化版本）；判定保守：仅 400/422 + 提到 max_tokens + 像值不合法/超限，避免把内容审核类 400 当上限问题反复重试。<br>④ 顺带修掉另一处同类硬编码：ultra 模式 `Math.max(config \|\| 4096, 16384)` 对上限 4096 的模型会打挂请求。<br>⑤ **按同一根因清查「静默丢数据」**：`provider.ts` 的 SSE 坏行 catch 以前只 `warn` 就丢弃整行 —— 丢的若是 `tool_calls` 参数增量，累积 JSON 即残缺（**与截断同一现象**）→ 现在计数、并在 `tool_use_end` 标注「参数可能不完整」，由循环拒绝执行并引导重试；`finish_reason=length` 且跑了内容型工具 → 提示核对完整性 + 事件 `output_truncated`；`write` 把已有非空文件写成空 → 结果里警告并带原文件大小。<br>⑥ **核查后确认没问题**（都已有明确提示）：read 截断（showing lines X-Y of Z）、落盘失败截断、上下文预算截断、终端 `[output truncated]`、附件预览、记忆压缩占位符。<br>**教训**：**「截断」从来不是单一 bug，而是一族** —— 每一处「数据被悄悄截短/丢掉」的地方都要回答同一个问题：*模型是否知道它拿到的不是全部？* 这次清查找到 3 处没答上来的。<br>校验：`tsc` 0 错误 / 216 文件 4714 用例通过 / 审计 25 条规则 0/0 / css-contract（2743 类）无变化 / 打包成功。 |
| **第 66 波** | 2026-09-13 | **0** ✅ | **0** ✅ | **修复「工具参数被 JSON 截断」+ 一个数据安全级隐患（用户报错：生成科研绘图脚本时控制台刷 `Unterminated string in JSON at position 6648`）**。<br>① **根因一**：单次输出上限被写死 **4096**（`index.ts` 默认值 + `processor.ts` 的 `?? 4096`）—— 6–10KB 的脚本连同 JSON 转义正好在这个量级，流在字符串中间被 cap 掉，参数 JSON 天生不完整。修：新增 `DEFAULT_MAX_OUTPUT_TOKENS = 8192` 并统一使用（仍可经 `codem-settings.maxTokens`/智能体/槽位覆盖），processor 未配置时不发 `max_tokens`。<br>② **根因二（更严重）**：解析失败时是「静默降级 + 正则兜底」，而**兜底比不兜底更危险** —— provider 只打日志仍返回 `input: {}`，循环再用正则从残缺 JSON 抽 `path`/`content`，而截断时**结尾引号还没生成** → `content` 取空串 → `write` 拿着**空内容**执行 → 对已存在的文件等于**清空**（auto/full 模式下覆盖保护不拦）。<br>③ **修法：参数不可用一律拒绝执行**（新 `src/core/llm/tool-args-guard.ts`）。删掉正则兜底，改为把「原因 + 原始长度 + 结束原因」带出来，并给一句**可操作**的指引：这次没有执行 / 若结束原因是 `length` 则明确写「已确认被输出上限截断」/ 请分块写入（先 write ≤~200 行，再用 `append: true` 追加）/ 不要原样重发。<br>④ `write` 新增 **`append: true`**（大文件分块落点）+ `content` 非字符串直接报错（绝不用空值覆盖文件）；工具指导语也写清「大文件要分块」。<br>⑤ provider 不再静默降级（`argsParseError` + `rawLength` 随事件带出，两条结束路径都带）；拒绝执行落结构化事件（`loop_stopped` + `args_truncated`），可统计「到底多常见」。<br>⑥ **同波审计自查**：provider 报错但**没有 rawArgs 可重试**时仍会空参数执行 → 补分支同样拒绝；提示补 `length` 的「已确认」；另一处残留的 4096 一并收口。<br>**教训（写进流程）**：**容错兜底必须先问「兜出来的东西会不会被当成真的用」** —— 这里的兜底把「半截/空内容」变成了「看起来合法的参数」，比直接报错危险得多；**上限类常量必须单点定义**（4096 散在两处，改一处漏一处）。<br>校验：`tsc` 0 错误 / 215 文件 4704 用例通过 / 审计 25 条规则 0/0 / css-contract（2743 类）无变化 / 打包成功。 |
| **第 65 波** | 2026-09-13 | **0** ✅ | **0** ✅ | **把「卡住」治理补齐成五层（预防 / 收敛 / 止损 / 恢复 / 可见）—— 用户追问「虽然没解决，但这是最佳策略了吗？」之后的作答与落地**。<br>① **为什么继续磨启发式不是出路**：任何**从行为推断意图**的检测器都逃不过两个失败模式 —— 脆弱（误杀合法长任务，如 15 分钟墙钟）与盲（漏放「每次输出都略有不同」的空转）。原理上，从外部只看行为**无法普遍区分「有用的探索」与「无用的空转」**，那需要目标模型。所以方向是**把判断依据从行为搬到契约与状态**。<br>② **L1 计划停滞检测**（新 `src/core/llm/stall-guard.ts`）：判据与内容无关 —— 「没修订计划（`update_plan`）+ 没产出交付物」；连续 12 个迭代**先问**，到 24 才停。**审计修正**：不要用 `macroStep`（UI 启发式步进，每个迭代首个非侦察工具就 +1）当推进指纹 —— 那会让计数不断清零、检测失效；只认 `planRevision`（`update_plan` 成功时 +1）。<br>③ **L3 先问再停**：停滞提醒作为系统消息注入（要求说清卡点 + 改计划或报告缺什么），不打断不杀；**审计修正**：排在「重复调用守卫停止」之后，否则会留下没有下文的孤儿提醒消息。<br>④ **L2 长工具不是沉默**（审计发现）：10 分钟的构建期间本来就没事件，原空闲看门狗会误杀、父会话也会误判「安静=卡住」。按 DSH 的分法拆开：**空闲**（没事件且没工具在跑）与 **`tool_hung`**（单工具在飞 >20 分钟）；工具在飞期间 30 秒心跳，续命 + **向父会话上报进度**。**资源预算**从「只算模型文本」改为把**工具入参/结果**也计入，默认 200k。<br>⑤ **L0 判据必须可判定**：交接里「完成判据：全部完成」这类空话会被拒（要求：会存在的文件 / 反引号命令 / 可比的量）。<br>⑥ **L4 停止原因结构化**（`loop-stop-log.ts`）：`no_gain`/`idle`/`tool_hung`/`plan_stale`(`_ask`)/`budget` 统一落 EventLog；**审计发现**：因停滞停止的任务不能当「已完成」上报父会话，改为按失败交回 + 附已产出内容。<br>⑦ **仍要承认的边界**：即使五层齐备，也**没有本地机制能保证既不误杀也不漏放**；最终兜底永远是**有界成本 + 便宜的人工/校验器介入**（DSH 同立场：不设迭代上限，靠 AbortSignal + 可配资源上限 + 任务化）。<br>校验：`tsc` 0 错误 / 214 文件 4696 用例通过 / 审计 25 条规则 0/0 / css-contract（2743 类）无变化 / 打包成功。 |
...
## 7. 交接快照（列表与数字**随波次更新** —— 标题不再挂某个波次号：旧标题挂过一个**比本节内容更小**的波次号，属自相矛盾的陈旧声明；各条目标注的波次才是准的）
...
> **约定**：本清单是**历史记录**，逐波追加、描述当时做了什么，因此**一律不标「本轮」**（早期版本靠每波脚本把上一波的标记降级，漏一次就会留下多个「本轮」，反而让人看不出当前波次）。当前进行中的波次以下方 §7 交接快照与 `CHANGELOG.md` 为准。
...
28. **第 28 波**：间距令牌化 —— 2906 处数值间距里 2354 处令牌化 + 60 处 2px 网格归并
...
33. **第 34 波**：**局部细节三件事**（门禁规则 15 → 17 条）——
...
40. **第 41 波**：**应用外壳：应用级菜单栏 + 窗口 chrome**（B 组第 5 项）——
...
41. **第 42 波**：**标签墙/选项墙改 auto-fill 网格**（把 grid 推到 **305** 处 / `grid-template-columns` **301** 处，
...
44. **第 46 波**：**把「搜索当前会话」「临时会话」从会话头部移到编辑器底部工具行**
...
| 计划是否在推进 | `stall-guard.test.ts`（STALL-1~9） | 判据与内容无关：计划修订号 + 交付物；先问后停；读-写节奏不误杀；UI 步进不算推进 | 「每次输出都不一样、任务一步没走」的空转（第 65 波） |
