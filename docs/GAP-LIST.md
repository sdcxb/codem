...
| 项 | 值 | 怎么核对的 |
| --- | --- | --- |
| 装机版（用户手里那份） | **1.16.131** | 真机启动后读 `version`。**127→128、128→129、129→130、130→131 四次都是更新器自己升的**（按钮 `检查更新 → 检查中… → 发现新版本 x，下载中…` 然后应用重启、版本变化）；1.16.126 那次因网络掐断用构建产物直装 |
| GitHub Latest | v1.16.131 | `node tools/release/verify-update-manifest.mjs --remote` **7/7** |
| 仓库 `latest.json` | 1.16.131 | 同上 |
| 本轮版本 | 1.16.131 | — |
