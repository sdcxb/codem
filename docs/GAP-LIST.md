...
| O-18 | **提示条文案会出现重复标点**（真机实测原文末尾「…有同一处状态与入口。。」） | 现场：装机版 1.16.136 的钻取里，提示条文本末尾是「。。」。原因：`composePersistAlertText` 的模板固定拼一个「。」，而调用方给的 reason / consequence 自己**已经以句号结尾**（新加的凭据文案就是这种写法）。影响：只影响观感与「印出来的东西是否干净」，不影响正确性；但同一模板对**所有**带句号的 reason 都会这样（不是孤例） | 在 `composePersistAlertText` 里折叠结尾重复标点（相邻的「。」「.」「！」「？」，以及「。（」这种组合），并加判据：**贴出来的那句话不许出现「。。」**；配一处变异（把折叠逻辑去掉必须变红）。随下一批一起出包 |
