...
| 项 | 值 | 怎么核对的 |
| --- | --- | --- |
| 装机版（用户手里那份） | **1.16.127** | 真机启动后读 `version`。1.16.125 是**更新器自己升上去的**；1.16.126 与 1.16.127 因本机网络掐断安装包下载，改用**构建产物直装**（`Codem_<ver>_x64-setup.exe /S`） |
| GitHub Latest | v1.16.127 | `node tools/release/verify-update-manifest.mjs --remote` **7/7** |
| 仓库 `latest.json` | 1.16.127 | 同上 |
| 本轮版本 | 1.16.127 | — |
...
| O-13 | **更新重试的"真机端到端"没跑到**（本机当前无可升级版本） | 第 82 轮的修复（有界重试 + 可读错误）已配门禁 7 条 + 6 处突变，且**确认进了装机包**（dist 含 `下载中断，正在重试`，安装包构建时间晚于 dist）。但真机上"下载被掐断 → 自动重试 → 成功"这一串**没跑到**：装的是最新的 1.16.127，清单里没有更高版本 ⇒ 走的是"未发现更新"分支。**如实标注：不给"真机验证过重试"这句话** | 下一次有可升级版本时，在装机版点「检查更新」并观察：按钮出现「下载中断，正在重试（第 n/3 次…）」+ 控制台出现 `[updater] 下载第 n/3 次失败（可重试）`；若网络一直掐断，则应看到**可读的失败文案**（而不是 `error decoding response body`）。工具：`.preview-shot/_observe-update-flow.mjs` |
