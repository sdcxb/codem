...
> **待发布（v1.15.2：修复「看板 → 时间线」为空）**：时间线事件来源太窄（对话消息不算事件）→ 只问答不调工具的会话恒为空；现把最近 30 条对话消息也纳入时间线（用户发言/助手回复/失败消息），空态改为自解释（六项采样计数 + 采集失败源 + 事件来源说明，类别为空时文案区分）；预览夹具事件数 4→60 让长列表纳入审计。199 文件 / 4554 用例通过（+15 跳过）。
>
> **v1.15.1 已发布（修复任务管理「概览」没有滚动条）**：内容区 `overflow` 原先绑在 `wide`（面板宽度）上，概览加宽后被 `hidden` 裁掉；现拆成 `wide`（board\|subagents\|overview）与 `fillsViewport`（board\|subagents），内容区按后者决定滚动，并加 `data-task-center-content` 标记 + DEDUP-7/8 回归用例。198 文件 / 4549 用例通过（+15 跳过）。
>
> **v1.15.0 已发布（用量迁进概览 + 场景归「子智能体」+ 结构树去重）**：①**修复样式丢失（P0）**：视图外壳漏导入 `styles/library-ops.css` → 看板/子智能体整片无样式、不自适应；现由 `LibraryOpsViewShell` 统一导入 + 样式入口守卫测试，版面审计新增 `stylesApplied` 断言并按宿主位置分别跑。②**「用量」迁移进「概览」**（新 slot `task-center.overview`），**看板里的用量视图删除**（原先只是指回去的引用卡片）。③场景 + 设置归「子智能体」页签（`task-center.subagents`：场景|设置），看板收敛为 看板|工具|错误|时间线；跨页签跳转统一 `requestView()`。④场景角色 = 队长 + 团队成员 + 子智能体（有/无团队）+ 仅在途委派会话；闲置会话与模板角色不再入馆 ⇒ 无团队/无子智能体/无在途委派时只有队长待命。⑤**功能结构树 `docs/TASK-CENTER-MAP.md`** + 13 项去重（死按钮、父会话下钻断链、token/成本重复三遍、概览手写事件列表、Issue 状态元数据 5 份合并、采样双轮询合并、SquadsTab 项目边界、底栏读真实配置、删死代码 DelegationPanel 等）；登记 5 项待做/待确认。验证：198 文件 / 4547 用例通过（+15 跳过）+ tsc 零错误 + 版面审计 3 位置 × 7 宽度全 0 + DOM 审计 0 issue。
>
> **v1.14.1 已发布（修复看板子视图被「实时事件」挤占/遮挡）**：①「看板」子视图默认不渲染右侧实时事件流（状态条新增开关可临时打开），其它监控视图不变；②新增 `.lo-board-host` 包裹宿主 `IssueBoard`（`flex:1 1 auto` + `min-height:0`），看板铺满内容区、横向滚动条可达；宿主列最小宽度改走 `--issue-col-min` 变量（默认 180px、插件内 128px）→ 宽面板 7 列一屏排完；③**补齐审计漏检根因**：`tools/preview` 改为渲染真实 `LibraryOpsBoardView`（导航栏 + 事件流）+ 宿主 `IssueBoard` 真组件（新增 `issue-stub.ts` / `store-stub.ts`），预览加载宿主全局样式 `src/styles.css` 对齐 `box-sizing`，子视图审计 6 → 7 个（含看板），新增 `tools/preview/probe-layout.mjs` 探针 + `?audit=1&view=<视图>` 定点检查；新增 LO-UI-13 回归用例。全量 196 文件 / 4534 用例通过（+15 跳过）+ 版面审计 7 宽度 × 7 视图全 0 + DOM 审计 0 issue。
>
> **v1.14.0 已发布（图书馆并入看板 + 场景图可上传/自动对位 + 界面自适应与图标统一 + 任务管理全量审计）**：①**并入看板**：图书馆场景本质就是「谁在做什么、在哪做」的可视化看板 → 与宿主「看板」页签合并：看板（宿主 Issues）为默认视图 + 插件追加 场景/用量（合并原「成本」）/工具/错误/时间线/设置；去掉重复的「总览/团队/会话」入口；宿主新增 `task-center.board` slot（`BoardTab` = `SlotBridge` + `IssueBoard` 回退），插件禁用即回退自带看板；删除独立面板/悬浮入口/store 的 open 状态。②**场景图可上传**：设置 →「场景图片」拖拽/选择上传（IndexedDB 存 Blob），`build-library-ops-scene-preset.mjs` 可做内置预设。③**自动对位**：`core/scene-align.ts`（地面掩码 vs 房间掩码 IoU 粗到细搜索）自动求缩放/位移 + 置信度，上传后自动应用，仍可手动微调（房间框/走道节点）。④**界面自适应**：容器查询按面板实际宽度分档 + 子视图自然高度 + 内容区滚动 + 固定值换 `minmax`/`clamp`；新增 `tools/preview/audit-layout.mjs`（7 种窗口宽度逐视图检查裁切/重叠，全部 0）。⑤**图标/样式统一**：emoji → lucide-react（`icons.tsx`，49 语义名）+ 卡片/标签对齐宿主 `.card`/`.badge`。⑥**修复**：dev 白屏 `process is not defined`（dev-only polyfill）、Vite 监听 `*.tmpdir` EBUSY、子视图固定高度挤压重叠。⑦**任务管理全量审计（15 项）**：P0「子智能体」列表恒空（render 里 `require()`）；P1 事件只认 `teams` 页签 / 子智能体点不开父会话 / 看板拖拽失效与同列误写库 / 看板缺 `blocked`+`cancelled` 列 / 看板无法滚动 / 委派统计与列表口径不一致 / 面板打开期间不跟随页签；P2 自动化「停止所有」无法恢复 / Issues 筛选缺 backlog+cancelled / 收件箱徽标随筛选变化且点击不跳转 / **无项目时跨项目串数据**（含孤儿 Issue）/ `IssueCard` hover 覆盖状态条 / 详情面板切换不刷新；新增 `task-center-audit-fixes.test.tsx` 12 例。⑧**二次去重**：概览只留最近 5 条 + 「查看完整时间线」入口、场景标注为子智能体/团队的可视化表达、自动化唯一入口、删除插件元数据里的旧「图书馆页签」按钮 id。⑨**第二轮独立复审 14 项**（P1 工具路径项目边界 + 非法 status 崩溃；P2 详情面板假评论 / 自动化暂停状态 / cron 步长为零 / 概览统计口径 / 委派历史恢复 / 切项目重查 / single 槽位优先级；P3 时间线深链 / 新建被筛选藏起来 / N+1 查询 / 收件箱裁剪 / 死代码 TeamsPanel）全部修复，新增 `task-center-audit-fixes-2.test.tsx` 14 例。全量 196 文件 / 4533 用例通过（+15 跳过）+ tsc 零错误 + DOM 审计 0 issue + 版面审计 7/7。
> **（已并入 v1.14.0）图书馆界面自适应 + 图标/样式统一**：融合进任务管理后第一版在窄窗口下元素挤压。①**版面改容器查询**：`.lo-task` 设为容器（`container-type: inline-size`），按面板实际宽度分档（≥1080 三栏 / 980–1080 收事件流 / <980 单列 / ≤820 子导航图标条）+ 按容器高度（≤620px）压场景高度；固定死值全部换成 `minmax()`/`clamp()`（236px 列 → `minmax(180px,236px)`、`min-height:460px` → `0`、事件流 `280px` → `clamp(200px,24cqw,280px)`、侧栏 `300px` → `minmax(200px,300px)`）；≤980px 场景+花名册改上下两段（下段 ≤45% 内部滚动）。②**图标统一 lucide**：新增 `components/icons.tsx`（`LoIcon` + `LO_ICONS` 49 个语义名），数据层 `icon` 改存语义名（`types.ts` 新增 `LoIconName`），替换全部 emoji（11 工作状态 / 5 角色来源 / 10 岗位 / 卡片 / 场景 HUD / 子导航 / 上传提示 / 趋势箭头）。③**样式对齐宿主**：`.lo-card` 改 10px 圆角 + `--bg-secondary` + hover 边框、`.lo-pill` 改 4px 圆角、去掉 uppercase、`.lo-stat` 统一；删除独立面板死样式（`.lo-launcher`/`.lo-overlay`/`.lo-shell*`）。④**新增门禁**：`tools/preview/audit-layout.mjs`（headless 7 种窗口宽度读页面自检 `#layout-audit`，要求 0 溢出）+ `library-ops-icons.test.tsx` LO-ICON-1~5。全量 193 文件 / 4515 用例通过 + tsc 零错误 + DOM 审计 0 issue + 版面审计 7/7 通过。
> **（已并入 v1.14.0）图书馆融合进任务管理，不再单独显示面板**：图书馆插件与「任务管理」在入口层大量重叠（两处都能看到同一批团队/会话/任务），本版把图书馆**降级为任务管理里的一个页签**。①**重叠分析**：逐页签对照——「总览」≈任务管理「概览」（同义入口，保留为「用量」子视图，因为 token/成本/健康/活动分布是任务管理没有的）、「团队」完全重复（**删除**，按钮跳任务管理「团队」）、会话/工具/成本/错误/时间线/设置/场景为独有（全部保留）。②**宿主扩展点**：`declare-slots.ts` 声明 `task-center.library` slot + `SlotBridge.tsx` 新增 `useSlotHasEntries()` + `TaskCenter.tsx` 有贡献者时才显示「图书馆」页签（`<SlotBridge>` 渲染内容，页签加宽到 1180px）；插件禁用 → 页签不出现、其余 8 页签不受影响；贡献者被移除 → 自动回落「概览」。③**插件侧**：新增 `LibraryOpsTaskView`（状态条 + 8 个子视图：场景/用量/会话/工具/成本/错误/时间线/设置 + 实时事件流），provider 注册到该 slot（React.lazy 独立 chunk）；**删除** `LibraryOpsPanel.tsx`（全屏外壳）与 `LibraryOpsLauncher.tsx`（悬浮圆钮 + 徽标）以及 store 的 `open/openPanel/closePanel/togglePanel`；`Ctrl/Cmd+Shift+L` 与 `codem:open-library-ops` 改为派发宿主已有的 `codem:open-task-center`；采样随页签挂载/卸载（切走即停）。④**验证**：新增 `library-ops-task-center.test.tsx` 7 例（无/有贡献者、直达页签、真实 provider 渲染、贡献者移除回落、快捷键与事件别名、slot 声明）+ 改写 `library-ops-ui.test.tsx` 12 例；全量 192 文件 / 4510 用例通过 + tsc 零错误。
> **（已并入 v1.14.0）场景图可上传替换 + 房间框可视化对位**：场景「画面」与「布局」解耦——角色站位/岗位标签/点击热区都来自固定数据，图片只是铺满 1920×1072 画布的一个图层，于是换图不再需要改代码。①**设置 →「场景图片」**：内置画廊（`claw` ClawLibrary 像素画双层 / `ai-library-01` 本项目自有 AI 整图，可商用）一键切换；点「选择图片」或**直接把图拖到场景上**即可上传（PNG/JPG/WebP/AVIF/GIF/BMP，≤32MB、≥640×360，比例偏离 16:9 超 8% 提醒会被拉伸）；图片以 Blob 存 **IndexedDB**（`codem-library-ops`/`scene-images`，避开 localStorage 5MB 配额），不支持时降级为「本次会话有效」并明确提示。②**对位编辑器**：场景右下角 ✥ 进入对位模式（Esc 退出），拖房间框 = 移动房间（工作锚点+标签一起走）、拖右下角小方块 = 改尺寸、拖圆点 = 移动路网节点（BFS 邻接表按版本号自动重建）；覆盖层按场景图片 id 分别持久化（`codem-library-ops-layout`），拖动过程只改本地预览、松手才提交；引擎通过模块级注册表读取 → 引擎签名零改动。③**新增工具**：`scripts/build-library-ops-scene-preset.mjs`（任意图 → 2752×1536 WebP 预设 + 缩略图 + 可直接粘贴的 `SCENE_PRESETS` 条目）+ `public/library-ops/scenes/`（自有素材 + SOURCE.md）。④**修复**：`vite.config.ts` 忽略 `*.tmpdir`/`*.tmp`，修复编辑器原子写入时 Vite 文件监听 EBUSY 崩溃（tauri dev 直接退出）。⑤**文档**：`docs/art-prompts/00`/`01` 增加「自己上传 + 对位」路径，`docs/LIBRARY-OPS-PLUGIN.md` §7.4、`docs/ASSET-LICENSES.md`、`public/library-ops/README.md` 同步。新增 5 个测试文件 / 40 用例（纯逻辑 10 / IndexedDB 6 / 设置卡 8 / 场景渲染与拖拽 9 / 对位覆盖 10）。全量 191 文件 / 4503 用例通过 + tsc 零错误。
> v1.13.0 已发布（2026-09-10，图书馆插件集成手绘像素美术 + 监控面板对标 lobster-pet）：**把美观提到第一位——直接集成参考项目的手绘像素美术资源，场景直接用参考项目的场景。**①**像素图书馆场景（默认风格）**：使用 `shengyu-meng/ClawLibrary` 的 `scene-floor`/`scene-objects`（2752×1536 手绘像素画）+ `walkGraph`（20 节点/19 边）+ 12 个资源分区坐标（bounds/labelAnchor/workZone）；角色使用其 **Capy-Claw / Cat-Claw** 精灵表（128×128 帧 @6fps，各 12 套动作：work/read/idea/repair/error/sleep/coffee/rest/walk/stand_front/stand_back/lie_flat/lie_side/front/game），按角色 id 稳定分配变体（70% 岗位默认 + 30% 另一个）；**11 种工作状态映射到上游动作**（待命→stand_front、行走→walk、思考→idea、阅读→read、撰写/执行→work、检索→read、等待授权→rest、完成→coffee、出错→error、休眠→sleep）；**寻路直接用上游 walkGraph 做 BFS 图最短路** + 末端直连工作锚点，房间内多角色按黄金角环形排布不叠人；相机滚轮缩放（0.3×–3.2× 指针锚点）/拖拽平移/双击复位/选中角色平滑居中；资源缺失自动降级到等距矢量并提示。②**资源管道与许可合规**：新增 `scripts/sync-library-ops-assets.mjs`（上游 PNG→WebP **30.1MB→5.1MB** 视觉无损 + 为每个来源写 `SOURCE.md`（出处/逐文件改动/义务）+ 复制上游 LICENSE 原文 + 生成总索引）；新增 `docs/ASSET-LICENSES.md`（逐项义务与商用替代方案）+ `THIRD_PARTY_NOTICES.md` 三个项目条目 + 插件设置页「美术资源许可」卡（运行时可见，含协议链接与改动说明）；**刻意排除** Star-Office-UI 内的 `guest_role_*`/`guest_anim_*`（来自 LimeZu，其许可禁止再分发）；**像素美术仅限非商业**，商用请切「等距矢量」场景（本项目自绘，无第三方约束）或替换资源。③**监控面板对标 lobster-pet 重排**：总览页改为其 `DetailPanel` 单屏卡片网格——行 1 = 状态卡(236px) + 最近会话卡网格 + 活动概览（14 天热力图/会话类型环形图/小时柱状图）；行 2 = 左栈（团队卡 + 任务与工具卡 + 数据源与健康度卡）与**图书馆场景大卡**；行 3 = 6 张紧凑 KPI 卡；**场景从「一个页签」变成「监控界面里的一张卡」**（对标其 MiniOffice），与其它监控卡共享同一份快照；全屏「图书馆」页签保留。④**修复**：两套场景引擎共享 store 槽位导致像素场景态被喂给等距引擎崩溃（`Cannot read properties of undefined (reading 'col')`）→ 改为 `isoScene`/`pixelScene` 双槽位；上游 mcp/images/log/schedule 四个房间的 workZone 锚点落在房间矩形之外（上游数据不一致）→ 按 28px 边距夹回房间内；场景根节点加 `data-scene` 标记，DOM 审计按场景分别校验。新增 2 个测试文件 / 18 用例（像素场景数据 12 例含**资源文件真实存在**与许可声明齐备 + 像素场景渲染 6 例含精灵 URL/帧偏移/角色落在自己房间/资源缺失降级）。全量 186 文件 / 4432 用例通过 + tsc 零错误 + vite build 成功（插件 chunk 126KB JS + 39KB CSS，美术资源 5.17MB 随包）+ headless DOM 审计通过。
> v1.12.0 已发布（2026-09-10，图书馆运营监控插件 @codem/ui-library-ops — 团队在图书馆里工作 + lobster-pet 式运营监控看板）：**完全独立的大插件，启停不影响任何现有功能。**①**团队角色 → 动画角色**：`data/characters.ts` 由角色 id 确定性生成外观（12 调色板 × 4 身形 × 5 发型 × 6 头饰 × 6 道具 × 4 表情 = 34560 种；岗位反向影响头饰/道具：队长礼帽 / 运维工帽 / 研究学者帽 / 编码耳机 / 写作贝雷帽）；**11 种工作动画**（待命/行走/思考/阅读/撰写/执行/检索/等待授权/完成/出错/休眠）由**真实工具调用与任务状态**驱动（read→阅读、write→撰写、bash→执行、web_search→检索、ask_user→等待授权…）。②**ClawLibrary 式图书馆**：等距 2.5D 场景（SVG 地板/区域/装饰 + DOM 角色层）、**10 个岗位**（前台/阅览大厅/编目室/代码工坊/写作工坊/档案室/机房/会议厅/借还台/静思角）、按角色标签关键词自动分配岗位 + 工位槽位（同岗位不叠格）、可通行网格 + 网格 BFS 寻路、到岗停留 + 头顶名牌 + 工作气泡 + 深度排序。③**lobster-pet 式监控界面**：标题栏（实时状态/时钟/刷新/关闭）+ 左侧 9 页签 + 右侧实时事件流；总览（6 KPI + 健康度环 + 14 天热力图 + 会话类型环形图 + 小时活跃柱状图 + 数据源健康）、图书馆、团队（成员 + 任务看板 + 完成率环）、会话、工具（频次排行 + 调用流水）、成本（token 构成 + 成本趋势）、错误（阻塞/出错角色 + 失败任务 + 来源健康）、时间线（按类别过滤）、设置（采样间隔/动画速度/显示开关/默认页签/自动打开）。**图书馆即监控界面内的场景页签**，与其它监控卡共享同一份快照。④**真实数据只读接入**：会话/运行时团队（AgentTeamsService）/子智能体（SubagentRuntime）/团队模板（SquadManager）/智能体定义（AgentRegistry）/工具调用（消息流）/token 与成本（CostTracker）/遥测（TelemetryCollector），逐源 try-catch + `sources.failed` 可见性，**零写入宿主**（LO-INT-7 禁止词表门禁）。⑤**集成零侵入**：挂宿主已有的 `app.overlay` slot → **App.tsx 零改动**；插件管理可禁用（provider 不装配 → 入口与面板都不存在）；面板关闭即停止采样（无后台轮询）；可拖拽入口圆钮（位置持久化，双击复位）+ Ctrl+Shift+L + `codem:open-library-ops` 事件。⑥**皮肤契约**：全插件零硬编码色值（调色板也是 `var(--token)`），四套皮肤自动适配 + 尊重 `prefers-reduced-motion`。**⑦四轮全面审计修复 28 项（P0 8 / P1 11 / P2 9）**——审计方法：真实服务联动（不用 fake，真 AgentTeamsService/SquadManager/CostTracker）+ 代码级审查 + headless 浏览器渲染 DOM 几何审计 + 回归门禁；**含 1 项宿主 Bug**：`AgentTeamsService.kick()` 跳过 working 成员且任务进终态无人释放 → 成员永久卡「工作中」（新增 `releaseAssigneeIfIdle()`，任务终态/转派后若名下无未完成非终态任务则释放为 idle）；其余 P0：活跃会话 `Map` 判定失效（原按 `Set.has` 判，所有会话永远不活跃）、气泡永不消失、到岗瞬间动画不切换（改 rAF 直写 DOM）、`done` 动画永久定格、角色站到岗位外（工位槽位只取区域内 + 吸附可通行）、**等距几何双重偏移**（区域高亮/地板/背墙在投影之上又加半瓦片偏移 → 整体放大错位、网格与地板不对齐，抽出 `components/library/iso.ts` 固化角点/中心两套约定）、道具转向换手（SVG 已镜像又按朝向换 x）；P1：相机缩放/平移/定位（画布 1424×768 塞进面板后角色仅 20 多像素）、采样超时兜底、岗位详情卡与超容告警、家具从文字字形升级为 8 类等距矢量家具、背墙加窗、入口圆钮状态徽标、耳机横梁被 fill 覆盖与帽带色失效的 CSS 特异性修复、角色锚点改「脚下」、选中角色镜头平滑居中、切页签保留馆内状态；P2：新增 4 个测试文件（真实联动 8 / 几何不变量 9 / 渲染几何 7 / 门控 1）、抽出 `monitor/labels.ts`、删死代码（同名不同义 shortId / 无调用 formatDuration·formatDay·clampTile·zoneTooltip）、禁用门控抽到 `core/ui-plugins/gating.ts`、`data-zone-id` 可测性挂钩、岗位键盘可达、`tools/preview/` 视觉预览与 DOM 审计脚本、修复新增测试导致的 vitest worker 收尾竞态（`afterAll` 调 `flushDatabase()`）。新增 9 个测试文件 / 100 用例。全量 184 文件 / 4414 用例通过（连续 4 次复跑稳定）+ tsc 零错误 + vite build 成功（插件独立 chunk 懒加载）+ headless DOM 审计 12/12 角色落在自己岗位的等距包围盒内、0 处 NaN。审计报告：docs/LIBRARY-OPS-AUDIT.md。
> v1.11.2 已发布（2026-09-09，zg 在线安装 Node 源根治 + 审计四坑修复 + 功能文档体系）：**①zg 在线安装根治**——便携 Node v24.19.0 并入 zg 单包（162MB 隔离 Node 不冲突系统），安装只从 GitHub Release 取包（不再访问 nodejs.org/npmmirror：部分网络对这些源返回 404 是此前一键安装失败根因，改 GitHub 分发根治）；系统 Node≥22 复用、包内 Node、网络兜底三级解析；离线 zip 全离线；打包脚本六步升级。**②审计四坑修复**——PPT「导出 PPTX」占位→真 OOXML 图片型 PPTX（buildPptxFromImages，16:9/contain/PNG·JPEG/IHDR 回退，OPC 结构完整性单测）；纠偏模型配置面板真实接线（codem-correction-model 持久化 + fact_check 专属/回退主模型诚实标注，移除 ctx.correctionProvider 假默认）；Whisper 语音输入双引擎闭环（语音设置引擎选择 + 麦克风 MediaRecorder 录音 → /audio/transcriptions 转写回填 + 未配置引导/无 SpeechRecognition 降级；公开 transcribeAudioFile 并让 vision-proxy 共用）；会话内搜索激活（头部入口 + 点击平滑定位含 unified 合并气泡回溯；修旧内联过滤残留残缺视图 bug）；附带清理 InputArea 重复声明。**③功能文档体系**——项目功能说明介绍（20 域 152 亮点，皮肤独立域 13 细粒度 + 附录分层索引）+ 项目功能树-全量（18 路并行只读审计 2779 项叶子，含证据路径与宣传句，占位/未接线不收录）。全量 174 文件 / 4302 用例通过 + tsc 零错误 + cargo check 通过。**覆盖包（50010f3）**：新增「手动添加自定义模型名」——/models 列表外的内测模型（如 deepseek-v4.1-flash-expires-on-0910）可在设置按 Provider 直接添加/删除；codem-custom-models 独立存储，引擎/设置页/聊天模型下拉/方案面板统一 merge 合并、不入服务器缓存；12 单测 → 全量 175 文件 / 4314 用例通过。
> v1.11.1 已发布（2026-09-09，zvec-grep（zg）语义检索集成 + archify 图表技能 + UI/体验修复打包）：**①zvec-grep（zg）语义检索（可选增强，不改架构）**——本地「语义向量+BM25+rg」统一检索（zvec 驱动），与内置 grep 双轨并行、模型按工具描述智能路由（精确锚点→grep；措辞未知/语义/跨文件→zvec_grep_search）。运行时按用户主动安装于 `<appData>/.codem/zvec-grep/`（不进安装包），经 MCP stdio（`zg server --stdio` 自动起/复用 daemon）接入现有 MCPRegistry；Rust 新增 `http_download_ext`（长超时大文件下载）/`extract_zip`（zip-slip 安全解压）；编排服务 `src/core/zvec-grep/`（node 检测/便携下载、在线一键安装、离线单包导入、卸载、索引重建与模型切换）；`syncZvecTools` 注册 zvec_grep_search（仿 codegraph）+ grep 描述双向路由 + 只读/并发/权限集合加白；插件市场新增「本地语义检索（可选增强）」卡片；发布单合并包 codem-zvec-win-x64.zip（~125MB = 裁剪运行时 + potion-code-16m-v2 模型 MIT；脚本 scripts/build-zvec-runtime.ps1；保留 *.wasm 供 tree-sitter；全离线 e2e 验证）。**②archify 图表技能内置**——src/core/skills/archify/（v2.17 tt-a1i MIT）：架构/工作流/时序/数据流/生命周期 JSON-IR→自包含交互 HTML（showcase 9/9 校验 0 错误），产出 Codem 架构图/功能结构图（artifacts/archify/，组件带源码证据 verified）。**③UI/体验修复打包（v1.11.0 覆盖包内容随本版首发）**——标题栏可拖拽（清 titlebar-center/nav-actions no-drag 残留）+ 品牌 Logo（icos/codem.ico→PNG）；左下角头像读 codem-user.avatar 即时刷新；右侧栏磨砂玻璃（z 920）；消息导航轨全内容比例铺轨/纯紫点/最小间距/预览跟随；Git 分支控件并入右侧栏；右侧栏智能体 tab 收敛。全量 171 文件 / 4260 用例通过 + tsc 零错误 + cargo check 通过。
> v1.11.0 已发布（2026-09-08，团队体系深合并 + 智能体双维度面板 + 审计修复）：**①团队体系深合并（B）**——两套"团队"（TaskCenter 静态 Squad 模板 + agent-teams 运行时 DAG 团队）合并为单一「团队 = 角色模板 + 运行时编排」：Squad 升级为团队模板（TeamTemplate/toTeamTemplate），`squad_dispatch` 按模板建 agent-teams 运行时团队（队长=当前会话/角色 spawn 可续聊成员/任务入共享池调度；无 agent 角色前置拒绝），`squad_status`=模板+运行时摘要；TaskCenter `squads`→`teams`（TeamTab：说明+运行时活动+模板管理），对话旁「团队活动」收敛为快捷入口（codem:open-task-center）；删除旧事件路由与 generateSquadRoster/SquadDispatchResult 死码。**②「智能体与团队」双维度面板**——顶部「子智能体」页首部显示活动团队卡片（成员=角色+状态点，订阅刷新），点成员行内展开个体动态预览（不跳页；「完整详情 →」进 AgentDetail），成员从平铺去重；修复 engine snapshot 丢成员 id（下钻/去重）。**③持续审计修复**（executor end 判据补全 / 宠物卡误清 / runPs 超时余量）。盘点矩阵 docs/AGENT-SYSTEMS-MATRIX.md，计划 docs/TEAM-CONSOLIDATION-PLAN.md。全量 168 文件 / 4239 用例通过 + tsc 零错误。
> v1.10.0 已发布（2026-09-07，EAC 对标 DSH-Desktop-EAC 第①②③④项 + 四路审计修复）：**④宠物大肥鱼式状态卡**（真实 Agent 事件驱动工作状态卡：项目名/阶段/真实步骤进度，total null 不编造；PetCard + pet-store updateCard + PetWindowApp 卡 UI + 锚点 resize）→ **③@codem/computer-use 电脑操作**（10 个 computer_* 工具：截屏/点击/输入/组合键/滚轮/拖拽/移动/等待/读光标/改模式；PS 零依赖后端；默认 manual 手动批准 /computer；computer_see 独立视觉模型）→ **②@codem/wechat-bridge 微信 ClawBot 桥（iLink 直连）**（Rust 传输层 8 态扫码登录 + 配对码 + getupdates 长轮询 + 收发 + 每 peer 10条/24h 配额软记账；TS 引擎桥 peer→持久会话 executeSessionTurn + /命令 + 白名单准入；QR/配对码/准入设置卡）→ **①@codem/phone-link 手机连接**（Rust LAN HTTP 地基扫码配对 + 手机浏览器浏览/续聊/新建桌面会话）。**四路只读审计修复**（P0 computer-use PS 调用约定断链 / 动作名失配 / 宠物卡隐藏信号 / executor 失败落库 / /api/chat 202 语义 / 配对 cookie 重放 / 插件禁用=关闭真实生效 / ui-pet·mimo-auth·hot 元数据对齐 / wx-workspace 过滤）。实施记录 docs/EAC-PHASE2-STATUS.md。全量 167 文件 / 4231 用例通过 + tsc 零错误 + Rust cargo test 38 通过。
> v1.9.9 已发布（2026-09-07，EAC 对标 DSH-Desktop-EAC）：**UI 交互改进 + 3 个新可启停插件** — ①**编辑并回退**（fork 保留原会话，对标 dsh-message-rewind）②**节点导航升级**（ScrollbarMarkers v2：滚动容器修复/hover 预览/滚轮切换/📌 消息精选 pin per-session）③**输入框失焦折叠**（meow-smooth 式）④**字号生效**（--ui-font-scale 全刻度缩放）⑤**设置搜索**（跳转分组）⑥**persona 人设卡插件**（管理 UI + # Persona 注入主 prompt + 文件热重载 + 插件门控；修 SOUL 孤儿缺陷）⑦**side-session 临时会话插件**（页内悬浮窗独立流式问答，text_delta 解析，不污染主会话）⑧**@codem/agent-teams 团队编排插件**（依赖任务 DAG 状态机/attempt 令牌防覆盖/成员邮箱租赁/共享调度/活动面板/10 个 agent_teams_* 工具 defer 注册）。差距矩阵 docs/EAC-GAP-ANALYSIS.md + 验证清单 docs/EAC-VERIFICATION.md。全量 163 文件 / 4204 用例通过 + tsc 零错误。
> v1.9.8 已发布（2026-09-07）：**对话用量/缓存命中率统计真实化（对标 anywhere-labs/dsh-desktop）+ 稳定前缀优化（date 尾置）+ 真实请求实证** — ①**真实数据链路**：TokenUsage 扩展 cacheHitTokens/uncachedInputTokens；provider 归一化透传 DeepSeek `prompt_cache_hit_tokens` / OpenAI `cache_read_input_tokens`（usage-normalize.ts 口径精确化，uncached 优先显式 miss）；agentic-loop 每轮累计真实缓存命中；token-tracker 真实优先、未上报才回退估算；StatsLine inputTokens 改 uncached 口径 + 诚实精度命中率显示（cacheReported 门控，未上报不显示防误导 0%）②**稳定前缀优化**：date 每分钟变化原在系统提示中段切断缓存——尾置为独立 # Current Date 段（>95% 公共前缀断言防回退）③**用量统计面板缓存命中卡**：近 7 天 provider 上报调用聚合（诚实精度）④**真实请求实证**：DeepSeek 前缀缓存需足够大前缀——短请求恒 miss、长前缀（3259 tokens）完全相同重复命中 98.2%、96K token 前缀纯重复稳态命中 **99.947%**（达 dsh 99.97% 量级；dsh 的高值来自更长前缀稳态）⑤端到端集成测试（真实 AgenticLoop + 脚本化 provider 回放，含缓存价差成本累计）。全量 157 文件 / 4159 用例通过 + tsc 零错误。
> v1.9.7 已发布（2026-09-04，含同版本补丁 7d1f329/88b49f2 覆盖发布：CodeGraph defer 接入与一键安装 + 技能市场联网搜索卡死修复 + 输入框光标视觉错位修复 + GitHub URL 标签重复生成修复）：**dsh 插件市场 Tab + dsh-compat 懒解析接入 + 插件皮肤兼容契约 + 插件架构审计修复** — 分析 harness 插件生态（npm 包 + cordis.patch.yml 分层装配 + Loader 激活）后确认 Codem 装配/激活同构但无法加载任意 npm 插件（运行时鸿沟），落地三类兼容模型：bundled（codemAnchor 等价映射，安装 = enable 内置插件；锚一对一唯一）/ adaptable（dsh-compat 懒解析代理桥接，恒注册 7 个 dsh 别名、调用时现取真实服务；服务名矩阵审计证实核心 seam 同名直通）/ unsupported（诚实标注）。插件管理弹窗新增「插件市场」Tab（50 条 @deepseek-ai/dsh-* 目录浏览/搜索/**动态分类**/安装/卸载——核心恒启条目只读"已启用（核心）"、可卸载条目"禁用" + npm registry 在线检索跟随输入 + 小窗自适应）。**插件皮肤兼容契约**：皮肤令牌唯一化（skin-tokens.ts 审计 + SC-1~5 门禁，插件 UI 零硬编码色，适配 default 亮/暗 + dream + hub；docs/SKIN-PLUGIN-CONTRACT.md）。另含执行轨迹事件日志持久化（重启回放）+ 首页无会话自动新建全局对话。全量 150 文件 / 4122 用例通过 + tsc 零错误。
>
> v1.9.6 已发布：**打包版运行问题修复** — ①CSP script-src/worker-src 加 blob:（知识笔记本导入 docx 的 transformers WASM 动态加载不再被拦，修复"no available backend found"索引失败）+ connect-src 加 ipc:（IPC 不再回退 postMessage）②codem.base.yml 移除已删 @codem/terminal-bash（启动 YamlLoader failed 0）③extractJSON 失败静默单次告警（不再刷屏）④知识摘要解析失败降级文本摘要（前 200 字）⑤SubagentRuntime 同步初始化（9 个依赖 subagent 插件正常激活，消除 assertActivated FAILED）。全量 145 文件 / 4102 用例通过 + tsc/cargo 零错误。
>
> v1.9.5 已发布：**对话步骤语义化与动态插入（对标 dsh todo）+ token 消耗审计修复 + 全面功能审计修复** — ①**步骤语义化**：任务意图检测修复"修复卡死"被判纯问答显示"回答问题"的根因 + 执行型任务强制 LLM 语义计划（planSteps 好/坏例 + 空白清洗）+ 新增 update_plan 工具动态插入（insert_before/after/append、编号顺延、禁插已完成区、上限校验、回执新计划）+ 每轮计划上下文注入 + fromLlm 门控防泛化步骤（STEP-P1~P8 + STEP-L1~L9 共 17 用例）②**token 审计 6 项**（用户报告同样任务比 dsh 大数倍）：read 单次 100k→50k 字符 + 陈旧大工具结果 head+tail 裁剪（context-fold 8192/4096/1024 保留最近 2 条）+ 7 低频工具 defer（schema 10.6k→7.5k tok/轮）+ systemPrompt 工具信息三重复裁剪 + select 预算对齐真实窗口 + 截断丢历史零成本折叠摘要（TOK-F1~F7）③**功能审计**：PTY 关闭/退出杀进程树 + 终端 spawn 失败 DOM 残留 + 4 处裸 fetch 超时 + 托盘退出先 flush 再退出。全量 145 文件 / 4102 用例通过 + tsc/cargo 零错误。
>
> v1.9.4 已发布：**dsh-desktop 全面对标稳健性审计修复（15 轮迭代，42+ 项，bug 级清零收敛）** — ①**崩溃检测与恢复链路（对标 dsh crash-evidence）**：active-run.json 崩溃标记（pid+启动时间，quit_app/托盘/ExitRequested 三重清理）+ 上次异常退出 previous-run-unclean 界面提示 + panic 落盘 codem-crash.log ②**渲染崩溃恢复边界（对标 dsh renderer-health）**：新增 AppErrorBoundary 顶层错误边界（白屏→恢复卡片：重试渲染/重新加载/重置界面设置并重新加载，仅清 codem-* 本地键不动会话数据 + 崩溃证据脱敏留存下次启动提示），测试 REC-R1~R6 ③**运行时文件日志（对标 dsh log-files.ts，新增）**：runtime_log.rs 按日 codem-runtime-YYYY-MM-DD.log + 段轮转（4MB×3）+ 目录上限 24MB + 保留 14 天 + 统一脱敏 mask_secrets + 行截断，启动/崩溃/命令/超时杀树/PTY/退出落盘，13 个 Rust 单测 ④**持久化失败可见性**：DB 写盘失败不再静默（codem:db-save-failed 事件 → guidance + 3s 自动重试 + 成功复位），DBSAVE-F1~F4 ⑤**命令执行**：超时杀进程树（600s 默认）+ PowerShell 安全转义（HEAD^{tree} ScriptBlock 崩溃修复）⑥**网络/安全**：fetchWithTimeout 统一超时全覆盖 + redact.ts 错误统一脱敏 + WebSocket 定时器泄漏修复 ⑦**前端稳健性**：全局错误 alert→记录 + 6 处 JSX 优先级修复 + 终端 pty-exit 僵尸会话回收 + mermaid strict + useWindowState + telemetry flush 保护 + mkdir Rust 化。新增 repro-ps-command/exec-timeout/bash-abort/jsx-classname/redact + app-error-boundary + db-save-failure-alert + KM-074 30s 超时修复。全量 141 文件 / 4079 用例通过 + tsc 零错误 + cargo 零警告 + cargo test 13/13。
>
> v1.9.3 已发布：**安全模式完全访问修复 + 工具调用配对修复 + 输入框历史 wrap 修复 + 引导栏 UI 对标 wecode + 思考过程紫色样式恢复** — ①**完全访问重启后失效修复**：dbReady 时序（DB 就绪前 securityMode state 回退 ask），DB 就绪后重新同步 + 依赖数组加入 currentProject?.path（切换项目重新解析）②**委派/后台任务遵循用户安全模式**：executor.ts 不再硬编码 auto，改为 getEffectiveSecurityMode(cwd)（full 放行）③**write 拒绝误判修复**：限定 name === "write"（任何工具输出含 "User rejected the overwrite" 字面量被误判为用户拒绝，ask/auto/full 全部失效）④**工具调用配对修复**：按 tool_call_id 精确配对，修复 API 400 "insufficient tool messages"（DeepSeek/OpenAI 严格配对要求）⑤**输入框历史 wrap 折行修复**：视觉行判断（镜像测量）替代 indexOf("\n")⑥**记忆检索正则元字符转义**⑦**引导栏卡片式三操作**（立刻引导/编辑/取消 + 状态胶囊，对标 wecode ChatInputCard）⑧**思考过程紫色样式恢复**（对标 v0.96.0）。新增 10 个 repro 回归测试（34 用例）。
...
### v1.10.0 (2026-09-07) — EAC 对标第①②③④项 + 四路审计修复（已发布）
- [x] ④ 宠物大肥鱼式状态卡（929a0f3）— PetCard 类型/pet-store updateCard/App 汇聚器（llm_status/step_progress/tool_*/end → 阶段与真实步骤）/PetWindowApp 卡 UI + 锚点 resize；不编造进度（total null 语义）
- [x] ③ @codem/computer-use（c7ad786）— 10 个 computer_* 工具 defer 注册 + PS 零依赖后端 + manual 默认手动批准（/computer）+ computer_see 视觉通道 + Rust read_file_base64 + 设置卡；三件套注册默认开启可禁用
- [x] ② @codem/wechat-bridge（229c0ae）— Rust ilink 传输层（8 态扫码登录/配对码/getupdates 长轮询/收发/配额软记账/会话落盘）+ TS 引擎桥（peer→会话 executeSessionTurn + /命令 + 白名单准入）+ 设置卡（QR/配对码/准入/主开关）；媒体/群聊二期标注
- [x] ① @codem/phone-link（163815d）— Rust LAN HTTP 地基（配对门卫 token+cookie + 内嵌手机页 + /api/* 事件代理）+ TS 引擎半层（status/sessions/messages/chat/chat_new）+ 设置卡；devices.json 哈希落盘重启保配对
- [x] 审计修复批1（bccd152）— computer-use runPs 调用约定 P0 断链（& '<ps1>' -Json 姿势）+ 动作名失配（wait/getpos/move/action2/from-to）+ vision-proxy 公开 describeImagePublic（computer_see 强制视觉模型）+ get_system_temp_dir + b64 去 Buffer + searchHint 双语 + 契约回归测试
- [x] 审计修复批2（57e2a3c）— 宠物卡隐藏信号（emitPetStateLight/sendFullStateToPet 恒发 card:null）+ finally 归位集合扩大 + 宠物停用清卡；/api/chat 先 202 后台执行；/status 配额按 peer；配对 cookie 重放；ilink fetch_qr epoch 校验；phone_start 并发互斥；executor end-reason 失败落库返回失败；回合 race 兜底超时；插件禁用=关闭（App 门控 wechat 停监听 / phone 停 LAN / computer 门禁全拒）+ ui-pet/mimo-auth/hot 元数据对齐 + wx-workspace 内部项目过滤 + sendTestMessage 接入 provider + CHANGELOG/README/PG/TODO 文档同步
- [x] 全量 167 文件 / 4231 用例通过 + tsc --noEmit 零错误 + Rust cargo test 38 通过
- [x] Release v1.10.0 发布

### v1.11.0 (2026-09-08) — 团队体系深合并 + 智能体双维度面板 + 审计修复（已发布）
- [x] 持续审计第 1 轮（dde3620）— executor end 失败判据补全（非 completed 即落库失败，覆盖 safety_valve 等）/ 宠物卡轻量推送误清修复（emitPetStateLight 缺省跟随 store 当前 card）/ runPs 默认超时 60s
- [x] 体系盘点矩阵（07dd260）— docs/AGENT-SYSTEMS-MATRIX.md（5 套体系入口/能力/重叠；确认 EAC dsh-agent-teams 为会话旁浮动面板）
- [x] B 深合并 Phase1（dab14c5）— Squad→团队模板（TeamTemplate/toTeamTemplate）；squad_list/squad_dispatch/squad_status 语义升级（dispatch 桥接 agent-teams 建队+spawn+派发，删 CustomEvent）；App 事件路由删除；测试适配 + team-consolidation 5 例
- [x] B 深合并 Phase2（6585c9d）— TaskCenter squads→teams（TeamTab 双区：说明+运行时活动（AgentTeamsPanel 嵌入）+模板管理）；对话旁按钮收敛（codem:open-task-center）；旧 id 归一
- [x] B 深合并 Phase3（abfec90）— 清理 generateSquadRoster/SquadDispatchResult 死码；测试对齐
- [x] 深合并后审计（a5b4e1e）— squad_dispatch 无 agent 角色前置拒绝 + TC-006
- [x] 双维度面板（86646ac/60b89aa/120ae66）— AgentPanel 首部团队卡片；engine snapshot 补成员 id（修下钻/去重）；成员行内个体动态预览（不跳页，完整详情→AgentDetail）；展开态随团队变化清理；离线成员提示；TC-007
- [x] 全量 168 文件 / 4239 用例通过 + tsc --noEmit 零错误 + cargo test 38 通过
- [x] Release v1.11.0 发布
