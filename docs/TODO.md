...
> **待发布（场景图可上传替换）**：场景「画面」与「布局」解耦——角色站位/岗位标签/点击热区都来自固定数据，图片只是铺满 1920×1072 画布的一个图层，于是换图不再需要改代码。①**设置 →「场景图片」**：内置画廊（`claw` ClawLibrary 像素画双层 / `ai-library-01` 本项目自有 AI 整图，可商用）一键切换；点「选择图片」或**直接把图拖到场景上**即可上传（PNG/JPG/WebP/AVIF/GIF/BMP，≤32MB、≥640×360，比例偏离 16:9 超 8% 提醒会被拉伸）；图片以 Blob 存 **IndexedDB**（`codem-library-ops`/`scene-images`，避开 localStorage 5MB 配额），不支持时降级为「本次会话有效」并明确提示。②**对位**：画面微调（缩放 0.5–2×、位移 ±600px）+「对位参考线」（12 房间框 + 20 行走节点）+ 设置卡内「房间框叠缩略图」的对位预览。③**新增工具**：`scripts/build-library-ops-scene-preset.mjs`（任意图 → 2752×1536 WebP 预设 + 缩略图 + 可直接粘贴的 `SCENE_PRESETS` 条目）+ `public/library-ops/scenes/`（自有素材 + SOURCE.md）。④**文档**：`docs/art-prompts/00-怎么用.md` / `01-图书馆场景.md` 增加「自己上传」路径，`docs/LIBRARY-OPS-PLUGIN.md` §7.4、`docs/ASSET-LICENSES.md`、`public/library-ops/README.md` 同步。新增 4 个测试文件 / 28 用例（纯逻辑 10 / IndexedDB 6 / 设置卡 7 / 场景渲染与拖拽 5）。全量 190 文件 / 4488 用例通过 + tsc 零错误。
