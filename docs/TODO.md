...
> **待发布（v1.15.1：修复任务管理「概览」没有滚动条）**：内容区 `overflow` 原先绑在 `wide`（面板宽度）上，概览加宽后被 `hidden` 裁掉；现拆成 `wide`（board\|subagents\|overview）与 `fillsViewport`（board\|subagents），内容区按后者决定滚动，并加 `data-task-center-content` 标记 + DEDUP-7/8 回归用例。198 文件 / 4549 用例通过（+15 跳过）。
>
