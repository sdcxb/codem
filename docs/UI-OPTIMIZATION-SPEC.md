...
> **1.16.170 执行时更正的口径（两条，原验收按字面做会做错）**
>
> ① **「geometry 声明 → 0」会把皮肤自己的界面删掉。** 实测皮肤文件里的几何声明是
> hub 75 / dream 66 条，其中 **41 条属于 `.hub-navbar` / `.hub-agent-card` / `.hub-task-icon` /
> `.dream-polaroid` 这类「只有该皮肤才有的布局类」**（主样式表里根本没有对应规则）——
> 它们是 hub/dream **自带的界面**，删掉皮肤就散架。真正的问题是「**覆盖共享组件**」那 34 条
> （hub 11 / dream 23）。**判据改成**：选择器里的类名**全部**在主样式表里出现过 ⇒ 才算覆盖共享组件。
>
> ② **「三个皮肤几何完全相同」也不该是目标。** 三个皮肤各自有一整套圆角刻度（默认 xs4/sm6/8/md10/lg14/xl20、
> hub 2/4/6/8/12/16、dream 4/6/10/12/16/24），那是**有意的皮肤语言**；把 `.sidebar-session` 的
> 6/4/6px 强行拉平，等于取消皮肤的圆角语言。真正的毛病是**分叉"读不出来"**（散在 34 条规则体里，
> 其中 8 条还是值等于主表的纯冗余）。**验收改成**：共享类的几何分叉 **34 → 0**（改为引用令牌），
> 且**剩余差异 100% 可追溯到该皮肤令牌块里的一条覆盖**（可机检）。
>
> 落地结果见 `CHANGELOG.md` 的 1.16.170 段：主表新增 15 条组件角色几何令牌，皮肤令牌块各一处集中覆盖，
> 装机版跨皮肤探针逐项对照**渲染零变化**。
...
> **1.16.171 执行时更正的口径（实测推翻了上面两个数字）**
>
> 实测（`.preview-shot/_measure-truncation2.mjs`）：`overflow: hidden` 规则 **217** 条，
> 其中无任何截断机制的 **134** 条 —— 但**绝大多数是布局裁切**（面板裁圆角、头像裁方、输入框裁溢出、
> `.sr-only`），不是文本容器；**真正的"硬切"形态（`nowrap` + `hidden` 且无 `text-overflow`）只有 1 条，
> 而且就是 `.sr-only` 本身**（屏读专用类，`nowrap + hidden` 是它的设计）。
> ⇒ 「148 → ≤20」这条棘轮会把力气花在**正当写法**上，故**不采用**。
>
> 真问题只有一条，而且此前没有任何门禁看得见：**10 条多行截断只写了 `-webkit-line-clamp`，
> 没有标准 `line-clamp`**（换引擎静默失效）。已全部双写，并新增工具类 + `OverflowText`（只在真被截断时挂 `title`）。
> **验收改成**：① clamp 双写 100%；② `.truncate` 一族存在且双写、且有真实消费者；③ 硬切形态 0 条（白名单 `.sr-only`）。
> 门禁 **TEXT-1** 守这三条。
...
- **皮肤**：hub/dream 几何声明 **74 / 53 条**、**三套圆角标尺**、**三套玻璃材质**、**两皮肤 `:active` 均为 0**（基线按压反馈在皮肤下失效）、自带宽窄断点与主样式冲突。
  改法：几何全部交回主样式表（皮肤只覆盖颜色/阴影/材质），并给皮肤补 `:active`（或让基线按压选择器覆盖皮肤）。  验收：皮肤文件几何声明 **127 → 0**；三皮肤下行高/圆角/内边距完全相同；皮肤下按压反馈可测（`transform` 变化）。

> **1.16.170 / 1.16.171 执行时更正（这一段的三条结论有两条不成立）**
>
> ① 几何声明「127 → 0」与「三档几何完全相同」：见上面 P2-3 的更正（41 条是皮肤自带界面；
>    三套圆角标尺是**有意的皮肤语言**）。已按「只盯共享组件 + 剩余差异可追溯」落地，门禁 SKIN-3。
> ② **「两皮肤 `:active` 均为 0 ⇒ 按压反馈失效」不成立。** 用 CDP `CSS.forcePseudoState` 真机强制 `:active` 实测：
>    默认档背景 `transparent → 墨色 9.8%`、位移 `translateY(0.98px)`；hub `→ 10% 浅墨` + `0.98px`；
>    dream `→ 9.3% 墨色` + `0.93px` —— **三档都有反馈**（皮肤文件里 0 条 `:active` 是因为基线选择器
>    是**级联下来**的，皮肤没有再覆盖 background/transform）。**判据必须看渲染结果，不能数源码里的规则条数。**
>    本项无需修改（记档以免下轮照着原文去"补 `:active`"）。
> ③ 浮层那一半（手写 Escape 14 → ≤2、`useDismissableLayer`）尚未做，留给 P2-6 的浮层批。
