...
| 深层面板走查（插件/技能/智能体/MCP） | **部分闭合**（1.16.95） | 关键阻塞已解：走查选择器补上 `.modal-overlay`（四个面板共用，z-index 1300 满视口）；面板关闭按钮补上可访问名 ⇒ **复量：插件管理面板内无名按钮 1 → 0、关闭按钮可按名字找到**。⚠️ 仍开着：①同面板 **170 个 `.market-skill-link-btn` 命中区 24×18**（密集列表，需按布局判断）；②`.lo-link-btn` 的命中区修复已进构建但**本轮未复量**（该面板里计数为 0）；③技能/智能体/MCP 三个面板尚未逐一跑读数 | 下一轮：按面板各跑一次读数，再按布局决定是否扩 `market-skill-link-btn` 命中区 |
