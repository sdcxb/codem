...
- **平台**：Windows 优先
- **版本**：v1.8.0（知识图谱 React Flow 重构 + DSH 框架穿透性整改 + UI 设计规范化，2026-08-31）
