...
| 版本 | 日期 | 主要内容 |
|------|------|---------|
| v1.16.181 | 2026-09-26 | **全面审计（第 1 轮）：插件管理一次造 6095 个 DOM 节点 → 分片渲染** | ①审计怎么做（四路都留机器读数）：**功能** — 全量用例 + 逐面板走查 142 入口 / 护栏拒绝 32，四类真缺陷**全为 0**（控制台 error 0 / 无名按钮 0 / 真控件相挡 0 / 间距例外不成立 0）；**UI** — `scan-ui` error 0 warn 0 + `css-contract` + 12 面板逐个开关全过；**内存** — 反复开合 12 轮 + 强制 GC 逐轮采节点与堆：后半段 **Δ0**（无泄漏）但首次打开面板后 DOM 从 620 涨到 10901；**性能** — `bench:mem` 156MB 峰值 / 5.2s（390MB 正文）、`bench:db` 10 万行写入 7.1s，与历史同量级； ②**抓到的真问题**：逐个面板**先重载到干净状态**再量 ⇒ 插件管理自身 **6153** 节点（其余面板 620/321/228/197/151），而干净状态的**整个应用只有 620** ⇒ 点开一次 DOM 变 11 倍；归因到 `.skill-market-grid` 一棵子树 **6095** 节点 = **208 张卡**（207 内置插件 + 1）× 约 29 节点/张（全页 1952 个内联 `<svg>` 全来自它们），根因是 `plugins.map(...)` 一次性全渲染； ③**处置**：新增共享原语 `src/components/ui/IncrementalList.tsx`（首屏 40 + 哨兵进视口追加 + **可聚焦的「再显示 N 项」按钮**（网格没形成滚动容器时它是唯一的路）+ `resetKey` 变化回到首屏片数，否则"筛出 5 项"仍留着旧片数、切回来一次性全渲染 ⇒ **修复被自己的状态抵消**），覆盖 4 处同型网格； ④门禁用例 **IL-1…IL-8**（IL-8 是源级判据：网格 div 里不许直接 `.map(` 出卡片；变异 **50/50 红**）； ⑤**两只仪器虫记档**：用 `Performance.getMetrics` 数节点而 `Page.reload` 会重置该域 ⇒ 全读到 -1、表里全是 0、结论"每个面板都不占节点"（**"全是 0"最危险**），改成页内数节点且无效值**抛错不当 0**；IL-2 第一版点的是夹具自己的按钮、IL-8 第一版只查文件里有没有 `IncrementalList` 字样 ⇒ 两条变异都漏过，改成判据落在被测对象上； ⑥**如实标注**："面板关掉不卸载"是**保活**形态（后半段 8 轮 Δ0，不是泄漏），本轮不改 —— 分片渲染把常驻成本从 6153 降到首屏那一片 |
| v1.16.180 | 2026-09-26 | **P2-6 收口：浮层挂载入口收成一个（21 个文件不再各自 import react-dom）** | ①实测（改前）：`createPortal` 手写 **43 处 / 21 个生产文件**，逐个数过容器实参 —— **21 处全是 `document.body`**，只有 `SelectionTooltip` 是 `containerRef.current \|\| document.body`；代价不是"不好看"而是三条具体的：容器策略散在 21 个文件里（改一次要动 21 处）、门禁**无法回答**"新浮层有没有走共享层"（共享层不存在）、无 DOM 环境要各自判断而今天没人判断； ②处置：新增 `src/components/ui/portal.ts` 作为**唯一挂载入口**（从 react-dom 取 createPortal，全项目只此一处；容器默认 `document.body`；需要别的宿主时显式传 `container`；**没有 `document` 时安全返回 null** —— 与 React 会抛错不同，"渲染不出来"比"整页崩掉"更可接受）；21 个文件的导入改为从它拿，**21 处冗余的 `document.body` 实参全部删除**，1 处自定义宿主保留； ③门禁 **PORTAL-1a**（入口存在 / 只有它从 react-dom 取 / 必须默认 body / 必须防无 DOM；扫描先剥注释）+ **PORTAL-1b**（不许再有"最后一个实参恰好是 document.body"的调用；用**字符扫描配对括号**，因为正则数括号必然数错 —— JSX 里 `onClick={() => x()}` 很常见）； ④变异 **46/46 红 + 还原逐字节回绿**（本轮新增 3 条）； ⑤**如实标注未做**：方案里的「浮层契约」（不得出现 z-index/transform/filter/backdrop-filter）本轮不动 —— 21 个浮层的层级统一重排是观感级改动，需要单独的读数与截图对照 |
| v1.16.179 | 2026-09-26 | **P2-1 收口：24 处空态内边距收进共享族（棘轮 24 → 1）+ 顺手做掉"第三族"（O-32）** | ①先按**真实标记形状**分族（`_classify-empty-markup.mjs`）：18 处纯文本、5 处「图标 + 一句话」、2 处需撑满父容器 ⇒ 补两个修饰 **`.empty-hint.is-stacked`**（图标在上、间距走新令牌 `--empty-hint-gap` = space-4，是各站点 4/6/8/8/12px 五档的折中值）与 **`.empty-hint.is-fill`**（`flex: 1`）； ②迁移 24 处 + 删掉随附死规则（`.project-retry-btn` 换成共享 **StatusBanner** 的重试按钮；`.project-empty` / `.workbench-empty` 空壳删除），共享族现为 1 类 + **4 修饰**，消费者 **73 / 31 / 9 / 6 / 2**； ③**棘轮 24 → 1，那 1 处是 ChatPanel 欢迎页 `.empty-state`**（标题 + 副标题 + 建议按钮的富空态 + `flex: 1`）—— 留一条并写明理由，不为凑 0 去改用户第一眼看到的那一屏； ④规格值对照（`_diff-empty-before-after.mjs`）：24 套内边距 → 2 个令牌（取值未变）；**如实标注可见变化**：部分空态字号由 fs-sm/fs-xs/fs-2xs 统一到 fs-base、内边距收到 space-10、`.tc-empty` 文字色由 text-secondary 收到 text-muted、`git-env-ops-empty`/`layered-empty-data` 不再斜体； ⑤**顺手发现并当场做掉"第三族"（O-32）**：修门禁盲点时做全仓扫描 —— 名字像空态却不含 `empty-hint` 的元素 **37 处**，其中 **9 处是真空态**（它们**不写 padding**，所以 COND-1 的棘轮从来没看见）全部迁入，其余 **15 个 token 分三类登记**（富空态本体与子元素 / 共享空态里的图标与标题 / library-ops 插件自有的 BEM 命名）； ⑥**四个坑都记档**：COND-1 的排除条件按**子串** `input` 把 `.input-popover-empty` 整条跳过（真值 1 → 2）；`count()` 按子串数消费者、数不到组合写法（改按 token）；`.kg-empty` 有**两条**规则只迁了一条（被 LEGACY 断言当场抓到）；迁移脚本写错 4 条规则的文件名且不兼容单行/多行两种写法（改空白折叠匹配）； ⑦门禁 COND-1 扩到 4 修饰 + **新增 COND-1b**（名字像空态者必须挂 `empty-hint` 或在登记表里写明理由 + 扫描量 > 80 防空集）+ 逐站点自证 28 项（它真的抓到过 `McpMarketplace` 漏挂，而当时所有门禁全绿）；变异 **43/43 红**；`css-contract` 两次刷新共 164 处变化逐条可解释 |
| v1.16.178 | 2026-09-26 | **修 O-31：清掉历史重复文本事件（真机 6069 → 0，回放逐条不变）** | ①先更正旧读数：缺口清单里「5853 条文本事件 / 5614 条重复 / 最大会话 4368」全部来自 `events.list { limit: 5000 }` —— 最大那个会话有 **5474** 条事件、被截断在 5000 行；新探针逐页读全并用 `events.count` 交叉校验 ⇒ 真值 **8425 事件 / 6327 文本事件 / 6069 条完全重复**，算术自证 **4368 + 474 = 4842**； ②再更正判据：原计划的「每组保留 seq 最大（最新）」**是错的** —— `event-projection.ts` 里 `applyUserMessage` 首次胜出、`applyAssistantText` 的位置也由首次那条 `push` 决定 ⇒ 保留最新会把消息整体往后挪，**静默破坏回放等价性**；正确规则是保留 **MIN(seq)**，用交错流（A@10、B@20、A@30）钉住； ③处置两层：引擎新增 **`events.dedup_text`**（默认干跑、范围刻意只有 `user_message`/`assistant_text`（工具事件靠 toolCallId 互相引用）、判据是**载荷逐字节相同**、保留 MIN(seq)、`measure_delete_impact` 报账 + `storage_audit` 触发器留痕、幂等）+ 读路兜底 `collapseExactDuplicateTextEvents` 接进 `session_event_search` + 启动维护一次性步骤（settings 标记，只在成功后写；removed>0 走 advisory 并进汇总行）； ④真机读数（副本）：文本事件 **6327 → 258**、重复 **6069 → 0**、非文本事件 **2098 → 2098（一条没动）**、**回放等价性：逐会话比对投影消息 253 条、不一致 0 个**、再跑一次 removed=0； ⑤口径细差如实标注：按解析后的 (type,messageId,content) 是 6073 条重复，按载荷逐字节是 6069 ⇒ 有 4 行「同 mid 同正文但载荷字节不同」**刻意保留**（宁可少删）； ⑥用例 Rust 8 条 + TS 9 条（DEDUP-1…9）+ 变异 **8/8 红**；顺带修两只自己的虫子（新返回类型写成 export interface ⇒ knip 报 types 59→60；变异脚本两条「片段没找到」其实是 CRLF/LF 的**假绿**）；装机版实测：真库 8425 → 2356 事件、汇总行首跑「文本事件去重 6069 条」/ 重载后「0 条（本库已做过）」、提醒条文案与副本读数逐字一致、控制台 error/exception 0 |
| v1.16.177 | 2026-09-26 | **P2-2 收尾：26 个对话框补上语义（role / aria-modal / 可访问名）** | ①实测：role=dialog 4 → **25**、role=alertdialog 1 → **4**、aria-modal 4 → **28**、对话框本体语义不全 **18/26 → 0/26**（改前读屏把绝大多数对话框当成「一个普通 div 里多了一堆东西」：既不知道打开了对话框、也念不出名字）； ②处置：对话框**本体**（.modal-editor / .modal-panel / .confirm-dialog / .search-dialog / .task-center-panel / .nb-dialog / .petm-panel / .plugin-mgr-dialog / .chat-search-panel）补 role=dialog（确认类用 alertdialog）+ aria-modal + 可访问名；**遮罩不加**（语义属于里面的盒子）；名字全部取自组件自己的可见标题（ConfirmDialog 用 title 变量、闪卡用 isZh 三元），不编造文案；覆盖 App.tsx 的 8 个面板外壳 + ConfirmDialog/CloseConfirmDialog/GitHubCloneDialog/InteractiveFormDialog/PluginManager/TaskCenter/PetMarketDialog/PlanApprovalCard/ChatPanel 搜索面板/FlashcardViewer×3/NotebookWorkspace×3/NoteEditor/移除项目确认； ③门禁 **A11Y-DLG-1**（role + aria-modal + 可访问名三者齐全；扫描到的本体不得少于 20 个，防判据写错导致空集也通过），变异 2 条全红（本轮变异合计 35/35）； ④**口径踩了两次**都写进门禁注释：把 .modal-overlay 遮罩算进来 ⇒ 永远报 31 处；用前缀匹配 nb-dialog ⇒ 把 -overlay/-header/-title/-close 全算成对话框，报出 77 个盒子 / 58 处缺语义。最终口径 = className token 精确匹配 + 遮罩不算 + 语义窗口 ±6/8 行； ⑤**施工 bug 记一笔**：App.tsx 那批属性第一版被重复插了一遍，多出来的那份变成 div 的**文字子节点**，而 tsc **照样通过**（裸文本是合法 JSX 子节点），只有逐行看 diff 才发现 ⇒ 第二版改成整体替换两行 + 自证 role=dialog 恰好 8 次、SlotBridge 行数不变 |
| v1.16.176 | 2026-09-26 | **修 O-29：重载后重复落库（真机实测 5614 条重复文本事件，不是 2 条）** | ①真机读数（改前，装机版 1.16.175，只读探针只查副本库）：逐会话统计 (event_type, payload.messageId, payload.content) 完全相同的行 —— 合计 7951 事件 / 5853 文本事件 / **5614 条重复**（最大那个会话 4368 文本事件里 4319 条是重复；有个会话同 4 条消息各写了 9 次，因为本轮为逐项复核重载过多次）； ②机制：writtenTextEventFingerprints 是模块级内存态，页面每次重载就空了，而重载后 store 会把当前会话消息再 saveMessages 一遍 ⇒ 同一条定稿正文被当成第一次写，**每重载一次再写一遍**； ③修法：判据换成**持久判据** —— 写前回查事件日志是否已有 (type, messageId, 内容指纹) 完全相同的事件；内存表退化为快路径（命中持久判据时顺手填上）；持久索引按会话惰性建一次并随写入增量维护（清理入口只有 __resetTextEventFingerprints 一处）；**镜像没就绪时不许当没有**（readAll 的空数组在未就绪时不代表没有事件）⇒ 延后判定（whenSessionEventsLoaded 就绪或超时后重跑同一套判据），既不重复也不丢；保留 seq !== 0 才记账的既有规则； ④证据：新用例 FC-D0d（__resetTextEventFingerprints 模拟重载后再落库同一份定稿正文 ⇒ 事件表不得新增行；反向对照：正文真被改写必须补一条）+ 变异 O29-停用持久去重 必红（本轮变异 33/33）； ⑤顺带记两个仪器坑：CLI 参数名是 **session_id**（写成 sessionId 会返回「缺少必填参数」，而第一版脚本把失败体 unwrap 成 {} ⇒ 每个会话都「0 条事件」，看起来像库里没有事件 —— **静默空结果**）；PowerShell 直接把 JSON 当参数会被剥引号，必须从 Node spawnSync 传数组 |
| v1.16.175 | 2026-09-26 | **P2-1 后半：加载 / 骨架 / 状态提示三个共享实现 + 转圈只有一个节拍** | ①先量：CSS 里 **10 条转圈规则用了 3 种时长**（0.8s ×6 / 1s ×3 / 1.2s ×1）—— 同一个「正在加载」三种节拍；TSX 里 **4 种加载图标**（Loader2 14 / LoaderCircle 10 / RefreshCw 8 / RotateCw 3）、**3 套类名**（spin 28 / spinner 9 / spinning 2）；**骨架规则 0 条**；role=alert 全仓**只有 4 处**； ②分族口径：**rotate（转圈）与 pulse（呼吸）必须分开收** —— `session-pulse`/`reasoning-pulse`/`connection-dot-pulse` 那些是**注意力/入场动画**不是加载，本轮不动（混在一起收会把入场动画也改掉）；11 条 rotate 规则（含新增 .spinner）全部走 `--spin-duration: 0.8s`； ③新增三个共享实现：**.spinner**（尺寸走 --spinner-size，默认回落 --icon-* 刻度）、**.skeleton**（1.2s 线性 shimmer，多行时最后一行 60% 收尾，减动效下**关动画退化成静态块**）、**.status-banner**（4 档 tone、内边距 12/16、圆角 8、1px 状态边、surface 10% / border 30%）； ④配套组件 src/components/ui/{Spinner,Skeleton,StatusBanner}.tsx 并接上真实消费者：Spinner → 文件浏览器/会话恢复；Skeleton → **文件预览加载态**（原来是「眼睛图标 + 加载中…」，预览出来本就是文本/代码 ⇒ 同形占位更贴内容、数据到达时布局不跳）；StatusBanner → 上下文压缩报错 + 恢复面板动作失败（拿到 role=alert + error 档 assertive）； ⑤门禁 **COND-2**（转圈时长必须全走令牌 / 三个共享类存在且几何走令牌 / 减动效必须显式关停 / 组件必须有真实消费者 / StatusBanner 必须 role=alert 且 error 档 assertive / 裸转圈图标棘轮 24 只许降）+ 行为用例 **CS-1…CS-6**；变异 **32/32 全红**； ⑥门禁自身的两个假绿也记了档：取 prefers-reduced-motion 块体用 `\{([\s\S]*?)\n\}` 会在第一个内层 `}` 处截断（读不到 .spinner 的关停）⇒ 改成大括号配对；StatusBanner 的**文档注释里也写着 role=alert** ⇒ 直接在原文上 test 会永远为真，必须**先剥注释**；另把 RA-1e 的判据从「textContent 出现『加载中』」改成「骨架 + role=status + 可访问名」（意图不变、形态变了） |
| v1.16.174 | 2026-09-26 | **修 1.16.173 的「焦点归还」没生效（自动聚焦的浮层恰恰是没归还的那一类）** | ①装机复核逐条验收时发现：打开搜索对话框后按真实 Esc → 对话框关掉了、但**焦点落在 body**，而「焦点归还」正是上一版新 hook 主打补的能力； ②诊断：触发器一直在文档里（不是被重新挂载）、手动 focus 能留住（没有别的焦点管理在抢）⇒ 问题在 hook —— SearchDialog 组件体里先写了自己的 useEffect(() => input.focus(), []) 再调用 hook，被动 effect 按注册顺序执行，等 hook 记录「打开前的焦点」时 activeElement **已经是浮层里的输入框**，关闭时输入框随浮层移除 ⇒ 记下的元素已脱离文档 ⇒ 被 isConnected 保护拦下 ⇒ 焦点留在 body；**自己做自动聚焦的浮层恰恰是没归还的那一类**； ③修法：把「记住打开前的焦点」从 useEffect 换成 **useLayoutEffect**（布局 effect 早于所有被动 effect，抢得到真正的「打开前」）+ 已有值不覆盖的守卫；另保留「只在没人接管焦点时才归还」（1.16.173 里被 app-menu-bar 既有用例逼出来的规则，否则会顶掉调用方刚设好的焦点）； ④为什么单元用例没抓到：原 ESC-4 用例浮层没有自动聚焦，记录时机对不对看不出来 ⇒ 新增 **ESC-7**，用例里刻意复刻 SearchDialog 的顺序（先自动聚焦再调 hook）；变异 `OVERLAY1-记住焦点退回被动effect`（useLayoutEffect 改回 useEffect）必须红，本轮变异 **28/28 全红** |
| v1.16.173 | 2026-09-26 | **P2-6 浮层收口：9 处手写 Escape 收敛成 useDismissableLayer（并更正方案口径）** | ①口径更正：方案写「手写 Escape 14 处 → ≤2」，实测 **22 处**，但分两族 —— **A. document/window 级的纯 Escape 关闭 9 处**（ConfirmDialog / SearchDialog / ImageGallery / McpMarketplace / Sidebar 会话右键菜单 / FileLinkContextMenu / AppMenuBar 捕获阶段 / SlashCommandMenu / PixelLibraryScene）全部走共享 hook、手写监听归零； **B. 元素级或导航 handler 里的一个分支 13 处**（输入框 Enter/Esc、菜单方向键导航、画布 Esc 取消选择、PPT 演示/搜索框键盘导航、注释与模板串）**不动** —— 那是元素自己的按键语义，搬到 document 级反而错；验收改成可机检的「document 级纯 Escape → 0」； ②迁移后每处自动获得手写版普遍缺的两件事：**只关最上层**（模块级栈 + 守卫；手写 9 处没有一处处理过 —— 侧栏同时开右键菜单与别的浮层时一次 Esc 会关两个）、**关闭后焦点归还**（手写版把焦点留在 body；现在还给打开它的元素，且只在它还连着文档且未 disabled 时还）；另有 inertBackground（打开时 #root 加 inert）与 capture（AppMenuBar 原本就是捕获阶段，迁移没顺手改语义）； ③门禁 **OVERLAY-1a/b/c**（hook 存在+9 个文件采用+「只关最上层」守卫在；document 级 keydown 不许再手写纯 Escape（判据要配对到回调本身、顺回调名找定义、只处理 Escape 才算违规）；Escape 相关行数棘轮）+ 行为用例 **6 条**（含两层叠着只关最上层、焦点归还、inert 加去）；变异 **27/27 全红**； ④**三次假结果都记进门禁注释**：只看注册点后面 12 行 ⇒ 回调定义在前抓不到（假绿）；只断言 /layerStack/ ⇒ 改名 layerStackRemoved 照样过；光断言 \blayerStack\b 不够 ⇒ 最终断言**守卫那一行**；棘轮基线必须用门禁自己那套口径（同时含 Escape 与 key/Key 的行数）22 → **16** |
| v1.16.172 | 2026-09-26 | **P2-1 内容态：13 个「单行空态」收成一个共享类（19 种内边距 → 2 种）** | ①先量再分族：空态 59 条规则里 padding **19 种**、字号 7 种、gap 5 种；按「TSX 里的真实形状」分族（正则抓标签整段看有没有嵌套 className）后发现 **13 个类 / 21 处用法是同一个形状**（一个元素 + 一句提示文本），只有 3 个是块级（图标+标题+说明）⇒ 真问题是「同一个『暂无数据』在 13 个面板里有 13 套内边距」； ②处置：新增唯一实现 **.empty-hint** + 两个修饰（.is-compact 列表/浮层内 8/6+fs-sm+左对齐、.is-boxed 虚线框），几何走新令牌 --empty-hint-pad / --empty-hint-pad-compact；**13 个旧类从样式表删除**、21 处 TSX 改用共享类；设置面板那条 .setting-group > :is(…, .sp-empty, …) 同步改成 .empty-hint（**样式表内部引用最容易漏**，不改就静默失配）； ③视觉影响：13 个类里 11 个现取值即新取值 ⇒ **零变化**；.panel-empty（16→24px、fs-sm→fs-base）与两处紧凑提示（8/4→8/6）差 1–8px，方向是与全体一致；块级那族（.empty-state 是新会话欢迎页 hero，20/14 字号刻意）按 §6.5「已经合格不要动」本轮不碰； ④门禁 **COND-1**（共享类+修饰存在且走令牌 / 13 个旧类不许回来 / 修饰必须有真实消费者 ≥6 与 ≥2 / 棘轮：空态自定 padding 处数只许降）+ 基线口径专门对齐（用门禁自己那套口径在 HEAD 与工作区各数一次 ⇒ **36 → 24**，不能拿「不同取值个数 19→18」当基线）；变异 **24/24 全红**；scan-ui error 0/warn 0（css-class-undefined 也是 0，删类没留下悬空类名） |
| v1.16.171 | 2026-09-26 | **P2-5 长文本：10 条多行截断缺标准属性 + 新增唯一截断工具类与 OverflowText（并更正方案口径）** | ①**口径更正**：方案说「overflow:hidden 246 条里 148 条无截断、约 58 条疑似文本容器」；实测 217 条 overflow:hidden、134 条无截断机制，但绝大多数是**布局裁切**（面板裁圆角/头像/输入框/.sr-only），真正的硬切形态（nowrap+hidden 无 text-overflow）**只有 1 条且就是 .sr-only**（屏读专用类，故意如此）⇒ 按 148→≤20 改会把力气花在正当写法上； ②真问题：**10 条 -webkit-line-clamp 没有标准 line-clamp 双写**（换引擎静默失效，与 SKIN-2「压缩器改写命名色」同类口径漂移）⇒ 全部双写（market-skill-desc / tj-summary / petm-desc / notebook-card-desc / -summary / guidance-item-text / scrollbar-marker-preview-text / quote-context-text / nb-source-preview / nb-note-preview）； ③新增唯一截断工具类 .truncate（=-1 单行三件套）/ .truncate-2 / .truncate-3（多行 clamp 双写）—— 此前 74 条规则各自手抄三件套、无任何通用工具类； ④新增 **OverflowText** 组件：只在**真的被截断**时才挂 title（既有做法是无条件 title，全仓 258 处 ⇒ 没截断也弹提示、且截断是布局结果写死的 title 不跟），已被侧栏会话标题与项目名采用；单行看宽度、多行看高度、文本与尺寸变化都重新丈量； ⑤顺带审计发现：方案里「两皮肤 :active 均为 0 ⇒ 按压反馈失效」**不成立** —— CDP forcePseudoState 真机实测三档按下后都有背景变化（9.8% / 10% / 9.3% 墨色）与 translateY(0.93~0.98px)，基线选择器是级联下来的；判据要看渲染结果、不能数源码规则条数（本项无需修改，记档以免下轮照着方案去改）； ⑥门禁 **TEXT-1**（clamp 必须双写 / 三条工具类必须存在且双写 / nowrap+hidden 必须有省略号（白名单只放 .sr-only）/ 工具类必须有真实消费者）+ OverflowText 行为用例 5 条（在元素原型上造 scrollWidth/clientWidth，jsdom 无排版引擎）；变异 **21/21 全红**；快照 27 处变化逐条过目（全是新增 line-clamp 与工具类，无既有取值漂移） |
| v1.16.170 | 2026-09-26 | **P2-3 皮肤几何收口：34 条「规则体里逐组件覆盖几何」收进令牌块（并更正方案口径）** | ①先量：装机版跨皮肤探针实测同一组件在三档下的计算几何 —— .sidebar-session 圆角 6/4/6、.input-card-container 14/8/0、 .panel-tabs 高度 auto/40/40、.user .message-content 14/12/16，而「为什么不同」要翻两个皮肤文件才能拼出来；皮肤规则体里的共享类几何声明 hub 11 / dream 23 条； ②**口径更正**：方案原文的「geometry 声明 127 → 0」里 41 条属于 .hub-navbar/.hub-agent-card/.dream-polaroid 这类只有该皮肤才有的布局类（主表没有对应规则），删掉皮肤就散架 ⇒ 改成只盯「覆盖共享组件」（判据：选择器里的类名全部在主表出现过），且剩余差异必须可追溯到该皮肤令牌块里的一条覆盖； ③三类处置：纯冗余 8 条直接删声明（气泡 0!important、.user .message-content 同令牌、.input-wrapper 0!important、dream 四个 inline-diff-btn 合成一条、skill-picker 的 padding、四个 margin: 0!important），真分叉 26 条改成引用组件角色令牌并写进皮肤令牌块，皮肤自有布局类 41 条保持原样； ④主表新增 15 条组件角色几何令牌（--radius-input-card/-input-wrapper/-code/-message-user/-message-assistant/-diff-review/-diff-btn/-diff-textarea/-skill-picker、--scrollbar-size/-thumb-radius、--panel-tabs-height/-padding/-gap），取值即主表现有生效值 ⇒ 默认档零变化； ⑤顺带修一处静默覆盖：主表里 ::-webkit-scrollbar 与 ::-webkit-scrollbar-thumb 各有两份定义（后一份赢，前一份的 --radius-xs 一直被盖掉）⇒ 两处都改用同名令牌； ⑥复核抓到一次误删并记档：第一版把 .message-bubble/.message 整条规则删掉，而它还有 background: transparent !important 与 border: none !important ⇒ 顺手删掉了气泡的颜色/边框；改成只删 border-radius 那一行（教训：删声明与删规则是两件事）； ⑦新门禁 **SKIN-3**（皮肤对共享类的几何必须是 var(--token) 且令牌在本皮肤令牌块里定义；不许字面量、不许 !important），变异 **17/17 全红**；CSS 生效取值快照恰好 10 处变化、全部是「值 → 同名令牌」，无取值漂移 |
| v1.16.169 | 2026-09-26 | **P2-2：8 个可点目标低于 24×24（含「第 63 轮补了同排的钉子、漏了删除」）** | ①口径先修正：命中区必须按**有效命中区**量 —— 仓库里有一批行内按钮用 ::after { inset: -4px } 扩圈，只看 getBoundingClientRect() 会误报； 修正后装机实测（侧栏/顶栏/输入区/消息工具条）**60 个可点目标、52 达标、8 不达标**； ②三处处置：.sidebar-session-delete ×4（16×18.7）照抄同一行 .sidebar-session-pin 的扩圈写法（视觉零变化）；.sidebar-project-btn ×3（22×22）盒子提到 24×24（与同族 .sidebar-section-btn 同档）；.send-more-btn（18×32）宽度提到 24（与发送键紧邻、不能用扩圈，只能加宽盒子）； ③两处「漏一族」的历史：第 63 轮给 pin 补扩圈时漏了同一行、同样盒子尺寸（16×18.7）的删除按钮；第 166 轮把 .sidebar-section-btn 从 22 提到 24 时漏了同族的 .sidebar-project-btn ⇒ 结论写进门禁（一族一起改的规则必须绑在同一判据上）； ④新门禁 **A11Y-ICON-7**：这些行内次要动作按钮必须「盒子两轴 ≥24px」**或**「有伪元素负 inset 扩圈」（二者其一 —— 强迫都长到 24 会毁掉行内密度）；变异 **14/14 全红**（新增 A11Y7 两条 + RHYTHM2 四条） |
| v1.16.168 | 2026-09-26 | **运行时审计抓到的真缺陷：列表行被「限高容器」压矮（源码 30px、装机渲染 28px）** | ①现象：侧栏导航行 30px、会话行 **28px**，而源码两侧都写 height: 30px ——「侧栏只有一个行高常量」在会话数 > 3 时根本不成立； ②因果实验（只改一个变量）：现状 容器 144/行 28/无滚动条 → 放开 max-height 得 152/30 → **只加 flex-shrink: 0 得 144/30 且 scrollHeight 152 > clientHeight 144（出现滚动）**； ③机制：Sidebar.tsx 在会话数 > 3 时加内联 maxHeight: 144 + overflowY: auto，而 .sidebar-sessions 是 flex 列 ⇒ 行是默认 flex-shrink: 1 的子项，**内容超限时被压矮而不是产生滚动条**；144 还是按 32px 行距反推的（实际行距 30+2 ⇒ 5 行 152px 塞进 144px）；会话越多压得越狠（10 个会话约每行 12px）； ④修法三步：行高提成令牌 --sidebar-row-h: 30px（此前三处各写一遍）+ 三个行类与同列定高成员补 flex-shrink: 0 + 上限改成 calc((var(--sidebar-row-h) + var(--space-1)) * 4 + var(--sidebar-row-h) / 2)（4 行 + 半行提示 + 间距 = 143px，不再写魔数）； ⑤新门禁 **RHYTHM-2**（令牌只许一处定义 / 行高必须走令牌 / 定高行必须 flex-shrink: 0 / TSX 不许再出现字面 maxHeight: 144），RHYTHM-1 改成先解析令牌再比生效 px；变异 **12/12 全红**（新增 4 条）； ⑥运行时全面审计（装机版 1.16.167 实测）：功能 控制台 0 error/0 warn、交互冒烟全命中、页面自报错误 0；性能 长任务(>50ms) **0 个**、交互布局耗时增量 89.6ms；内存 强制 GC 后 36.5MB、开关设置 10 次 DOM 节点增量 **0**（601→601→601，与 1 次增量相同 ⇒ 无泄漏）、document 监听器 6 个； 后端 bench:mem 2000 条 × 200KB（390.6MB 正文）峰值 **139MB**、未崩溃、5.7s、WAL 0； ⑦新增「被压扁列表」**因果检测器**（逐个打开设置与 4 个工具入口全应用扫，只此一处），并记下检测器第一版**为什么假绿**：拿 getComputedStyle().height（已使用值）当设定值去比渲染高度，两者都是压缩后的值、永远相等 |
| v1.16.167 | 2026-09-26 | **P1：「门禁在说假话」的三个洞 + 字号/图标/装饰色收口（修完门禁浮出 58 处真违规）** | ①**先修工具再修代码**：scan-ui.mjs 报 error 0 / warn 0 的同时，仓库里有 58 处它本该报出来的硬编码 —— (a) 字号/圆角/间距三条规则写成「整行出现过 var() 就整行放过」⇒ 单行规则里的 font-size: 11px 永远看不见； (b) 令牌块豁免写成 if (inRoot) return，而 inRoot 由选择器**前缀** [data-skin 触发 ⇒ **皮肤文件里的普通规则整块被跳过** （两个皮肤的字号字面量在门禁里一直是 0 条，实际有 10 条）； (c) 多行注释只按行剥 ⇒ 令牌区散文里的色值被报成 62 处硬编码（假阳性）⇒ 改成按文件剥、保留换行； (d) border-radius: 0 !important 里的 !important 被当成一个圆角取值（4 处假阳性）； (e) 新增**行内指令豁免**（写在出事那一行、必须带理由），豁免数量随 error/warn 一起打印并进 --json —— 允许写例外，不允许例外悄悄长草（当前 4 处） ②**P1-4 字号令牌化 38 → 0**：--fs-* 全是 calc(Npx * var(--ui-font-scale))，字面 px **不吃缩放系数** ⇒ 「设置→字号」对这批文字完全没反应（这是功能缺口，不是风格问题）；34 处按**逐值相同**映射改令牌， 3 处刻度外按最近档吸附（8/9px→--fs-xs、15px→--fs-lg）并就地在文件里留理由， 4 处皮肤**美术图形**尺寸（40/60/75/80px 的玫瑰/宝丽来/画像）用行内豁免并写清「不是排版字号、不跟缩放」 ③**装饰色 18 → 0**：新增两档各一处的 --shadow-bubble / --reasoning-accent / --separator-fade / --backdrop-blob-1/2/3 / --backdrop-vignette / --ambient-backdrop（后两个是**整条渐变**，含断点位置）， 并**删除 13 条重复规则**（8 条 [data-theme="light"] 覆写 + 5 条皮肤 background 覆写；皮肤改为覆盖同名令牌）；取值逐字符搬迁，零视觉变化 ④**P1-6 图标刻度**：--icon-lg: 20px 此前是**双重死令牌**（.icon-lg 只有组合选择器、没有尺寸声明；TSX 引用 0 次） ⇒ 把 18 处 size={20} 改成语义类 icon-lg（同一设计意图此前有两套来源，现在只剩一处）， 新增门禁 **ICON-2**（8 档不许有死档位：令牌有定义 + 有只含尺寸的独立规则 + 被 TSX 引用），ICON-1 棘轮基线 **918 → 897**，变异 **5/5 全红** ⑤**随手修真缺陷**：LIGHT-UI-4 抓到浅色档暗角用**纯黑 alpha**（与暖墨调色板同屏两种黑点）⇒ 改 rgb(31 31 30 / 8%)； 随「收窄皮肤豁免」浮出 **28 个死令牌**（dream 22 / hub 6，逐个核对无同值孪生）⇒ 删除；顶栏三个窗口按钮补 aria-label ⑥证据：mutate-scanner-scope.mjs **6 项全绿**（第 ① 项**双向对照** —— 同一变异在新口径下红、在老口径 --legacy-var-line-exempt 下确实漏报， 证明修的是「门禁的洞」而不是「顺手多报几条」）；CSS-INTEGRITY-7 生效取值快照 **139 处**变化逐条人工过目； verify exit 0、audit exit 0、scan-ui **error 0 / warn 0** |
| v1.16.166 | 2026-09-26 | **P0：层次 / 状态 / 节奏（前五轮改令牌看不到效果的三件事）+ 一处真缺陷** | ①**内容做成浮起的圆角纸面**：`.main-area` 左两角 **24px** + 左缘阴影 `--shadow-sheet`；`.app` 壳底改 `--chrome-surface`（壳与纸面**必须不同色**，否则圆角切出来还是同色）；侧栏右边界换**墨色 hairline**；拖拽把手 4→6px 且**画出反馈**（`:has()` 描纸面圆角边） ②**修状态层**：删掉 1.16.164 那条「拿侧栏自己的颜色叠自己」的覆盖（实测 hover 背景 **0% 面积变化** + 压过 `.active` ⇒ 选中一碰就没），改 `--row-hover`(墨 6%)/`--row-pressed`(墨 10%)、hover 带 `:not(.active)`、补按下态、行默认改次级文字色、过渡换 `--transition-color` ③**统一行节奏**：行高 40.33/31.5/38 → **30px**、分组标题 22→24px、行内 `0 var(--space-2)`、容器 `var(--space-1) var(--space-3)`、行间距 `var(--space-1)`、圆角统一 `--radius-sm`、工具簇竖排 59px → 行内 chip 30px ④**修 `styles.css:762` 悬空逗号**：浮层被并进模态组、`transform-origin` 全是 center（与 D-4 意图相反）⇒ 拆分 + 输入区三个浮层设 `--popover-origin` ⑤门禁 **SHELL-1**（新增）、**RHYTHM-1**（新增，断言生效 px 值以免与 `spacing-raw` 打架）、**NATIVE-2**（改判据：不再钉那条坏覆盖） + 变异 **7/7 全红**；顺带修死规则 `.sidebar-session-icon`、未消费令牌 `--row-hover-strong`、分组按钮 22→24px ⑥实测 `verify` exit 0（403 文件 / 6315 通过）、`audit` exit 0、`scan-ui` **error 0 / warn 0** |
| v1.16.165 | 2026-09-26 | **P2-1 的另一半：高对比档此前只把玻璃关掉（文字/边框/焦点环一个像素都没变）+ 补齐 P1-3/D3/D4 的变异自证** | ①缺口：`data-contrast="high"` 的 6 处规则**全是**玻璃降级，没有任何令牌级覆盖块；而唯一盯着它的 APPEARANCE-5 只断言 `[data-contrast="high"] .sidebar` 存在 ⇒ 半成品全绿 ②补法：一套值同时管两档（文字三档是「墨→纸」混合，把 N 调大在亮色档是更黑、暗色档是更亮）—— 实测 `--text-secondary` 7.37→**12.30:1**（暗 8.93→10.55）、`--text-muted` 5.28→**8.75:1**（暗 6.25→8.53）、`--border-primary` 可见度 1.19→**1.73:1**（暗 1.47→1.90）、焦点环 2px→**3px** 且改实色；不动 `--text-base`（D1 用户说别动） ③为量「边框更实」，给 `contrast-checker.ts` **新增** α 感知解析（`resolveRgba`/`compositeOver`/`visibleContrastOver`）—— 老的 `parseRgba` 只认逗号写法，`rgb(31 31 30 / 9%)` 直接返回 null（**半透明边框的可见度此前从没人量过**）；新增门禁 **ALPHA-1…4**，ALPHA-1 用规范参照值钉住「color-mix 必须预乘 α」（本仓库踩过三次） ④覆盖盘点：P1-3/D3/D4 的三条阴影断言**从来没有变异自证** ⇒ 新增 `mutate-shadow-gates.mjs`（S1–S5）+ 外观组 A7–A11（含「把检查器改成不预乘 α」），全部被抓且还原回绿；O-30 的变异自证合计 62 条（玻璃 15 / 样式 23 / 外观 12 / 颜色角色 7 / 阴影 5） ⑤**如实标注**：P1-1 的「结构翻转（灰工作区+白卡）」**不做** —— 实测「灰面占屏比」我们 32.8% / 参考实现 5.5%，整片改灰方向相反，故按清单原文收口并写清理由 ⑥实测 `verify` exit 0（403 文件 / 6313 通过）、`audit` exit 0 |
| v1.16.164 | 2026-09-26 | **产物口径对齐：压缩器把命名色 `white` 改写成 `#fff`，SKIN-1 看不见（新增 SKIN-2）** | ①由来：1.16.163 装机版复核（CDP 读运行中 CSSOM + 拉服务端 CSS 文本）实测产物里 Dream 皮肤规则体仍有 **2 处** `#fff`（`.btn-send/.send-button`、`.inline-diff-btn.accept`），而源码写的是 `color: white` —— **lightningcss 会把命名色改写成 hex**，SKIN-1 的口径（hex/rgb/rgba）两种都不匹配 ⇒ 源码全绿、产物漏了 ②处置：改用同值令牌 `var(--dream-c7)`（=#ffffff，零视觉变化）+新增门禁 **SKIN-2**（规则体禁命名色，内置 CSS Color 4 全表 148 个；豁免 `transparent`/`currentColor` —— 压缩器对它们逐字保留），变异 `SKIN2-命名色 white` 红；并把 SKIN-1 的「注释/`var()`/令牌块」区间抽成两个门禁**共用的一个函数** ③产物实测：`dist/assets/main-Cucc8UK7.css` 里字面量 hub 0/dream 0、命名色 hub 0/dream 0 ⇒ 两个口径都是 0 ④如实记写复核脚本踩的 6 个坑（`CSSStyleRule` 也有 `cssRules` ⇒ 扫描静默全 0；产物选择器去引号；压缩器规范化取值 ⇒ 逐字比对假红；自写宽松正则把 `transparent` 算成字面量 ⇒ 29 处假红；`--hub-c` 前缀多算 `--hub-color-*` ⇒ 66≠63；选择器里的类名 `.hub-task-icon-blue` 被当成命名色） ⑤实测 `verify` exit 0（402 文件 / 6308 通过）、`audit` exit 0、23 条变异全红且还原回绿 |
| v1.16.163 | 2026-09-26 | **P2-3 皮肤数据化：两套皮肤规则体里的颜色字面量归零（零视觉变化有机证）** | ①先量：`skin-hub.css` 规则体 **89 处**裸颜色（68 种）、`skin-dream.css` **112 处**（60 种）⇒ 收进各自文件顶部的**调色板令牌块**：Hub 新增 63 个 `--hub-cN`、Dream 新增 81 个 `--dream-cN`，替换 91 + 112 处；与既有令牌等值的 38 处直接复用既有令牌 ②三条口径：值逐字相同 / 既有等值令牌优先 / **不动 `var(--x, #fff)` 兜底**；命名是值优先机械编号，语义命名留待人确认 ③**零视觉变化是机证的**：CSS-INTEGRITY-7 比对 2733 个类的生效取值快照，**不加 --write 直接通过**（`css-contract --check` 输出 ✅无变化）；新门禁 **SKIN-1**（规则体不许有颜色字面量）+ 变异 SKIN1 红；棘轮随之收紧：`color` **140 → 17**、`box-shadow` **29 → 8** ④如实记两个坑：迁移脚本第一版会删光注释（改用原文替换 + 注释区间跳过）；SKIN-1 假红 4 处（算偏移时按「每行 + 1」累加，而文件是 CRLF ⇒ 偏移漂移） ⑤实测 `verify` exit 0（402 文件 / 6307 通过）、`audit` exit 0 |
| v1.16.162 | 2026-09-26 | **P1-1 表面角色（卡片面解耦）+ D8 身份色（执行模式徽标）** | ①新增 `--surface-raised: var(--bg-primary)`（**零视觉变化**），把「卡片面」从「画布面」解耦，接上 4 处语义上真的是卡片的规则（市场技能卡 / 多模态内嵌面板 / diff 面板 / 性能面板）；将来翻「灰工作区 + 白卡」只需改一行。**不新增 `--surface-subtle`**：已有 `--surface-1/2` 两档局部浅面（引用 19 次），再加同义名就是两套阶梯并存 ②D8 只做**有真实消费方**的身份色：没有头像元素、工具卡本就该用状态色 ⇒ 不做 assistant/user/tool；真正缺的是执行模式徽标（`.execution-mode-toggle` 的 `.active` 此前无任何视觉区分）⇒ `--identity-mode-workspace/-worktree`（亮 #7c3aed / 暗提亮）+ 14% 同色浅底，命名刻意不含状态词 ③新增 3 条用例（LIT-12、IDENTITY-1/2）；变异 **21/21 红**；顺带修掉两处**变异脚本自己**的问题（判据缩进、挂在错的套件上）④实测 `verify` exit 0（402 文件 / 6306 通过）、`audit` exit 0 |
| v1.16.161 | 2026-09-26 | **颜色用量注册表（audit 第 20 道）：把 157 轮人工数出来的那类问题变成机器判据，当场抓出 9 处真实违规** | ①新门禁 `tools/audit/scan-color-roles.mjs`：三层判据 —— **登记制**（文字角色 × 表面必须是注册表里的组合）、**实算对比度**（按两档主题解析 var()/color-mix() 后算，低于角色下限就红）、**预算棘轮**（解析不了的记 unresolved 只许降）；角色下限只允许 WCAG 档位 3/4.5/6/7（用例钉住，不许拍数） ②**第一次跑抓到 9 处真实违规**：白字压暗色状态色块 **2.54/2.52**（success/warning，8 处）⇒ 新增 `--text-on-status`（暗色=深墨水，四色 5.5–8.3）并**按规则**迁移 22 处；`--error` 文字压悬停面 **4.14** ⇒ 用 `--error-content`（暗色提到 72% 混白）；`--accent` 文字压内嵌块/悬停面 **3.92/3.48** ⇒ 用 `--accent-strong`；`.quote-context-banner` **5.99**（差 0.01 没到 6）⇒ 换正文色；`.sp-toggle` 白字压浅灰 **1.16** ⇒ 删掉那句**死声明**（没人用 currentColor） ③**门禁自己踩的三个坑**（都进了变异自证）：亮色档整档没被检查（选择器里的空格 vs 文件里的换行 ⇒ 只在暗色档上跑还报通过，变异 C5）、报出的行号错位几百行（去注释把偏移吃掉，CR-7/C6）、半透明表面漏了预乘还原（`color-mix(色 20%, transparent)` 被读成 rgb(41,7,9)，C4）——预乘这个坑在本仓库已是**第三次** ④新增 7 条用例 + **变异 6/6**；`audit` 现为 **20 道**；实测 `verify` exit 0（402 文件 / 6302 通过）、`audit` exit 0 ⑤**如实标注**：暗色白字压品牌紫 **3.99** 按 AA-large 的 3 登记（要过 4.5 得改紫或改深色字，属产品决策） |
| v1.16.160 | 2026-09-26 | **外观档位的启动路径补上（1.16.159 的模块在产品代码里没人调）** | 复核 1.16.159 装机版实测 `data-contrast`/`data-density` 都是 `null` ⇒ 查出 `applyAppearanceAttributes()`（DB→镜像→默认 那条链）**产品代码里没人调**、只有测试在调；后果是「改了档但镜像丢了」（清缓存/换机恢复设置）会一直停在默认档，直到再碰一次设置页。修法：`main.tsx` 首帧前按镜像应用一次 + `TitleBar`（DB 就绪后）用 `getSetting` 校正；新增 **APPEARANCE-6** 钉住两处都在（DB 那次必须传 `getSetting`，否则读的还是镜像）；变异扩到 **7/7 红**。实测 `verify` exit 0（401 文件 / 6295 通过）、`audit` exit 0 |
| v1.16.159 | 2026-09-26 | **高对比/密度档真的接上了设置页（此前是「规则写了没人触发」）+ 文字与品牌浅底解耦** | ①**自查发现一条死规则**：`[data-contrast="high"]` 的 CSS 从 1.16.156 就有，但**全项目没有任何地方设置这个属性** ⇒ 用户永远看不到、CSS 侧门禁还全绿（它只问「规则在不在」）；与 `--message-bubble-user` 那次「定义了零引用」同类 ②**P2-1 补上写入方**：新增 `src/core/theme/appearance-modes.ts`（唯一真相源，DB→镜像→默认；非法值退回默认；写属性幂等；读设置抛错不崩）+ `index.html` **首帧前**读镜像（不闪）+ 设置页外观页签两个下拉（对比度、界面密度）+ `[data-density="compact"]` **只覆盖 `--control-*` 五档**（不缩 `--space-*`：间距承载分组意图）且每档 ≥**24px 命中区**；组件 CSS 一行没改 ③**P1-2 解耦**：文字三档派生自 `--text-base` + `--text-ramp-paper`（浅 75/65% 纸用**暖纸** `#fffbe2`、暗 82/67% 纸是画布、hub 59/43%）——实测与手工值：浅色差 **0 / 3**、暗色 **≤1**、hub **0**（观感不变）；`--accent-muted` 两档改为派生自 `var(--accent)`（原来是写死 rgba，浅色档那个字面量甚至不是自己的 accent）⇒ 皮肤换品牌色时浅底自动跟随 ④**顺带补工具能力**：`contrast-checker.ts` 现在能解析 `var()`/`color-mix()`（令牌一解耦原检查器就「瞎了」，skin-contrast 当场红 —— 那是检查器缺能力，不是测试要改） ⑤新增 8 条用例（APPEARANCE-1…5、LIT-10/11）、变异 **6/6 + 16/16** ⑥**仪器教训**：完整 verify 报 `issue.ts functions 42.1% < 地板 46%`，而该文件这一轮一行没动 —— 重跑即过；函数级覆盖会随 worker 分片抖动，遇红先重跑 ⑦实测 `verify` exit 0（**401 文件 / 6294 通过**）、`audit` exit 0 |
| v1.16.158 | 2026-09-26 | **查清对标项目「看上去不是实色」的机制：系统窗口材质（Mica/Acrylic）—— 我们早就有一半，另一半（前端让出底色）从来没做** | 用户追问"对标项目是怎么做的呢？学习借鉴一下"①**对方机制（源码取证，三件配套缺一不可）**：`appearance.rs` 里 `cfg!(any(windows,macos))` + 窗口 `.transparent(true)` + `Effect::Acrylic`（macOS vibrancy）；启动注入 `setAttribute('data-openbitfun-native-material','sidebar')` 且 `root/body.backgroundColor='transparent'`；CSS 在这一档把 `backdrop-filter` 关掉（注释原文：The OS blurs desktop pixels; a CSS backdrop only sees the webview）；控件用 `sidebar-overlay($surface, 18%)` **给材质上色** ②**我们的差距**：Rust 侧**早就 apply 了**（`apply_mica`→退 `apply_acrylic`、macOS `apply_vibrancy`、窗口已 transparent），但前端一路不透明底 + 自己的场景层 ⇒ **材质被网页整块盖住、等于白开**（两次"看上去还是实色"的根因，也说明 1.16.157 的场景层只解决了一半）③**落地**：新增 Rust `native_material` 命令（记下 mica/acrylic/vibrancy 是否成功 —— 前端猜不出来；失败时让出底色会得到"没有材质的透明窗口"）；`main.tsx` 在**首次渲染前**（`bootstrap()` 里 await + 400ms 超时兜底）打 `data-native-material`；材质档**只让外壳透明**（html/body/.app/标题栏/侧栏），**内容面照旧不透明** ⇒ 壁纸透不进正文；`.sidebar`/`.titlebar` 的 CSS 模糊全关；控件 hover 改 18% overlay；三条降级同样覆盖；α 单独一档 `--surface-glass-chrome-native`=**88%**（对方 90%；背后是无界壁纸⇒保守）④**装机版实测**：`kind=mica`、`.app` 透明、侧栏 88% + `backdrop-filter:none`、主内容面仍 `rgb(255,255,255)`；A/B（摘属性再截）标题栏 Δ**16.6**、侧栏 Δ7.1、主内容面 Δ1.4≈0 ⑤**反直觉实测**：同一套像素统计量对方官方截图 —— 对方侧栏 rgb(242,242,243) vs 内容 rgb(249,249,249)（差 **7**），我们 rgb(242,241,243) vs rgb(246,246,246)（差 **4**）⇒ **对方也不是"看穿"，只比内容暗 7 个色阶**；如实标注仍不同处：对方侧栏有色像素 0.0%（中性选中态）、我们 20.7%（品牌紫，属我们自己的设计选择）⑥顺带修掉构建问题：main.tsx 顶层 await 被构建目标拒绝，改 `bootstrap()` 并加门禁禁止回归 ⑦新增 NATIVE-1/2/3 用例、**变异 15/15**；`verify` exit 0（400 文件 / 6287 通过）、`audit` exit 0 |
| v1.16.157 | 2026-09-26 | **玻璃终于看得见：真因是「背后没有东西可透」；顺带阴影阶梯统一 + 状态色四角色** | 用户反馈"侧栏还是没有玻璃材质的效果、还是比较深的灰色"⇒ 装机版诊断（`_diag-sidebar-glass.mjs` 读运行中页面的父链与计算样式）：侧栏是 `.app > .app-content > .sidebar`（flex 一列），**背后只有 `.app` 的纯色底** ⇒ 90% 玻璃压纯色，模糊前后逐位相同 = 实色；"深灰"同理（内容面纯白 + 侧栏 #f8f8f7 ⇒ 侧栏是唯一偏灰那块，而参考实现的工作区更灰、外壳最亮）①**落地**：外壳铺**有界场景** `--scene-layer`（品牌/信息两层薄雾 + 168° 渐变底，`.app` 当 background-image）、玻璃 α 90%→**62%**（可见度 =(1−α)×着色强度，是一对参数）、浅色外壳底 #f8f8f7→**#fbfbfa** ②**判据换成它本来想守的东西**：原来是"α ≥ 90%"（抄来的代理指标，反过来挡住正确做法），现在直接算「最坏场景像素+玻璃」合成上三档文字 ≥4.5/≥6/≥10，α 只留 ≥0.6 硬下限，并要求 `--scene-layer` **引用** `--scene-veil/-alt`（声明=画出来的）③**客观读数**（同会话 A/B 截图像素统计）：侧栏上段 浅 rgb(243,242,244)→**rgb(229,226,246)** Δ21.5、暗 rgb(25,27,29)→**rgb(42,41,67)** Δ43.9，下段几乎不变 ⇒ 顶部有渐隐薄雾 ④**P1-3/D4 阴影**：一条中性阶梯 `--shadow-raise-1…4`，`--shadow-md/popover/lg` 变**别名**、`--shadow-sm` 并入第 1 档；品牌色从高度里拿掉（`--shadow-lg` 原为品牌紫、且**零消费方**）只留 `--shadow-glow`（消费点：发送按钮悬停、选中项目 chip）；顺带查出既有断言读的是**文件内夹具**而非 styles.css（改源码不会红）已改成读真文件；皮肤 skin-hub 改为覆盖阶梯本身 ⑤**D3 暗色阴影**：显式阶梯，浮层档 0.30/0.40 → **0.42/0.55**，门禁 DARK-UI-8（单调 + 浮层峰值 ≥0.5）⑥**P1-4 状态色四角色**：surface 10% / surface-strong 20% / border 30% / content（浅色压深 86%）——**75 处等值替换 + 7 处修可读性**；实测缺口：57 处"状态色文字压同色浅底"，浅色档 error 在 20% 上仅 **3.87:1**（10–12px 小标签）⇒ -content 让 10/15/18/20% 四档全过 4.5；新棘轮族 `status-tint`（剩 79）⑦新增 9 条用例、**变异 12/12 + 13/13**；`verify` **exit 0**（400 文件 / 6284 通过）、`audit` exit 0 ⑧**仍未做**：P1-1 表面角色（灰工作区+白卡，需用户定）、P1-2 文字单一来源化、P2-1 高对比/密度档**接进设置页**（现在没有写入方）、P2-2/D8 身份色、P2-3 皮肤数据化、P2-4 颜色注册表 | 用户问"对方的背景（尤其左侧栏和弹出菜单）是不是有透明过渡或者渐变"⇒ **源码取证回答：是玻璃材质、不是渐变**（侧栏 `color-mix(chrome 90%, transparent)` + `blur(12px) saturate(1.2)` + 顶边内高光，浮层 `raised 94%` + 同一档模糊；全仓 `linear-gradient` 只在卡片装饰/场景/thinking 上，侧栏与菜单一条都没有）①**我们的实情**：只有 `.popover-shell` 写了 `blur(12px)` 而底色 **98% 不透明**（模糊等于没生效），侧栏纯实色，`.model-picker` 用实色 `--bg-secondary` ②现在侧栏 `@supports` 90% 玻璃 + `--blur-medium` + 内高光（浅色侧栏底 `#f4f4f2`→`#f8f8f7`、内嵌块 `--bg-tertiary` `#f2f2f0`→`#f4f4f2`），浮层四处基础规则即 94% 玻璃 + 令牌模糊，**5 条规则 9 处**裸 `blur()` 全令牌化并删掉零消费方的 `--blur-base` ③三条降级（`prefers-reduced-transparency` / `prefers-contrast: more` / `[data-contrast="high"]`）+ 不透明度下限 90%/94%，门禁 **LIGHT-UI-10 / DARK-UI-6**（判据在**去注释后**的源码里找 —— 只在原文 grep 会被自己的注释骗过，这条是变异 M5 逼出来的；D7 高对比档同时落地）④**按下态**：`.press-layer-host:active` 原来用的是悬停档底色（按下去和悬停一样）⇒ 新增 `--surface-pressed`（浅 10% 黑 / 暗 **14%** 白）；暗色那个数是算出来的：按下是半透明的、必须先合成到各面上再比，**最小成立 α = 13%**（10% 时在画布上比悬停还暗 ⇒ 方向反了），门禁 LIGHT-UI-11 / DARK-UI-7 ⑤**禁用态**：0.3/0.4/0.45/0.5/0.55/0.6 **六个数**、35 处 ⇒ `var(--opacity-disabled)` = 0.5，新增令牌卫生 **H5**（禁用族写裸数值就红、`:hover:not(:disabled)` 不误伤、还守"用了却没人定义"）⑥**D5 品牌色阶梯**：**64 处**手写 `color-mix(… var(--accent) N%, transparent)`、**20 种百分比** ⇒ 四档令牌（8/15/30/45%，全派生自 `var(--accent)`）+ **28 处等值迁移**（零视觉变化）+ 新棘轮族 `accent-tint`（剩 43 处只许降）⑦**口径修正**：棘轮曾把 `box-shadow: none` 这种**取消**也算"写死"，玻璃降级块这个正确做法反把棘轮推高 2 处 ⇒ `none/inherit/initial/unset/revert` 计入 `keyword`（可见、不进 raw；`bold` 仍算写死），按新口径**收紧** 5 个族基线（box-shadow 31→29 等）
| v1.16.156 | 2026-09-26 | **玻璃材质：侧栏与弹出菜单改成「半透明 + 背景模糊」；按下态/禁用态/品牌色浅底三处收敛** | ⑧新增 11 条契约、**变异 9/9 + 10/10**；实测 `verify` **exit 0**（400 文件 / 6278 通过）、`audit` 全绿；**仍未做**：P1-2 文字/`--accent-muted` 解耦、P1-3 阴影阶梯统一（`--shadow-lg` 品牌紫被既有用例钉住）、P2-1 密度档、P2-2/D8 身份色、P2-3 皮肤数据化（hub 51 / dream 72）、P2-4 颜色注册表、D1 暗色主文字（用户说好看⇒不动）、D3 暗色阴影强度（需人眼确认） |
