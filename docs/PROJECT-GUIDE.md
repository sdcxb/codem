...
| 版本 | 日期 | 主要内容 |
|------|------|---------|
| v1.16.116 | 2026-09-20 | **细节精度：状态表达去品牌色 + 线条/圆角/阴影归一（深度对标参考实现 styles.css 23249 行）** — 用户二次反馈"过度不精致"并点名气泡边框/内容线条粗细/侧栏紫框与图标对比度。承认上轮**对标深度不够**（只读了参考的 58 行令牌、没读它 23249 行组件 CSS）⇒ 先建两套仪器：`bench-css-detail.mjs`（同一提取器量两边：边框/圆角/阴影/字体/状态表达/动效/密度 + 抓参考组件原文）、`audit-visual-detail-01.mjs`（真机台账：**线条配对双线检测**/侧栏逐元素/紫框识别/图标族与对比度/**计算字重分布**）。🔴 **根因一 品牌色进了状态表达**：选中态品牌填充 **69→11**（参考 1）、中性填充 **11→71**（参考 85）、品牌边框 **44→3**（参考 2）、hover 品牌色 **116→48**（参考 3）；参考原文是 `.rail-action:hover,.rail-action.active{background:rgb(255 255 255/52%);color:#102d28}` + `.rail-item:hover,.rail-item.active{border-color:transparent}` ⇒ 新增 `--state-hover/selected-bg/border` 令牌，三轮迁移共改 **91 条规则**（品牌色只留主操作/进度/开关/危险/焦点环/页签下划线）。⚠️ **仪器 bug**：keep 清单 `/bar-/` 匹配上 `sideBAR-…` 导致**整个左侧栏被前两轮整片跳过**，第三轮真机复量仍量到紫底才发现，教训写进门禁注释。🔴 **根因二 平行双线 21 对**：其中 20 对是用户气泡品牌紫 16% 底边与 `collapse-btn` 底边**相距 0px**、与 `toolbar-btn` 上边相距 1px ⇒ 气泡改**中性浅底+无边框+无投影+圆角 10px**、工具条去边框 ⇒ 复量 **21→1 对**。🔴 **根因三 非 1px 边框 51 处**（真机 8 段全在 32px 头像上，行内样式 CSS grep 找不到）⇒ 全部归 1px（环/挖空例外）⇒ **8→0 段**，线条段数 **654→407**。🔴 **根因四 图标对比度** 3.94（弱图标落紫选中底）⇒ 中性化 + 头像图标改 `--text-secondary` ⇒ **3.94→4.77**。🟡 顺带：圆角兜底写法 **47 处**归一、阴影 **27 处**收敛三档并删 3 处重投影、再删 12 行死令牌。🟡 **两处如实修正**：缓动不是问题（四档令牌齐全，此前统计到的"5 种 cubic-bezier"是令牌定义本身）、字重不是问题（计算字重 400 占 76%）。**门禁** LIGHT-UI 12 例（新增状态中性化与边框粗细两条）。**实测**：`tsc` 0；351 文件 / 5934 通过 / 0 失败；10 道门禁 exit 0；UI 门禁 error 0 / warn 0。⚠️ 无目视核对。 |
