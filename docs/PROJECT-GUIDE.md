...
- **平台**：Windows 优先
- **版本**：v1.9.5（对话步骤语义化与动态插入 + token 消耗审计修复 + 全面功能审计修复，2026-09-02）
