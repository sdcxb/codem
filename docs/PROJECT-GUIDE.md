...
| 版本 | 日期 | 主要内容 |
|------|------|---------|
| v1.16.8 | 2026-09-11 | **修复：打开设置后全部文字变大、关了也不回退（第 56 波）** — ①现象：点开「设置」后主页（左侧面板、会话标题等）文字突然放大，关掉设置仍保持放大，重启才恢复。②**根因**：界面字号存**两个键**且各带一个**不同默认值** —— 启动路径（`Sidebar` 挂载）读旧扁平键 `codem-font-size`（**只由字号滑杆写入**，没拖过就是空 → 回落基准 13px），设置页打开时应用的却是 `codem-settings.fontSize`（默认 **14**）→ 全站瞬间 +7.7%；变量写在 `<html>` 行内样式上，关设置不复原。③**实测**（无头浏览器 + 真实 CSS）：会话标题 13→14.0px、项目名 14→15.08px、按钮 12→12.92px，整体 +7.7%。④**修复**：`core/ui-font.ts` 收敛为单一解析器（旧键=显式选择 → 设置对象值 → 基准 13），应用点统一为「DB 就绪后（App.tsx）+ 侧栏挂载 + 设置页打开/拖动」；设置页默认值由 14 改为 `FONT_BASE_PX`。⑤**兼容**：旧代码保存任何设置都会把默认 14 写进设置对象 → 判定"存在旧键（真拖过滑杆）时 14 才算用户选择，否则视为未设置归一为 13"，避免这批用户被永久放大；滑杆显示值同步归一。⑥同类排查：`--font-weight`/`--font-family` 读写同键，无此问题。⑦新增 6 条契约测试（默认=基准、优先级、"14 无记录=未设置"、越界钳制、启动必须在 initDatabase 之后、设置页不得直接应用 parsed.fontSize）。校验：tsc 0 错误 / 206 文件 4609 用例通过 / 审计 24 条规则 0/0 / 打包成功。 |
