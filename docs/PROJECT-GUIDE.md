...
| Unreleased（EAC 对标） | — | **UI 交互改进 + 3 个新插件（对标 DSH-Desktop-EAC）** — ①编辑并回退（fork 保留原会话）②节点导航升级（滚动容器修复 + hover 预览 + 滚轮 + 📌 精选 pin）③输入框失焦折叠 ④字号生效 ⑤设置搜索 ⑥persona 人设卡（# Persona 注入 + 热重载，修 SOUL 孤儿）⑦side-session 临时会话（不污染主会话）⑧**@codem/agent-teams 团队编排**（依赖任务 DAG/attempt 防覆盖/成员邮箱/共享调度/活动面板/10 工具）。全量 163 文件 / 4202 用例通过 + tsc 零错误 |
