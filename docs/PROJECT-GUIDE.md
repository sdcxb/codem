...
- **平台**：Windows 优先
- **版本**：v1.5.2（大文件性能修复 + Agent Loop 无上限改造 + 模型系统动态化 + Skills 增量搜索，2026-08-24）
