...
- **平台**：Windows 优先
- **版本**：v1.5.0（Cordis "一切插件化" 工具发现机制 — ToolDef guidance + systemPrompt 动态收集，2026-08-21）
