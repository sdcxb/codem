...
- **平台**：Windows 优先
- **版本**：v1.9.2（LLM 请求级超时加固 + 安全模式按钮颜色反馈 + 引导消息注入体验改造 + LLM 失败可见性，2026-09-01）
