...
6. **大富翁「开始游戏」之后永远进不去游戏。** 选完角色后 effect **先清标记、下一帧**才 `initGame`；
...
7. **`ctx.permissions` 是个说谎的 API**：`check` / `setMode` / `getActiveMode` 转发的三个方法在真实
...
8. **`uiPermissionPresets` 是一套平行且不生效的权限系统**：它的 `shouldAutoApprove` / `isPathBlocked` /
...
9. **编辑模型档案的槽位会写到"激活的"档案上**：激活内置档案时静默无效，激活另一个自建档案时
...
| 项 | 修复前 | 修复后 |
| --- | --- | --- |
| 渲染侧测试 | 5300 条 / 275 文件 | **296 文件 / 5497 通过 / 0 失败**（+197 条） |
