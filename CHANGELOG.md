...
## [1.16.116] - 2026-09-20 — 细节精度：状态表达去品牌色 + 线条/圆角/阴影归一（深度对标参考实现）

> 起因：用户第二次反馈亮色"还是很粗糙、过度不精致"，并点名三处 ——
> **对话气泡的边框线条**、**回答内容间的线条粗细和颜色**、**左侧栏里紫色的框与图标对比度**。
> 第 65 轮动的是结构（面阶梯/线条强度/调色板），这一轮承认**对标深度不够**：
> 上一轮只读了参考项目的 `ui-tokens.css`（58 行令牌），**没读它 23249 行的 `styles.css`** ——
> 细节表现全在组件规则里。于是先建两套新仪器，再做修复。

### 🔬 新仪器（同一套提取器量两边）

1. **`.preview-shot/bench-css-detail.mjs`** —— 对**两边**的样式表跑同一套提取器：
   边框语言（宽度×颜色）、圆角语言、阴影语言、字号/字重/字距/行高、**状态表达**
   （选中态用中性还是品牌色）、动效、密度（高度/内边距），并抓参考实现关键组件的规则原文。
2. **`.preview-shot/audit-visual-detail-01.mjs`** —— 装机版真机台账：
   **线条配对（双线检测）**、侧栏逐元素台账、"紫色的框"识别、图标族/尺寸/stroke-width/对比度、
   **计算字重分布**。
...
| 选中态用**品牌色**填充 | **69** | **1** |
| 选中态用中性色填充 | 11 | **85** |
| 选中态用**品牌色边框** | **44**（按"任意边框含 accent"算 **219**） | **2** |
| hover 里用品牌色 | **116** | **3** |

参考实现的原文写得很直白：
```
.rail-action { height: 34px; border: 0; border-radius: 8px; padding: 0 9px; }
.rail-action:hover, .rail-action.active { background: rgb(255 255 255 / 52%); color: #102d28; }
.rail-item:hover, .rail-item.active { border-color: transparent; background: rgb(255 255 255 / 35%); }
```
即**选中/hover 只有中性填充、边框显式 transparent、品牌色不参与状态表达**。
我们侧栏则是：`.sidebar-tool-row` **常驻 8% 紫底** + 内凹阴影、`.sidebar-tool-item.active` 18% 紫底、
`.sidebar-project.active`/`.sidebar-session.active` 15% 紫底，四个 22×22 图标按钮 hover 时
**整块变紫 + 紫边框 + 白字**。

- **修法**：新增状态令牌 `--state-hover-bg / --state-hover-border / --state-selected-bg / --state-selected-border`
  （亮色 5%/9% 中性、暗色 7%/11% 白 alpha，与参考的 5%/9% **同档 alpha**；方向相反是因为我们的轨道本身已是
  浅灰、再"提亮"没有空间，故改为略微加深 —— 这条**有意不同**写进了令牌注释）。
  三轮迁移共改 **10 + 13 + 68 = 91 条规则**（脚本 `.preview-shot/neutralize-states*.mjs`，带干跑与逐条 diff）。
  品牌色只留给**语义正确**处：主操作按钮、进度/图表填充、开关、危险色、焦点环、页签下划线、徽标。
- **⚠️ 一个必须记下来的仪器 bug**：keep 清单里的 `/bar-/`（本意放行 `.progress-bar-*`）**匹配上了 `sideBAR-…`**
  —— 于是**整个左侧栏被前两轮迁移整片跳过**。是第三轮跑完真机复量仍量到紫底才发现的；
  该教训同时写进了门禁注释（`/bar-/`、`/-bar\b/` 明令禁止）。

### 🔴 根因二（"气泡边框线条"）：**平行双线 21 对**

真机台账（1.16.115）量到 21 对"相距 ≤6px 且重叠 >24px"的平行线，其中 **20 对**由用户气泡引起：

| 双线 A | 双线 B | 间距 |
| --- | --- | --- |
| `div.message-content` 底边（**品牌紫 16%**） | `span.collapse-btn` 底边（中性 9%） | **0px**（两条不同色的线叠在一起） |
| 同上 | `button.toolbar-btn` 上边（中性 9%） | **1px**（×9 处） |

- 修法：气泡改为 **中性浅底 + 无边框 + 无投影 + 圆角 10px**；悬浮的 `.message-actions-bar`（工具条）
  **去掉边框**，靠白底 + 轻投影分层（与参考"用极轻投影替代边框"同一手法）。
- 复量：**21 对 → 1 对**（剩下那对是链接下划线与展开按钮上边相距 0.47px 的巧合，已记档不追）。

### 🔴 根因三（"线条粗细"）：非 1px 边框 51 处

真机台账里 **全部 8 段非 1px 的线都在同一个 32px 头像上**（`border: "2px solid …"` 写在
`MessageBubble.tsx` 的行内样式里，CSS grep 找不到它）；静态普查另有 51 处
（`2px` 27 / `3px` 15 / `1.5px` 3 / `6px` 1）。

- 修法：`2px/1.5px + 中性色` → **1px**；状态规则里 `2px/3px + 品牌色` → **1px 中性**；
  `border-left: 3px + 状态色` → **2px**（状态条保留语义色，只是不再是最粗的线）；头像行内样式 2px → 1px。
  例外只留"**环/挖空**"这类必须是 2px 的（spinner 转圈环、徽标用底色挖空）。
- 复量：**非 1px 的线 8 段 → 0 段**；线条总段数 **654 → 407**。

### 🔴 根因四：图标对比度

侧栏 12px 弱图标（`--text-muted`）落在**品牌紫选中底**（`#dfddf0`）上只有 **3.94:1**；
头像里的用户图标用 `--text-muted` 也只有 4.68。

- 修法：选中底变中性后不再压暗图标；头像图标色 `--text-muted → --text-secondary`。
- 复量：侧栏图标**最低对比度 3.94 → 4.77**（15 个图标全部 ≥4.5，stroke-width 统一 1.75px）。

### 🟡 顺带归一

- **圆角**：`var(--radius-sm, 6px)` 这类**带兜底**写法 **47 处** → 令牌本体（兜底把同一个 6px 写了两遍，
  还让"改令牌"不生效）。
- **阴影**：裸写的 `0 Xpx Ypx var(--shadow-*)` **27 处**收敛到 `--shadow-sm/md/popover` 三档，
  去掉 **3 处**重投影（`16px 48px` / `20px 60px`）。
- **死令牌**：再删 12 行定义（`--message-bubble-user(-border)` 在气泡改中性后归零引用；
  `--border-strong` 定义后确认无消费方，一并删除）。
- **两处如实修正**（上一轮我自己的推断被数据推翻）：**缓动不是问题**（`--ease-out/in-out/drawer/standard`
  四档令牌齐全，此前统计到的"5 种 cubic-bezier"其实是令牌定义本身）；**字重也不是问题**
  （真机计算字重 400 占 76%）。

### ✅ 装机版复量（同一套探针，1.16.115 → 1.16.116）

| 指标 | 1.16.115 | **1.16.116** | 参考实现 |
| --- | --- | --- | --- |
| 非 1px 的线 | **8 段** | **0 段** | — |
| 平行双线（≤6px 重叠 >24px） | **21 对** | **1 对** | — |
| 侧栏品牌色面色块 | 选中行 15% + 常驻工具行 8% | **0**（只剩品牌色头像圆 28×28） | — |
| 侧栏图标最低对比度 | **3.94:1** | **4.77:1** | — |
| 线条段数 | 654 | **407** | — |
| 选中态品牌色填充（静态） | 69 | **11** | 1 |
| 选中态中性色填充（静态） | 11 | **71** | 85 |
| 选中态品牌色边框（静态） | 44 | **3** | 2 |
| hover 里品牌色（静态） | 116 | **48** | 3 |

### 门禁与实测

- `light-theme-contrast.test.ts` 扩到 **12 例**：新增 **LIGHT-UI-10**（状态选择器不许用品牌色填充/边框，
  并**按语义**放行"静止态就是品牌色"的主操作按钮与页签指示器）、**LIGHT-UI-11**（实线边框只允许 1px，
  环/挖空例外）；LIGHT-UI-6 改为钉住"气泡中性无边框"。
- 顺带修好三处被改动**打红**的既有断言：`library-ops-integration` 的令牌前缀契约（补 `--state-`）、
  `library-ops-icons` 记录着已被清掉的 `var(--radius-md, 10px)` 兜底写法、`skin-compat-plugin` 契约里已删除的 `--border-strong`。
- `tsc` 0 错误；全量 **351 文件 / 5934 通过 / 16 跳过 / 0 失败**（仅剩三道"发布后才有内容"的版本门禁）；
  10 道审计门禁 exit 0；UI 一致性门禁 **error 0 / warn 0**；CSS 生效取值快照已按有意改动重写。
- ⚠️ **未做目视核对**（本会话模型不能读图）：以上全部是程序判据（computed style、双线配对几何、
  对比度计算、逐条色值台账）。**"现在看起来是否够精致"仍需人眼判断。**
