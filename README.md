...
> ⑥**工程侧**：UI 门禁从 9 条扩到 **22 条**（error 0 / warn 0）；`display: grid` 50 → **302** 处、`:has()` 父级状态 7 → **41** 处；**删掉 223 条从未被使用的 CSS 规则**（整族清理 `composer-*` / `code-block-*` / `native-title-bar-*` / `right-rail-*` / `hub-*` 等遗留）；新增**版本号一致性测试**（`package.json` / `tauri.conf.json` / `Cargo.toml` + CHANGELOG + PROJECT-GUIDE 任一漏改即失败）。全量 **201 文件 / 4566 用例通过**。设计与门禁细节见 `docs/UI-DESIGN-SYSTEM.md`。
>
