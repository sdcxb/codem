...
>
> **v1.16.0 更新（UI 设计体系统一：从「项目级」到「产品级」，2026-09-10）**：用户反馈「精细度比不上参考实现，它像产品级、我们像项目级」，于是这轮不靠感觉改配色，而是把决定观感的量**逐项量化对比**（两侧同一脚本、同一口径）后按可感知收益排序动手，共 44 波。
>
> ①**默认主题改为浅色暖中性**（画布近白、卡片暖灰、文字近黑），并修掉**启动时闪一下黑**（首屏镜像脚本在渲染前设好主题）；浅色档补齐到与深色档完全对称（此前只覆盖 49/76 个主题令牌）。
> ②**新增应用级菜单栏**（文件 / 视图 / 帮助，带快捷键提示，只放真实可用的命令），键盘完整可用（↓ 打开、↑↓ 选项、←→ 换菜单、Esc 关闭并把焦点交回触发器）。
> ③**窗口外壳对齐 mac 风格工具条**：外壳高度 36 → 44px、标签条两端渐隐、**专用拖拽区 + 左右安全区**（不再「整条标题栏可拖 + 每个按钮单独声明不可拖」）、标签条末尾补「新建对话」。
> ④**尺度校正**（这些是「看着不精致」的直接来源）：字重启用细档（区块标题 620 / 列表项 560 / 数值回到 400，此前「哪里都半粗」用了 235 次 600）；控件高度统一到 26/30/34/38/44（此前靠 padding 撑，同一行能差 1–6px）；圆角阶梯修正为严格单调（`--radius-xs` 此前竟比 `--radius-sm` 大）；149 处 13/15/18px 离刻度图标尺寸吸附回八级；圆角 / 间距 / 字号 / 颜色 / 字体栈全部收敛为令牌。
> ⑤**可访问性**：焦点环此前被 21 条组件规则 + 14 处内联样式抑制（键盘用户看不到焦点），现已全部交回统一焦点环令牌；57 条循环动画在系统「减少动效」下**全部停下**（此前只是「加速」），宠物精灵与图书馆场景的逐帧动画也尊重该偏好（此前全仓库没有一处读这个系统设置）；文件树条目与知识图谱节点从**键盘到不了**改成可聚焦。
> ⑥**工程侧**：UI 门禁从 9 条扩到 **24 条**（error 0 / warn 0）；`display: grid` 50 → **302** 处、`:has()` 父级状态 7 → **41** 处；**删掉 223 条从未被使用的 CSS 规则**（整族清理 `composer-*` / `code-block-*` / `native-title-bar-*` / `right-rail-*` / `hub-*` 等遗留）；新增**版本号一致性测试**（`package.json` / `tauri.conf.json` / `Cargo.toml` + CHANGELOG + PROJECT-GUIDE 任一漏改即失败）与 **CSS 结构完整性测试**（括号平衡 / 悬挂选择器逗号 / 顶层裸声明 / 空规则体，不依赖 postcss 即可拦住"批量改写留下的残骸"）。全量 **204 文件 / 4586 用例通过**。设计与门禁细节见 `docs/UI-DESIGN-SYSTEM.md`。
>
> **v1.15.0 更新（任务管理再收敛：场景归「子智能体」+ 角色只绑定团队/子智能体，2026-09-10）**：按「任务管理里还有与看板重复的功能」的反馈做三处整改。①**概览不再重复活动列表**：「概览」只保留四张统计卡（收件箱/Issues/委派/自动化）+ 一个「活动与用量」入口（直达「看板 → 用量 / 时间线」），活动明细只在看板里呈现；插件禁用时仍回退显示宿主活动预览。②**场景 + 设置移到「子智能体」页签**：宿主新增 `task-center.subagents` 扩展点，插件以 `LibraryOpsSceneView` 接管（子视图 **场景 | 设置**），宿主自带的子智能体列表作为插件禁用时的回退；「看板」页签收敛为 **看板（宿主 Issues）| 用量 | 工具 | 错误 | 时间线**；「设置」跟着场景走（它调的全是场景显示）；状态条新增子智能体/团队成员计数；新增 `requestView()` 统一跨页签跳转（如「用量 → 场景 →」），`Ctrl/Cmd+Shift+L` 改为打开「子智能体 → 场景」。③**场景角色只与团队/子智能体绑定**：队长（当前会话 + 各运行时团队队长）+ 团队成员 + 子智能体（有/无团队都显示）+ **仅在途委派**的目标会话；普通闲置会话不再各自变成角色、团队模板角色不再占位 —— 于是**没有团队、没有子智能体、没有在途委派时，馆内只有队长一人处于待命态**，且「用量」里的场景大卡与「场景」视图共用同一份快照。全量 vitest 196 文件 / 4538 用例通过（+15 跳过）+ tsc 零错误 + 版面审计（按页签：看板 5 视图 / 场景 2 视图 × 7 种窗口宽度）全 0 + DOM 审计 0 issue。
>
> **v1.15.2 更新（修复：有对话但「看板 → 时间线」是空的，2026-09-10）**：时间线的**事件来源太窄** —— 以前只有工具调用 / 团队任务 / 角色焦点 / 宿主遥测会变成事件，**对话消息本身不算事件**，所以只问答不调工具的会话时间线恒为空，且空态只有一句「暂无事件」无法判断原因。①**对话消息也进时间线**（最近 30 条）：用户发言 → `session`/active、助手回复 → `session`/ok、失败消息 → `error`/bad，纯工具调用消息不重复出条目；②**空态自解释**：列出采样实际看到的六项计数（会话/消息/工具调用/子智能体/运行时团队/遥测事件）+ 采集失败来源 + 「事件来自哪里」说明，类别筛选下为空时文案区分；③预览夹具事件数 4 → **60 条**（线上上限 120），长列表裁切/滚动以后也能被审计覆盖。顺带验证列表渲染本身无问题（60 条事件下版面审计 0 裁切 / 0 重叠）。全量 vitest 199 文件 / 4554 用例通过（+15 跳过）。
>
> **v1.15.1 更新（修复：任务管理「概览」没有滚动条，2026-09-10）**：v1.15.0 把「用量」迁进概览后概览内容变高，但内容区被裁掉且没有滚动条（下面的用量面板看不到）。根因是加宽面板时把内容区的 `overflow` 也绑到了同一个 `wide` 判断上（`wide ? "hidden" : "auto"`）——概览因此被 `hidden` 裁掉。修法：把「面板宽度」与「是否铺满一屏」拆开 —— `wide = board|subagents|overview`（宽度），`fillsViewport = board|subagents`（滚动归属），内容区只按后者决定 `overflow`，并加 `data-task-center-content="<tab>"` 便于断言排查。新增 DEDUP-7（源码契约：两个判断必须分离）与 DEDUP-8（真实渲染：概览/收件箱 `auto`，看板/子智能体 `hidden`）两条回归用例。全量 vitest 198 文件 / 4549 用例通过（+15 跳过）。
>
> **v1.15.0 更新（任务管理再收敛：用量迁进概览 + 场景归「子智能体」+ 结构树去重，2026-09-10）**：①**修复样式丢失**：重写视图外壳时漏了插件样式表 import → 看板/子智能体整片无样式、不自适应；现由 `LibraryOpsViewShell` 统一导入，并给版面审计加上 **`stylesApplied` 断言**（样式没生效直接判失败），审计改为按宿主位置分别跑（board / scene / 概览用量嵌入）。②**「用量」真正迁移进「概览」**：新增宿主扩展点 `task-center.overview`，插件把 KPI/健康度/活动分布/token 与成本整块贡献到概览，**看板里的「用量」视图删除**（原先只是个指回去的引用卡片，已删）。③**场景 + 设置归「子智能体」页签**（`task-center.subagents`，子视图 场景|设置），看板收敛为 看板|工具|错误|时间线；跨页签跳转统一 `requestView()`。④**场景角色只绑定团队/子智能体**：队长（当前会话 + 各运行时团队队长）+ 团队成员 + 子智能体（有/无团队）+ 仅在途委派会话；闲置会话与模板角色不再入馆 ⇒ 无团队/无子智能体/无在途委派时只有队长待命。⑤**功能结构树 + 去重**：新增 `docs/TASK-CENTER-MAP.md`（8 页签 + 3 个接管点，落到最小叶子，25 项重叠判定），本版修掉 13 项 —— 概览/错误面板的「定位」死按钮、子智能体父会话下钻断链、概览 token/成本重复三遍、概览手写事件列表（改用 `EventList`）、Issue 状态元数据 5 份合并为唯一表（顺带补齐自动化漏掉的 backlog/todo）、采样双轮询合并为共享引用计数定时器、`SquadsTab` 跟随项目重查、底栏委派限制读真实配置、删除死代码 `DelegationPanel`。全量 vitest 198 文件 / 4547 用例通过（+15 跳过）+ tsc 零错误 + 版面审计 3 位置 × 7 宽度全 0 + DOM 审计 0 issue。
>
...
> **v1.9.7 更新**：dsh 插件市场全面落地（对标 deepseek-harness 插件生态）+ 插件架构审计修复 + 皮肤兼容契约（执行轨迹持久化 / 首页无会话自动建全局对话随版）— 分析 harness 插件机制（npm 包 + cordis.patch.yml 分层装配 + Loader 激活）后评估：Codem 装配框架同构但无法加载任意 npm 插件（运行时鸿沟），dsh-compat 桥重写接入。落地：①**插件市场目录**（50 条真实官方 @deepseek-ai/dsh-* 包，与 harness packages 逐一核对）按三类兼容性评估——bundled 37（Codem 内置等价，codemAnchor 指向真实插件且一对一，DM-4 校验）/ adaptable 9（dsh 协议、无第三方依赖，经 dsh-compat 桥接）/ unsupported 4（依赖 Node/npm 运行时，诚实标注）+ npm registry 在线检索（**跟随搜索框输入**）②**插件管理弹窗新增「插件市场」Tab**：浏览/搜索/**动态分类**（无空分类）+ 兼容徽标 + bundled 一键"安装并启用"对应内置插件（核心恒启条目只读显示"已启用（核心）"，可安全卸载条目提供"禁用"）+ 小窗自适应 ③**dsh-compat 接入运行**：懒解析代理别名（dshLlm/dshShell/dshFs/dshTools/dshSessions/dshEvents/dshCredentials 恒注册、调用时现取真实服务）；**服务名对齐矩阵审计**（harness 插件 inject 78 项 vs Codem provides 203 项）证实核心 seam 同名直通、差异经别名承接、宿主装配名诚实标注 ④**插件皮肤兼容契约（Skin Token Contract）**：插件 UI 只消费皮肤令牌、零硬编码色（skin-tokens.ts 审计 + SC-1~4 门禁），自动适配 default 亮/暗 + dream + hub 四视觉态（docs/SKIN-PLUGIN-CONTRACT.md）⑤测试 DM-1~5 / DC-1~4 / PM-1~5 / MT-1~6 / SC-1~5 / R-1~9 / LC-1~6（含真实依赖图下全部 bundled 锚安装级联可达性验证；审计修复：enable 失败必报错 / error 持久化 / loading 连点保护 / manager 单例幂等 / 元数据同源 / **禁用=真卸载 ctx 插件** / 管理入口 core 防死锁 / 动态分类 / 无障碍。全量 152 文件 / 4141 用例通过 + tsc 零错误）。v1.9.7 另有**同版本补丁覆盖发布**（commit 7d1f329/88b49f2）：CodeGraph defer 接入与一键安装 / 技能市场联网搜索卡死修复 / 输入框长 URL 光标视觉错位修复 / GitHub URL 标签逐字符重复生成修复。
>
> **v1.11.2 更新（zg 在线安装 Node 源根治 + 审计四坑修复 + 功能文档体系，2026-09-09）**：①**zvec-grep 在线安装根治**——便携 Node 并入 zg 单包（162MB，含隔离 Node，安装只从 GitHub Release 取包、不再访问 nodejs.org/npmmirror），彻底解决部分网络下官方/镜像源被拦致"一键安装"失败；系统 Node≥22 自动复用，离线 zip 亦可全离线。②**审计四坑修复**——PPT「导出 PPTX」从"HTML 改名"改为真实可打开的 OOXML 图片型 PPTX（OPC 结构校验）；纠偏模型配置面板真实接线（codem-correction-model 持久化 + fact_check 专属/回退主模型诚实标注）；Whisper 语音输入双引擎闭环（设置选引擎 + 麦克风录音云端转写）；会话内搜索激活（入口+点击跳转，修隐藏过滤残留）。③**功能文档体系**——项目功能说明介绍（20 大域 152 亮点，皮肤独立域细粒度）+ 项目功能树-全量（18 路只读审计 2779 项叶子，含证据与宣传句）+ 分层检索索引。④**覆盖包新增「手动添加自定义模型名」**——服务商 /models 不在列表内的内测/灰度模型（如 deepseek-v4.1-flash-expires-on-0910，调用同协议仅改模型名）可在「设置 → 模型与 API Key → 对应 Provider」直接添加（chips 可删）；引擎/设置/聊天模型下拉/方案面板统一合并生效，不污染服务器缓存。全量 vitest 175 文件 / 4314 用例通过 + tsc 零错误。
>
> **v1.11.1 更新（zvec-grep 语义检索集成 + archify 图表技能 + UI/体验修复打包，2026-09-09）**：①**zvec-grep（zg）语义检索（可选增强，不改架构）**——本地「语义向量+BM25+rg」统一检索，与内置 grep 双轨并行（精确锚点→grep；措辞未知/语义/跨文件→zvec_grep_search，模型按工具描述智能路由）；运行时按用户主动安装于应用数据目录（不进安装包），经 MCP stdio（`zg server --stdio` 自动起/复用 daemon）接入；Rust 新增 `http_download_ext`/`extract_zip` 命令；插件市场新增「本地语义检索」卡片——一键在线安装（复用系统 node）/导入离线单包(.zip)/为当前项目建索引/Embedding 模型切换（默认 potion-code-16m-v2 ~33MB MIT）；发布单合并包 `codem-zvec-win-x64.zip`（~125MB 运行时+模型）。②**archify 图表技能内置**——架构/工作流/时序/数据流/生命周期图 JSON-IR→自包含交互 HTML（showcase 9/9 校验）；已用其产出 Codem 架构图与功能结构图（artifacts/archify/）。③**UI/体验修复打包（v1.11.0 覆盖包内容随本版首发）**——标题栏可拖拽修复与品牌 Logo、左下角用户头像即时刷新、右侧栏磨砂玻璃、消息导航轨全内容比例铺轨/纯紫点/最小间距、Git 分支控件并入右侧栏、右侧栏智能体 tab 收敛。全量 vitest 171 文件 / 4260 用例通过 + tsc 零错误 + cargo check 通过。
>
> **v1.11.0 更新（团队体系深合并 + 智能体双维度面板 + 审计修复，2026-09-08）**：①**团队体系深合并（B 方案）**——把两套"团队"（TaskCenter 静态 Squad 模板 + agent-teams 运行时 DAG 团队）合并为单一「团队 = 角色模板 + 运行时编排」：Squad 升级为团队模板（TeamTemplate/toTeamTemplate），`squad_dispatch` 桥接 agent-teams（按模板建队：当前会话=队长、按角色 spawn 可续聊成员、任务入共享池调度），`squad_status`=模板+运行时摘要；TaskCenter tab `squads`→`teams`（TeamTab：说明+运行时活动+模板管理），对话旁「团队活动」按钮收敛为快捷入口；删除旧事件派发路由与死码（盘点矩阵 docs/AGENT-SYSTEMS-MATRIX.md、分阶段计划 docs/TEAM-CONSOLIDATION-PLAN.md）。②**「智能体与团队」双维度面板**——顶部「子智能体」页面升级：有活动团队时首部显示团队卡片（成员=角色+状态点），点成员行内展开个体动态预览（不跳页，活动/结果摘要 + 「完整详情 →」），团队成员从平铺去重；修复 engine snapshot 丢成员 id 的下钻/去重 bug。③**审计修复**：executor end 失败判据补全、宠物卡轻量推送误清、runPs 超时余量等。全量 vitest 168 文件 / 4239 用例通过 + tsc 零错误。
>
> **v1.10.0 更新（EAC 对标 DSH-Desktop-EAC 第①②③④项 + 全量审计修复，2026-09-07）**：仿照 DSH-Desktop-EAC 完成四项功能（顺序按工作量递增）。①**手机连接 @codem/phone-link（对标 dsh-phone）**：Rust LAN HTTP 地基（零新依赖手写 HTTP/1.1）——扫码配对（token URL 5min 轮换 + 桌面批准 + HttpOnly cookie + secret 仅存 sha256 落盘，重启保配对）+ 手机浏览器浏览/续聊/新建桌面会话（请求经事件代理到 WebView 引擎，不编造数据）。②**微信 ClawBot 桥 @codem/wechat-bridge（iLink 直连，官方 openclaw-weixin 2.4.6 协议）**：Rust 传输层（8 态扫码登录 + 配对码 + getupdates 长轮询 + 文本收发 + 10条/24h 配额软记账）+ TS 引擎桥（peer→持久会话 executeSessionTurn 驱动 + /help /status /new /attach /model /clear /reconnect 等命令 + 白名单准入）。③**@codem/computer-use 电脑操作（对标 EAC computer-user，Codex-style）**：10 个 computer_* 工具（截屏/点击/输入/组合键/滚轮/拖拽），PS 零依赖后端，默认 manual 手动批准（/computer 会话批准），computer_see 视觉理解。④**宠物大肥鱼式状态卡（对标 dsh-dafeiyu）**：真实 Agent 事件驱动（项目名/阶段/真实步骤进度，不编造完成百分比）。随后 **四路只读审计**（注册一致性 / Rust↔TS 桥 / computer-use 工具链 / 引擎侧接线）并修复：computer-use PS 调用约定断链（P0，实测全挂）、动作名失配、宠物卡隐藏信号丢失、executor 异常终止不落库（静默无回复）、/api/chat 15s 必 504、配对 cookie 不可重试、「插件禁用=关闭」真实生效（微信/手机外部可达面 + computer 工具门禁）、ui-pet/mimo-auth/hot 元数据对齐等。全量 167 文件 / 4231 用例通过 + tsc 零错误 + Rust cargo test 38 通过。文档：docs/EAC-PHASE2-STATUS.md（四项实施记录）+ docs/EAC-GAP-ANALYSIS.md。
>
> **v1.9.9 更新（EAC 对标 DSH-Desktop-EAC）**：差距分析（47 内置插件，矩阵 docs/EAC-GAP-ANALYSIS.md）→ 仿照实施。①**UI 交互改进**：消息「编辑并回退」（fork 保留原会话）/ 节点导航升级（滚动容器修复 + hover 预览 + 滚轮 + 📌 消息精选 pin）/ 输入框失焦折叠 / 字号设置真正生效（--ui-font-scale）/ 设置搜索可用 ②**新增可启停插件**：临时会话 side-session（悬浮窗独立问答不污染主会话）/ persona 人设卡（管理 UI + # Persona 注入 + 热重载，修复 SOUL 孤儿缺陷）/ **@codem/agent-teams 团队编排**（依赖任务 DAG + attempt 防覆盖 + 成员邮箱 + 共享调度 + 活动面板 + 10 工具，defer 省 token）。验证清单 docs/EAC-VERIFICATION.md。全量 163 文件 / 4204 用例通过 + tsc 零错误。
>
> **v1.9.8 更新**：**对话用量统计 + 缓存命中率真实化（对标 anywhere-labs/dsh-desktop）+ 稳定前缀优化 + 真实请求实证** — ①**真实数据链路**：采集 DeepSeek `prompt_cache_hit_tokens` / OpenAI `cache_read_input_tokens`（TokenUsage 扩展 cacheHitTokens/uncachedInputTokens，usage-normalize 口径精确化）→ 每轮助手消息下显示 uncached 输入/输出/**诚实精度缓存命中率**（99.97% 不圆成 100）/成本（含缓存价差计价，uncached 按原价、命中按缓存价）；provider 未上报缓存字段时不显示误导性的"命中 0%"（与 dsh 一致）②**用量统计面板缓存命中卡**：设置→用量统计概览新增近 7 天「缓存命中率」聚合卡 ③**稳定前缀优化**：date 等每分钟易变字段从系统提示中段移出、**尾置为独立 # Current Date 段**——不同分钟的两份 prompt 公共前缀覆盖 >95%（测试断言防回退），缓存尽量不被切断 ④**真实请求实证（经授权受控探测）**：DeepSeek 前缀缓存需足够大的前缀——短请求恒 miss，长前缀（3259 tokens）完全相同重复命中 98.2%；**96K token 前缀纯重复稳态命中 99.947%**，达 dsh 99.97% 量级（dsh 高值来自更长前缀稳态，同 key 同形态表现一致）⑤端到端集成测试（真实 AgenticLoop + 脚本化 provider 回放，含缓存价差成本累计）。全量 157 文件 / 4159 用例通过 + tsc 零错误。
>
...
> 任务管理再收敛：场景归「子智能体」+ 角色只绑定团队/子智能体。详细见 `CHANGELOG.md`。

**概览不再重复「最近活动」**
- 「概览」= 四张统计卡 + 「活动与用量」入口（直达「看板 → 用量 / 时间线」）
- 插件禁用时回退显示宿主自带的最近活动预览（不丢信息）

**场景 + 设置移到「子智能体」页签**
- 新扩展点 `task-center.subagents` → 插件 `LibraryOpsSceneView`（场景 | 设置）；禁用时回退宿主列表
- 「看板」收敛为 看板（宿主 Issues）| 用量 | 工具 | 错误 | 时间线
- 新增 `requestView()` 统一跨页签跳转；`Ctrl/Cmd+Shift+L` → 打开「子智能体 → 场景」
- 两个页签都加宽到 1180px；状态条显示子智能体 / 团队成员计数

**场景角色绑定规则（角色 = 真实干活的人）**
- 队长（当前会话 + 各运行时团队队长）/ 团队成员 / 子智能体（有团队无团队都显示）/ 仅在途委派的目标会话
- 普通闲置会话不再各自变成角色；团队模板角色不再占位
...
- 根因：时间线事件来源太窄 —— 对话消息本身不算事件（只有工具调用/团队任务/角色焦点/遥测）
- 修复：对话消息也进时间线（用户发言 / 助手回复 / 失败消息）；空态改为「自解释」
  （列出会话/消息/工具调用/子智能体/团队/遥测六项计数 + 采集失败源 + 事件来源说明）
- 预览夹具事件数 4 → 60，长列表裁切/滚动纳入版面审计

### 2026-09-10（v1.15.1）

> 修复：任务管理「概览」没有滚动条，下面的内容看不到。

- 根因：加宽面板时把内容区 `overflow` 也绑到 `wide` 上（`wide ? "hidden" : "auto"`），
  概览因此被裁掉 —— 概览是普通文档流页面，必须由内容区滚动
- 修法：拆成 `wide`（宽度：board|subagents|overview）与 `fillsViewport`（滚动归属：board|subagents）
- 新增 `data-task-center-content="<tab>"` 标记 + DEDUP-7/8 两条回归用例（概览/收件箱 `auto`，看板/子智能体 `hidden`）

### 2026-09-10（v1.15.0）

> 任务管理再收敛：用量迁进概览 + 场景归「子智能体」+ 结构树去重。功能结构树见 `docs/TASK-CENTER-MAP.md`。

**修复：看板 / 子智能体样式丢失（P0）**
- 根因：视图外壳漏导入 `styles/library-ops.css` → 整片无样式、页面不自适应、内容显示不全
- 修复：外壳统一导入 + 新增样式入口守卫测试；版面审计新增 `stylesApplied` 断言（样式未生效即失败）

**「用量」迁移进「概览」（并删除看板里的用量视图）**
- 新扩展点 `task-center.overview`：插件把 KPI / 健康度 / 活动分布 / token 与成本贡献到概览
- 看板视图收敛为 看板 | 工具 | 错误 | 时间线；原先那个「都在看板里 →」的跳转卡片删除
- 用量块自带容器查询上下文（`.lo-embed`）与采样，不渲染 `.lo-task` 外壳
- 插件禁用时概览回退宿主自带的「最近活动」预览

**场景 + 设置归「子智能体」页签**
- 新扩展点 `task-center.subagents` → `LibraryOpsSceneView`（场景 | 设置）；禁用时回退宿主列表
- 跨页签跳转统一 `requestView()`；`Ctrl/Cmd+Shift+L` 打开「子智能体 → 场景」
- 状态条显示子智能体 / 团队成员计数；场景角色详情新增「打开父会话」
...
**结构树去重（13 项，详见文档）**
- 概览/错误面板「定位」死按钮 → 改为「选中角色 + 切到场景」
- 概览 token/成本重复三遍 → 删掉状态卡两行；概览手写事件列表 → 复用 `EventList`
- Issue 状态元数据 5 份 → 唯一表 `issue-status-meta.ts`（自动化补齐 backlog/todo）
- 采样双轮询 → 引用计数的共享定时器；`SquadsTab` 跟随项目重查；底栏委派限制读真实配置
- 删除死代码 `DelegationPanel.tsx`；`ErrorsPanel` 自带时钟函数删除
...
> 图书馆并入「看板」+ 场景图可上传/自动对位 + 界面自适应与图标统一。
> 详细设计与审计记录见 `docs/LIBRARY-OPS-PLUGIN.md` 第九节。

**重构：图书馆并入「看板」页签（不再有独立面板）**
- 图书馆场景的初衷是「谁在做什么、在哪做」的可视化看板 → 与宿主「看板」页签（Issues 状态列）合成一个页签
- 看板 = 默认视图（宿主 Issues 看板）；插件追加 **场景 / 用量 / 工具 / 错误 / 时间线 / 设置** 六个视图
- 去掉与任务管理重复的入口：「总览」→「用量」（并合并原「成本」）、「团队」→ 任务管理「团队」、「会话」→「委派 / 子智能体」+ 场景花名册
- 宿主扩展点：`task-center.board` slot（`BoardTab` 用 `SlotBridge` 渲染，插件禁用即回退到自带 Issues 看板）
- 删除 `LibraryOpsPanel.tsx` / `LibraryOpsLauncher.tsx`（独立面板与悬浮入口）与 store 的 `open/close/togglePanel`

**新增：场景图可上传替换**
- 设置 →「场景图片」：内置像素画 / 内置 AI 场景图一键切换，或**把图片拖到场景上**上传
- 图片以 Blob 存浏览器 IndexedDB（不写宿主数据），支持 PNG/JPG/WebP/AVIF/GIF/BMP，≤32MB、≥640×360
- `scripts/build-library-ops-scene-preset.mjs`：任意图 → 2752×1536 WebP 预设 + 缩略图

**新增：场景自动对位（不再需要手工拖拽）**
- 上传后自动拟合：地面掩码（亮度 + 局部平滑）vs 内置房间掩码，做「缩放 × 平移」粗到细搜索最大化 IoU
- 输出 `{scale,x,y}` + 置信度，置信度足够就自动应用；不满意点「手动对位编辑器」再微调（房间框/走道节点，按图分别保存）
- `core/scene-align.ts` 全部纯函数（7 个单测用例，含已知变换反解）

**改造：界面自适应（容器查询）**
- `.lo-task` 设为容器，按**面板实际宽度**分档：≥1080 三栏 / 980–1080 收事件流 / <980 单列 / ≤820 导航图标条；按容器高度压缩场景
- 固定死值全部换弹性值；子视图改为**自然高度 + 内容区滚动**（不再被塞进固定高度导致挤压/重叠）
- 新增 `tools/preview/audit-layout.mjs`：7 种窗口宽度 × 每个子视图检查裁切/元素重叠，全部 0

**改造：图标与样式统一**
- 新增 `components/icons.tsx`（`LoIcon` + 49 个语义名 → lucide-react），数据层只存语义名；全部 emoji 替换
- 卡片/标签/圆角/底色对齐宿主 `.card` / `.badge`（10px 圆角 + `--bg-secondary` + hover 边框）

**审计与修复：任务管理（宿主 8 个页签，两轮共 29 项）**
- **P0**：「子智能体」列表恒空（render 里 `require()` 取运行时，ESM 下必抛错）→ 顶层 import + 订阅
- **P1**：事件只认 `teams` 页签 / 子智能体条目点不开父会话 / 看板拖拽失效与同列误写库 / 看板缺 `blocked`、`cancelled` 列 / 看板内容无法滚动 / 委派统计与列表口径不一致 / 面板打开期间不跟随页签请求 / **LLM 工具路径**无项目边界（`issue_list` 退化成全库查询、`issue_create` 建孤儿 Issue）/ 非法 status 让整个应用崩到错误边界
- **P2**：自动化「停止所有」无法恢复（且编辑触发器会静默重启）/ cron 步长为零 → `RangeError` 让后续触发器全部失效 / Issues 筛选缺 backlog、cancelled / 收件箱徽标随筛选变化且点击不跳转 / **无项目时跨项目串数据**（含孤儿 Issue）/ 详情面板点当前状态写假评论 / 切项目不重查 / 重启后委派历史与统计全丢 / `single` 槽位「最低优先级胜出」/ `IssueCard` hover 覆盖状态色条 / 详情面板切换不刷新
- **P3**：「查看完整时间线」不切插件子视图 / 新建 Issue 被筛选藏起来 / 详情面板每击键 N+1 次 SQL / 收件箱只增不减 / 删除死代码 `TeamsPanel`
- 新增 `src/test/task-center-audit-fixes.test.tsx`（12 用例）+ `src/test/task-center-audit-fixes-2.test.tsx`（14 用例）钉住以上行为

**去重：任务管理内不再有相似功能**
- 图书馆「团队 / 会话 / 总览 / 成本」入口删除或合并（团队→任务管理「团队」、会话→「委派 / 子智能体」、总览+成本→「用量」）
- 「概览」只保留统计卡 + 最近 5 条活动，并给「查看完整时间线」入口（插件启用时出现）
- 场景明确标注为「子智能体 / 团队」的可视化表达，不再重复提供明细列表
- 自动化触发器唯一入口是「任务管理 → 自动化」（设置面板只留跳转提示）

**修复**
- dev 模式白屏 `process is not defined`（`src/stubs/process-polyfill.ts`，dev-only shim）
- Vite 文件监听 `*.tmpdir` EBUSY 导致 `tauri dev` 直接退出
- 用量/设置/错误等子视图在窄窗口下元素重叠、内容被裁切
...
> 图书馆运营监控插件（团队/子智能体可视化）+ 全面审计修复 28 项（含 1 项宿主 Bug）。
> 详见 `docs/LIBRARY-OPS-AUDIT.md` 与 `CHANGELOG.md` 的 [1.12.0] 条目。

**新增：`@codem/ui-library-ops` 图书馆运营监控（完全独立可启停）**
- 团队角色 / 子智能体 → 各自不同的动画角色（id 确定性生成 34560 种外观组合；岗位影响头饰与道具）
- 11 种工作动画由真实工具调用与任务状态驱动（read→阅读 / write→撰写 / bash→执行 / web_search→检索 / ask_user→等待授权）
- 等距 2.5D 图书馆：10 个职能岗位 + 网格 BFS 寻路 + 工位槽位 + 名牌 / 气泡 / 深度排序
- 监控界面对标 lobster-pet：标题栏 + 9 页签 + KPI 卡 + 健康度环 + 14 天热力图 + 环形图 + 小时柱状图 + 实时事件流；图书馆即监控界面内的「场景」页签
- 相机可交互：滚轮缩放 / 拖拽平移 / 双击复位 / 选中角色镜头平滑居中
- 只读接入会话、运行时团队、子智能体、团队模板、智能体定义、工具调用、token 与成本、遥测；零写入宿主
- 挂宿主已有 `app.overlay` slot → App.tsx 零改动；插件管理可禁用；面板关闭即停止采样

**审计修复（28 项，含宿主 Bug）**
- 宿主：agent-teams 成员完成任务后状态永不回落 `working`（调度器不再派活）→ 新增 `releaseAssigneeIfIdle()`
- 插件：活跃会话 Map 判定失效 / 气泡永不消失 / 到岗动画不切换 / `done` 动画永久定格 / 角色站到岗位外 / 等距几何双重偏移（区域高亮整体放大错位）/ 道具转向换手 / 耳机横梁被 fill 覆盖 / 采样无超时兜底 / 窗口 resize 重置缩放 等
- 新增 4 个测试文件（真实服务联动 8 例 / 几何不变量 9 例 / 渲染几何 7 例 / 门控 1 例）+ `tools/preview/` 视觉预览与 DOM 审计脚本

**验证**：`tsc` 零错误；vitest **184 文件 / 4414 用例通过**（连续 4 次复跑稳定）；`vite build` 成功；headless 浏览器 DOM 审计 12/12 角色落在自己岗位的等距包围盒内、0 处 NaN。
...
> 对话步骤语义化（对标 dsh todo 语义列表）+ token 消耗审计修复 + 全面功能审计修复。

**对话步骤语义化 + 动态插入（对标 dsh 客户端）：**
- 执行型任务强制 LLM 生成语义步骤：不再出现"1、回答问题；2、执行命令"——任务意图检测修复了"修复卡死的问题"被判纯问答的根因；步骤标题面向任务（分析原因→诊断链路→修复→验证）
- 新增 `update_plan` 工具：执行中发现必须先处理的新问题时，把新步骤插入当前进行中的步骤之前，编号顺延、总步数自动更新、界面"第X/X步"与列表即时刷新
- 模型每轮都能看到"第 X/Y 步 + [完成/进行中/待办]"计划状态，语义计划不再混入泛化步骤

**token 消耗审计修复（对比 dsh 省数倍的根因）：**
- read 单次结果上限 100k→50k 字符（对齐 dsh ~50KB；中文内容此前是 dsh 的 6 倍）
- 陈旧大工具结果 head+tail 裁剪（对齐 dsh pruner）：保留最近 2 条完整，更早 >8KB 结果从 12-25k token 降到 ~1.3k
- 7 个低频大工具延迟加载（PPT/浏览器/Figma/GitHub/工作流/图片/语音）：每轮工具 schema 10.6k→7.5k token
- 系统提示词工具目录/guidance 三重复裁剪
- 上下文选择预算对齐模型真实窗口（不再超窗口被服务端截断）
- 上下文截断丢弃早期消息时插入零成本折叠摘要（打断"失忆→重复读取执行"的恶性循环）

**功能审计修复：**
- PTY 关闭/退出杀整个进程树（长命令孙进程不再残留为孤儿）
- 终端创建失败不再残留空 DOM
- 4 处网络调用补超时（网页抓取/搜索/能力发现不再永久挂起）
- 托盘"退出"先 flush 数据库再退出（不再丢最近写入）

**回归测试：** 步骤 17 用例 + 上下文折叠 7 用例。全量 145 文件 / 4102 用例通过 + tsc/cargo 零错误。
...
> 安全模式（完全访问）修复 + 工具调用配对修复 + 输入框历史/记忆正则修复 + 引导栏 UI 对标 wecode + 思考过程紫色样式恢复。

**安全模式（完全访问）修复：**
- 修复选择"完全访问"后重启仍弹审批：`dbReady` 时序导致 securityMode state 回退 ask，现在 DB 就绪后重新同步 + 切换项目重新解析（项目级 > 全局）
- 委派/后台任务遵循用户安全模式：`executor.ts` 不再硬编码 auto，跨会话委派同样放行 full
- 修复 write 拒绝误判：任何工具输出含 "User rejected the overwrite" 字面量被误判为用户拒绝，限定为 write 工具本身

**工具调用配对修复：**
- 上下文截断部分工具结果时按 `tool_call_id` 精确配对，修复 API 400 "insufficient tool messages"（DeepSeek/OpenAI 严格配对要求）

**输入框历史浏览修复：**
- 多行输入（wrap 折行）时光标在第二行按 ↑ 误触发历史填充，改为视觉行判断（镜像测量）

**其他：**
- 记忆检索正则元字符转义（`+`/`*`/`(` 等导致 SyntaxError）
- 引导栏按钮 UI 对标 wecode 优化：卡片式三操作（立刻引导/编辑/取消）+ 状态胶囊 + 圆角浮卡样式
- 思考过程紫色样式恢复（对标 v0.96.0：紫色文字 + 紫色左边框）

**回归测试：** 新增 10 个 repro 文件（34 用例）。全量测试通过 + `tsc --noEmit` 零错误。
