...
>
> **[Unreleased] EAC 对标（DSH-Desktop-EAC）**：差距分析（47 内置插件）→ 仿照实施。①**UI 交互改进**：消息「编辑并回退」（fork 保留原会话）/ 节点导航升级（滚动容器修复 + hover 预览 + 滚轮 + 📌 消息精选 pin）/ 输入框失焦折叠 / 字号设置真正生效（--ui-font-scale）/ 设置搜索可用 ②**新增可启停插件**：临时会话 side-session（悬浮窗独立问答不污染主会话）/ persona 人设卡（管理 UI + # Persona 注入 + 热重载，修复 SOUL 孤儿缺陷）/ **@codem/agent-teams 团队编排**（依赖任务 DAG + attempt 防覆盖 + 成员邮箱 + 共享调度 + 活动面板 + 10 工具）。差距矩阵 docs/EAC-GAP-ANALYSIS.md。全量 163 文件 / 4202 用例通过 + tsc 零错误。
>
